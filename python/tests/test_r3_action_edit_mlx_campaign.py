...
import pytest
import r3_action_edit_mlx_campaign as campaign
...
def test_cross_host_smoke_proof_is_content_addressed_and_all_checks_must_pass(
    ...
    assert campaign.validate_smoke_proof(path, s1_cache_id="1" * 64) == proof
    ...
    with pytest.raises(campaign.CampaignError, match="smoke proof"):
        campaign.validate_smoke_proof(path, s1_cache_id="1" * 64)
...
def test_inert_queue_runs_four_nonduplicative_arms_and_exact_classification(
    ...
    bundle = repository / "artifacts/bundles/r3"
    ...
    control = repository / "artifacts/experiments/r3/control"
    ...
    assert len(specs) == queue["task_count"] == 18
