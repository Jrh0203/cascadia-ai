...
from cascadia_mlx.action_ranking_promote import (
    load_promoted_action_ranking_model,
    promote_action_ranking,
)
...
def _qualified_run(root: Path, *, passed: bool = True) -> Path:
    ...
    (run / "run.json").write_text(json.dumps({"schema_version": 1, "kind": "action-delta-ranking"}))
...
def test_action_ranking_promotion_requires_and_packages_test_qualification(
    tmp_path: Path,
) -> None:
    run = _qualified_run(tmp_path)
    output = promote_action_ranking(run, tmp_path / "model")
    loaded = load_promoted_action_ranking_model(output)
    manifest = json.loads((output / "model.json").read_text())

    assert manifest["kind"] == "action-delta-ranking"
    assert manifest["test"]["passed"] is True
    assert loaded.config.architecture == "action-delta-ranker-v1"


def test_action_ranking_promotion_rejects_failed_test_gates(tmp_path: Path) -> None:
    run = _qualified_run(tmp_path, passed=False)
    with pytest.raises(PromotionError, match="did not pass"):
        promote_action_ranking(run, tmp_path / "model")
