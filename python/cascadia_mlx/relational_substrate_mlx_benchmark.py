"""Fresh-process serving benchmark for the frozen ADR 0161 tournament."""
...
class RelationalServingBenchmarkError(RuntimeError):
    """An isolated ADR 0161 benchmark request or result is inconsistent."""
    ...
    *,
    ...
    completed = subprocess.run(
        [
            ...
            "-B",
            "-m",
            "cascadia_mlx.relational_substrate_mlx_benchmark",
            "--request",
    ...
    if completed.returncode != 0:
        ...
        raise RelationalServingBenchmarkError(
            f"isolated relational substrate benchmark failed: {detail}"
        )
    result = _read_json(
        result_path,
        "isolated relational substrate benchmark result",
    )
...
def create_serving_benchmark_request(
    *,
    ...
    if arm not in ARMS:
        raise ValueError("relational serving benchmark arm is unknown")
    if verification_source not in VERIFICATION_SOURCES:
        raise ValueError(
            "relational serving benchmark verification source is invalid"
        )
...
def execute_serving_benchmark_request(
    ...
    performance = benchmark_relational_substrate(
        model,
        validation,
        arm=identity["arm"],
    ...
    performance["combined_with_r6"] = _combine_with_r6(
        performance,
        r6,
    )
    runtime = {
...
def _validate_request(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or open_data_verification_id(
            identity["open_data_verification"]
        )
        != identity.get("open_data_verification_id")
    ):
        raise RelationalServingBenchmarkError(
            "isolated relational benchmark request is malformed"
        )
    return identity
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="cascadia-r6-replay-") as raw:
        ...
        if completed.returncode != 0:
            detail = completed.stderr.strip() or completed.stdout.strip()
            raise RelationalServingBenchmarkError(
                f"R6 replay failed: {detail}"
            )
        report = _read_json(output, "R6 replay result")
    ...
    if (
        ...
        raise RelationalServingBenchmarkError(
            "R6 replay result is malformed or inexact"
        )
    return identity


def _combine_with_r6(
...
def _validate_result(
    ...
    if (
        ...
        raise RelationalServingBenchmarkError(
            "isolated relational benchmark result is malformed"
        )
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one isolated ADR 0161 serving benchmark"
    )
...
def main() -> None:
    ...
    request = _read_json(
        args.request,
        "isolated relational benchmark request",
    )
