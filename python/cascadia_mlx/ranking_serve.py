"""Long-lived scalar inference service for the MLX action ranker."""
...
MESSAGE_RANKING_PREDICTION = 0x8002
...
def serve_ranking(
    ...
    """Serve scalar afterstate scores until shutdown or EOF."""
    while True:
...
def main() -> None:
    ...
    if args.model_dir is not None:
        model = load_promoted_ranking_model(args.model_dir)
    ...
    print(
        f"cascadia-mlx ranking service {source_name} on {mx.default_device()}",
        file=sys.stderr,
    )
    serve_ranking(model, sys.stdin.buffer, sys.stdout.buffer)
