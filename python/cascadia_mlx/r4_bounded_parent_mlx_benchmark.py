"""Isolated serving benchmark for the frozen ADR 0156 comparison."""
...
class R4ServingBenchmarkError(RuntimeError):
    """An isolated ADR 0156 benchmark request or result is inconsistent."""
    ...
    *,
    ...
    completed = subprocess.run(
        [
            ...
            "-B",
            "-m",
            "cascadia_mlx.r4_bounded_parent_mlx_benchmark",
            "--request",
    ...
    if completed.returncode != 0:
        ...
        raise R4ServingBenchmarkError(f"isolated R4 parent benchmark failed: {detail}")
    result = _read_json(result_path, "isolated R4 parent benchmark result")
...
def create_serving_benchmark_request(
    *,
    ...
    if arm not in ARMS:
        raise ValueError("R4 parent serving benchmark arm is unknown")
    if verification_source not in VERIFICATION_SOURCES:
        raise ValueError("R4 parent serving benchmark verification source is invalid")
    ...
    identity = {
        ...
        "parent_cache": str(parent_cache.resolve()),
...
def execute_serving_benchmark_request(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise R4ServingBenchmarkError("isolated benchmark checkpoint differs from its request")
    model.eval()
    decision_rows = identity["decision_rows"]
    performance = benchmark_r4_bounded_parent(
        model,
        validation,
        arm=identity["arm"],
...
def _validate_request(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise R4ServingBenchmarkError("isolated R4 parent benchmark request is malformed")
    return identity
...
def _validate_result(result: dict[str, Any], request: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise R4ServingBenchmarkError("isolated R4 parent benchmark result is malformed")
...
def _read_json(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise R4ServingBenchmarkError(f"cannot read {label}: {error}") from error
    if not isinstance(value, dict):
        raise R4ServingBenchmarkError(f"{label} must be a JSON object")
    return value
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run one isolated ADR 0156 benchmark")
...
def main() -> None:
    ...
    request = _read_json(args.request, "isolated R4 parent benchmark request")
