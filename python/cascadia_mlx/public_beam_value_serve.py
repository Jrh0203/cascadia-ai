"""Long-lived scalar inference service for the public beam-value model."""
...
from cascadia_mlx.public_beam_value_promote import load_promoted_public_beam_value_model
...
def serve_public_beam_value(
    ...
    while True:
        ...
        try:
            ...
            current_base_score = actions["immediate_score"].astype(np.float32)
            current_base_score -= np.sum(
                actions["immediate_deltas"].astype(np.float32),
                axis=-1,
            )
...
def main() -> None:
    ...
    if args.model_dir is not None:
        model = load_promoted_public_beam_value_model(args.model_dir)
    ...
    print(
        f"cascadia-mlx public beam-value service {source_name} on {mx.default_device()}",
        file=sys.stderr,
    )
    serve_public_beam_value(model, sys.stdin.buffer, sys.stdout.buffer)
