"""Frozen training and bounded smoke runner for ADR 0156."""
...
from cascadia_mlx.r3_action_edit_mlx_cache import R3ActionEditBatch, R3ActionEditMlxCache
from cascadia_mlx.r4_bounded_parent_mlx_benchmark import (
...
from cascadia_mlx.r4_bounded_parent_mlx_metrics import (
    CANDIDATE_CHUNK,
    evaluate_r4_bounded_parent,
)
...
class R4BoundedParentTrainingProtocol:
    """All scientific constants held equal across the four parent arms."""
    ...
    d6_schedule: str = "blake3-of-seed-step-slot-over-rust-d6-ids-0-through-11"
    candidate_surface: str = "accepted-adr0150-exact-r2-afterstate-control"
    loss: str = (
    ...
    def validate(self) -> None:
        if self != R4BoundedParentTrainingProtocol():
            raise ValueError("R4 parent training protocol drifted from ADR 0156")
...
@dataclass(frozen=True)
class R4BoundedParentTrainingConfig:
    ...
    protocol: R4BoundedParentTrainingProtocol = field(
        default_factory=R4BoundedParentTrainingProtocol
    )
    ...
    def validate(self) -> None:
        ...
        if self.arm not in ARMS:
            raise ValueError("R4 parent training arm is unknown")
        if self.production:
            if self.authorization is None or self.preflight is None:
                raise ValueError(
                    "production R4 parent training requires authorization and preflight"
                )
        elif (
            ...
            raise ValueError(
                "bounded R4 parent smoke must be fresh, uncontrolled, and at most 10 steps"
            )


def run_r4_bounded_parent_training(
    config: R4BoundedParentTrainingConfig,
) -> dict[str, Any]:
    """Train one parent arm under the frozen schedule and emit complete evidence."""
    config.validate()
    ...
    if config.production:
        ...
        if actual_host != ARM_HOSTS[config.arm]:
            raise ValueError(
                f"R4 parent arm {config.arm} must run on {ARM_HOSTS[config.arm]}, not {actual_host}"
            )
    ...
    parent_cache = R4BoundedParentMlxCache(
        config.parent_cache,
        r3_cache=r3,
        verify_checksums=not config.production,
        verify_semantics=not config.production,
    ...
    controls = (
        validate_launch_controls(
            ...
            parent_cache_id=parent_cache.cache_id,
    ...
    preverified_proof_id = (
        str(controls["open_data_verification_id"]) if controls is not None else None
    )
    train = parent_cache.bind_dataset(
        config.train_dataset,
        s1_cache=s1_cache,
        verify_dataset_checksums=not config.production,
        preverified_open_data_proof_id=preverified_proof_id,
    )
    validation = parent_cache.bind_dataset(
    ...
    model_config = R4BoundedParentModelConfig(arm=config.arm)
    run_manifest = {
        "schema_version": 1,
    ...
    if config.resume:
        existing = _read_json(config.run_dir / "run.json", "R4 parent run manifest")
        if existing != run_manifest:
            raise ValueError("R4 parent resume manifest differs from the frozen run")
        model, optimizer, state, _ = load_latest_checkpoint_with_factory(
            config.run_dir,
            learning_rate=LEARNING_RATE,
            weight_decay=WEIGHT_DECAY,
            model_factory=lambda values: R4BoundedParentRanker(
                R4BoundedParentModelConfig.from_dict(values)
            ),
        )
    else:
        if (config.run_dir / "latest.json").exists():
            raise ValueError("R4 parent run already has checkpoints; pass --resume")
        mx.random.seed(TRAINING_SEED)
        model = R4BoundedParentRanker(model_config)
    ...
    if (
        ...
        or parameter_layout_blake3(model) != cross_arm["parameter_layout_blake3"]
        or (
            not config.resume
            and parameter_tensor_blake3(model) != cross_arm["initial_parameter_tensor_blake3"]
        )
    ):
        raise ValueError("R4 parent model graph differs from the matched initialization")

    loss_and_grad = nn.value_and_grad(model, r4_bounded_parent_loss)
    ...
    while state.global_step < config.target_steps:
        ...
        if not math.isfinite(loss_value):
            raise ValueError(f"R4 parent training produced nonfinite loss at step {step}")
        candidate_count = int(np.asarray(batch.base.candidate_mask, dtype=np.bool_).sum())
        ...
        if state.global_step % METRIC_STEPS == 0:
            probe_rows = _validation_probe_rows(validation.group_count)
            probe = evaluate_r4_bounded_parent(
                model,
                validation,
                arm=config.arm,
                rows=probe_rows,
            ...
            metric_event = {
                ...
                "mean_recent_loss": float(
                    np.mean([value["loss"] for value in loss_trace[-METRIC_STEPS:]])
                ),
                "candidates_per_second": measured_candidates / max(measured_seconds, 1e-12),
    ...
    model.eval()
    if config.production:
        validation_metrics = evaluate_r4_bounded_parent(
            model,
            validation,
            arm=config.arm,
        )
        benchmark_rows = np.arange(validation.group_count, dtype=np.int64)
        performance = run_isolated_serving_benchmark(
            train_dataset=config.train_dataset,
            validation_dataset=config.validation_dataset,
            r3_cache=config.r3_cache,
            parent_cache=config.parent_cache,
        ...
        smoke_rows = np.arange(min(validation.group_count, 5), dtype=np.int64)
        validation_metrics = evaluate_r4_bounded_parent(
            model,
            validation,
            arm=config.arm,
...
def scientific_batch_blake3(batch: R3ActionEditBatch) -> str:
    """Hash only arm-invariant group, target, action, and D6 identities."""
    digest = blake3.blake3()
    digest.update(b"r4-bounded-parent-mlx-scientific-batch-v1")
    ...
    source_indices = np.asarray(batch.source_candidate_indices, dtype=np.uint16)
...
def validate_launch_controls(
    ...
    *,
    ...
    if authorization_path is None or preflight_path is None:
        raise ValueError("R4 parent production launch controls are absent")
    authorization = _read_json(authorization_path, "R4 parent authorization")
    preflight = _read_json(preflight_path, "R4 parent preflight")
    authorization_identity = authorization.get("identity")
    preflight_identity = preflight.get("identity")
    verification_id = open_data_verification_id(open_data_verification)
    ...
    if (
        ...
        or _canonical_blake3(authorization_identity) != authorization.get("authorization_id")
        or authorization_identity.get("protocol_id") != PROTOCOL_ID
        or authorization_identity.get("r3_cache_id") != r3_cache_id
        or authorization_identity.get("parent_cache_id") != parent_cache_id
        ...
        or authorization_identity.get("source_blake3") != source.get("v2_source_blake3")
        or authorization_identity.get("protocol") != R4BoundedParentTrainingProtocol().to_dict()
        or authorization_identity.get("open_data_verification") != open_data_verification
        or authorization_identity.get("open_data_verification_id") != verification_id
    ):
        raise ValueError("R4 parent production authorization is stale or malformed")
    if (
        ...
        or preflight_identity.get("authorization_id") != authorization.get("authorization_id")
        or preflight_identity.get("r3_cache_id") != r3_cache_id
        or preflight_identity.get("parent_cache_id") != parent_cache_id
        ...
        or preflight_identity.get("open_data_verification_id") != verification_id
        or preflight_identity.get("mlx_gpu_verified") is not True
        or preflight_identity.get("open_data_only_verified") is not True
        or preflight_identity.get("initialization_parity_verified") is not True
        ...
        raise ValueError("R4 parent arm preflight is stale or incomplete")
    return {
...
def _cross_arm_initialization() -> dict[str, Any]:
    ...
    for arm in ARMS:
        mx.random.seed(TRAINING_SEED)
        candidate = R4BoundedParentRanker(R4BoundedParentModelConfig(arm=arm))
    ...
    if (
        ...
        raise ValueError("R4 parent arm initialization or parameter graph differs")
    return {
...
def _validation_probe_rows(group_count: int) -> np.ndarray:
    ...
    return np.unique(np.linspace(0, group_count - 1, count, dtype=np.int64))
...
def _require_production_runtime(runtime: dict[str, Any]) -> None:
    ...
    mx.eval(probe)
    if runtime.get("machine") != "arm64" or "gpu" not in str(mx.default_device()).lower():
        raise ValueError("R4 parent production training requires Apple Silicon MLX GPU")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train or smoke one ADR 0156 parent arm")
...
def main() -> None:
    ...
    report = run_r4_bounded_parent_training(
        R4BoundedParentTrainingConfig(
