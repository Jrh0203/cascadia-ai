"""Promote a sealed-test-qualified public beam set-ranker checkpoint."""
...
def promote_public_beam_set(run_dir: str | Path, output: str | Path) -> Path:
    ...
    try:
        ...
        raise PromotionError(f"public beam set run is not promotion-ready: {error}") from error
    if run.get("kind") != "public-beam-set-ranking":
        raise PromotionError("run is not a public beam set-ranking experiment")
    if not test_report.get("passed"):
        raise PromotionError("sealed public beam set test gates did not pass")
    ...
    if (
        ...
        raise PromotionError("best public beam set checkpoint failed integrity validation")
...
def load_promoted_public_beam_set_model(
    model_dir: str | Path,
) -> PublicBeamSetRanker:
    ...
    try:
        ...
        raise PromotionError(f"cannot read promoted public beam set model: {error}") from error
    if (
        manifest.get("schema_version") != PROMOTION_SCHEMA_VERSION
        or manifest.get("status") != "promoted"
        or manifest.get("kind") != "public-beam-set-ranking"
    ):
        raise PromotionError("unsupported or incomplete public beam set model")
    if (
        model_path.stat().st_size != model_info["bytes"]
        or _checksum(model_path) != model_info["blake3"]
    ):
        raise PromotionError("promoted public beam set model failed integrity validation")
    model = PublicBeamSetRanker(PublicBeamSetModelConfig.from_dict(manifest["model_config"]))
...
def main() -> None:
    ...
    print(promote_public_beam_set(args.run_dir, args.output))
