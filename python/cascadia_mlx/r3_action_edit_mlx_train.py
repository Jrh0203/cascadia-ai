"""Frozen training and bounded smoke runner for ADR 0150."""
...
class R3ActionEditTrainingProtocol:
    """All scientific constants held equal across the four arms."""
    ...
    def validate(self) -> None:
        if self != R3ActionEditTrainingProtocol():
            raise ValueError("R3 training protocol drifted from ADR 0150")
...
@dataclass(frozen=True)
class R3ActionEditTrainingConfig:
    ...
    protocol: R3ActionEditTrainingProtocol = field(default_factory=R3ActionEditTrainingProtocol)
    ...
    def validate(self) -> None:
        ...
        if self.arm not in ARMS:
            raise ValueError("R3 training arm is unknown")
        if self.production:
            if self.authorization is None or self.preflight is None:
                raise ValueError("production R3 training requires authorization and preflight")
        elif (
            ...
            raise ValueError("bounded R3 smoke must be fresh, uncontrolled, and at most 10 steps")


def run_r3_action_edit_training(
    config: R3ActionEditTrainingConfig,
) -> dict[str, Any]:
    ...
    if config.production:
        ...
        if actual_host != ARM_HOSTS[config.arm]:
            raise ValueError(
                f"R3 arm {config.arm} must run on {ARM_HOSTS[config.arm]}, not {actual_host}"
            )
    ...
    model_config = R3ActionEditModelConfig(arm=config.arm)
    run_manifest = {
        "schema_version": 1,
    ...
    if config.resume:
        existing = _read_json(config.run_dir / "run.json", "R3 run manifest")
        if existing != run_manifest:
            raise ValueError("R3 resume manifest differs from the frozen run")
        ...
        if (config.run_dir / "latest.json").exists():
            raise ValueError("R3 run already has checkpoints; pass --resume")
        mx.random.seed(TRAINING_SEED)
        model = R3ActionEditRanker(model_config)
    ...
    if (
        ...
        raise ValueError("R3 model graph differs from the matched initialization")

    loss_and_grad = nn.value_and_grad(model, r3_action_edit_loss)
    ...
    while state.global_step < config.target_steps:
        ...
        if not math.isfinite(loss_value):
            raise ValueError(f"R3 training produced nonfinite loss at step {step}")
    ...
    if config.production:
        validation_metrics = evaluate_r3_action_edit(
            model,
            validation,
            arm=config.arm,
        )
...
def scientific_batch_blake3(batch: R3ActionEditBatch) -> str:
    ...
    digest.update(b"r3-mlx-scientific-batch-v1")
...
def validate_launch_controls(
    ...
    *,
    ...
    if authorization_path is None or preflight_path is None:
        raise ValueError("R3 production launch controls are absent")
    authorization = _read_json(
        authorization_path,
        "R3 authorization",
    )
    preflight = _read_json(preflight_path, "R3 preflight")
    ...
    if (
        ...
        or authorization_identity.get("protocol") != R3ActionEditTrainingProtocol().to_dict()
        ...
        raise ValueError("R3 production authorization is stale or malformed")
    if (
        ...
        raise ValueError("R3 arm preflight is stale or incomplete")
    return {
...
def _cross_arm_initialization() -> dict[str, Any]:
    ...
    for arm in ARMS:
        mx.random.seed(TRAINING_SEED)
        candidate = R3ActionEditRanker(R3ActionEditModelConfig(arm=arm))
    ...
    if (
        ...
        raise ValueError("R3 arm initialization or parameter graph differs")
    return {
...
def cross_arm_initialization() -> dict[str, Any]:
    """Return the frozen, exact four-arm initialization contract."""
...
def runtime_identity() -> dict[str, Any]:
    """Return the runtime identity consumed by production preflight controls."""
...
def _require_production_runtime(runtime: dict[str, Any]) -> None:
    ...
    if runtime.get("machine") != "arm64" or "gpu" not in str(mx.default_device()).lower():
        raise ValueError("R3 production training requires Apple Silicon MLX GPU")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train or smoke one ADR 0150 R3 action-edit arm")
...
def main() -> None:
    ...
    report = run_r3_action_edit_training(
