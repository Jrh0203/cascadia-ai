...
HEADS="${HEADS:-8}"
MLP_DIM="${MLP_DIM:-512}"
SEEDS="${SEEDS:-20260650,20260651,20260652}"
...
EXPERIMENT_PREFIX="${EXPERIMENT_PREFIX:-crt-wide32-r16p80x2-semantic-vanilla-public-token}"
REPORT_PREFIX="${REPORT_PREFIX:-cascadiav3/reports/crt_wide32_r16p80x2_semantic_vanilla_public_token}"
CHECKPOINT_PREFIX="${CHECKPOINT_PREFIX:-cascadiav3/checkpoints/crt_wide32_r16p80x2_semantic_vanilla_public_token}"
PREFILTER_PREFIX="${PREFILTER_PREFIX:-cascadiav3/reports/crt_wide32_r16p80x2_semantic_vanilla_public_token}"
ENSEMBLE_REPORT="${ENSEMBLE_REPORT:-cascadiav3/reports/crt_wide32_r16p80x2_vanilla_public_token_seed_ensemble_3x_eval.json}"
ENSEMBLE_PER_ROOT_OUT="${ENSEMBLE_PER_ROOT_OUT:-cascadiav3/reports/crt_wide32_r16p80x2_vanilla_public_token_seed_ensemble_3x_eval_roots.jsonl}"
ENSEMBLE_SUMMARY_OUT="${ENSEMBLE_SUMMARY_OUT:-cascadiav3/reports/crt_wide32_r16p80x2_vanilla_public_token_seed_ensemble_3x_eval_summary.md}"
SWEEP_SUMMARY_OUT="${SWEEP_SUMMARY_OUT:-cascadiav3/reports/crt_wide32_r16p80x2_semantic_vanilla_public_token_seed_sweep_summary.md}"
REMOTE_LOG_DIR="$REMOTE_ROOT/cascadiav3/logs"
REMOTE_JOB="$REMOTE_LOG_DIR/r16p80x2_semantic_vanilla_public_token_sweep_job.sh"
REMOTE_LOG="$REMOTE_LOG_DIR/r16p80x2_semantic_vanilla_public_token_sweep_job.log"
REMOTE_PID="$REMOTE_LOG_DIR/r16p80x2_semantic_vanilla_public_token_sweep_job.pid"
...
for seed in \$(echo '$SEEDS' | tr ',' ' '); do
  ...
  echo "[sweep] training vanilla-public-token seed \${seed} -> \${report}"
  PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=cascadiav3/src python -m cascadiav3.torch_semantic_vanilla_public_token_merit \
    --train '$TRAIN_OUT' \
    --val '$VAL_OUT' \
    --steps '$STEPS' \
...
PREFILTER_PREFIX='$PREFILTER_PREFIX' \
...
prefilter_prefix = os.environ["PREFILTER_PREFIX"]
ensemble = json.loads(Path(os.environ["ENSEMBLE_REPORT"]).read_text(encoding="utf-8"))
lines = [
    "# CRT Wide-32 R16p80x2 Semantic Vanilla Public-Token Seed Sweep",
    "",
    "Status: completed.",
    "",
    "## Data",
    ...
    "## Results",
    "",
    "| Artifact | K=16 recall | K=16 oracle regret | K=24 recall | K=24 oracle regret | Mean regret | Top-1 |",
    "|---|---:|---:|---:|---:|---:|---:|",
]
for seed in seeds:
    report = json.loads(Path(f"{report_prefix}_seed{seed}_pilot.json").read_text(encoding="utf-8"))
    eval_report = json.loads(Path(f"{prefilter_prefix}_seed{seed}_prefilter_eval.json").read_text(encoding="utf-8"))
    metrics = eval_report["models"]["vanilla_public_token_transformer"]["metrics"]
...
lines.append(
    ...
    f"{ensemble_k24['recall']:.4f} | {ensemble_k24['mean_oracle_regret']:.4f} | "
    f"{ensemble_metrics['mean_regret']:.4f} | {ensemble_metrics['top1_agreement']:.4f} |"
)
lines.extend(
    [
        "",
...
ps -eo pid=,ppid=,args= | awk '
  ...
  /torch_semantic_vanilla_public_token_merit/ && /r16p80x2/ && !/awk/ {print \"matching trainer pid \" \$1 \" ppid \" \$2}
...
pids=\"\$(ps -eo pid=,args= | awk '
  ...
  /torch_semantic_vanilla_public_token_merit/ && /r16p80x2/ && !/awk/ {print \$1}
