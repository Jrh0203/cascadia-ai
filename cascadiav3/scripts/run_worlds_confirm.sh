...
# cycle4 scalar, n1024/top16, K1 on, samples 8, det16 vs det32, fresh seed block
# 2027071600..1599. Determinizations cycle inside the fixed simulation
...
FIRST_SEED="${FIRST_SEED:-2027071600}"
GAMES="${GAMES:-100}"
JOBS="${JOBS:-12}"
...
TAG="worlds_confirm_20260710_n1024"
...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  if [ "$(git rev-parse HEAD)" != "$SOURCE_REVISION" ]; then
    echo "[worlds-confirm] SOURCE_REVISION does not match HEAD" >&2
    exit 1
  fi
elif [ ! -s "$DEPLOYED_REVISION_FILE" ] \
  || [ "$(tr -d '[:space:]' < "$DEPLOYED_REVISION_FILE")" != "$SOURCE_REVISION" ]; then
  echo "[worlds-confirm] source snapshot lacks the deployed revision marker" >&2
  exit 1
fi
...
hold_gate() {
  while [ -e "$LOG_DIR/HOLD_worlds_confirm" ]; do
    echo "[worlds-confirm] $(date '+%F %T') holding on HOLD_worlds_confirm"
    sleep 60
  done
}
...
raise SystemExit(
    ...
    and search.get("n_simulations") == 1024
...
run_arm() {
  ...
  if [ "$games" = "$GAMES" ] && report_matches "$report" "$worlds"; then
    echo "[worlds-confirm] reuse $report"
    return
  fi
  hold_gate
...
echo "[worlds-confirm] source=$SOURCE_REVISION seeds=${FIRST_SEED}x${GAMES} arms=det16,det32"
run_arm 32 1 1
run_arm 16
run_arm 32
