...
fn main() {
    ...
    println!("  greedy_move:            {:>7.1} μs/call ({} calls)", per, n);
    ...
    let per = t.elapsed().as_micros() as f64 / n as f64;
    println!("  candidate_moves_pub:    {:>7.1} μs/call ({} calls)", per, n);
    ...
    let per = t.elapsed().as_micros() as f64 / n as f64;
    println!("  pick_best_move_nnue:    {:>7.1} μs/call ({} calls)", per, n);
    ...
    let per = t.elapsed().as_micros() as f64 / n as f64;
    println!("  NNUE forward (w/ bag):  {:>7.1} μs/call ({} calls)", per, n);
    ...
    for _ in 0..n {
        ...
        std::hint::black_box(cascadia_core::scoring::ScoreBreakdown::compute(&mut b, &cards));
    }
    let per = t.elapsed().as_micros() as f64 / n as f64;
    println!("  ScoreBreakdown::compute:{:>7.1} μs/call ({} calls)", per, n);
    ...
    let per = t.elapsed().as_micros() as f64 / n as f64;
    println!("  GameState clone:        {:>7.1} μs/call ({} calls)", per, n);
    ...
    let per = t.elapsed().as_micros() as f64 / n as f64;
    println!("  Board clone:            {:>7.1} μs/call ({} calls)", per, n);
}
