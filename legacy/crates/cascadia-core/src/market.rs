...
impl WildlifeBag {
    ...
    /// Per-wildlife-type counts of remaining tokens. Used by AZv2 to feed the
    /// bag-belief entity token (so the network sees how many of each animal
    /// are still drawable rather than just the total bag size).
    pub fn counts_per_type(&self) -> [u8; 5] {
        let mut out = [0u8; 5];
        for &w in &self.tokens {
            out[w as usize] = out[w as usize].saturating_add(1);
        }
        out
    }
