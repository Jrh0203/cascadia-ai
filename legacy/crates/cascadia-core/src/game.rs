    ...
    [
        ...
        [
            (
                p0,
                StarterHex {
                    tile: TileData::single(Mountain, m(&[Bear])),
                    rotation: 0,
                },
            ),
            (
                p1,
                StarterHex {
                    tile: TileData::dual(Forest, Wetland, m(&[Hawk, Fox, Elk])),
                    rotation: rot_p1,
                },
            ),
            (
                p2,
                StarterHex {
                    tile: TileData::dual(Prairie, River, m(&[Salmon, Bear])),
                    rotation: rot_p2,
                },
            ),
        ],
        ...
        [
            (
                p0,
                StarterHex {
                    tile: TileData::single(Wetland, m(&[Hawk])),
                    rotation: 0,
                },
            ),
            (
                p1,
                StarterHex {
                    tile: TileData::dual(Forest, River, m(&[Salmon, Hawk, Elk])),
                    rotation: rot_p1,
                },
            ),
            (
                p2,
                StarterHex {
                    tile: TileData::dual(Mountain, Prairie, m(&[Bear, Fox])),
                    rotation: rot_p2,
                },
            ),
        ],
        ...
        [
            (
                p0,
                StarterHex {
                    tile: TileData::single(Prairie, m(&[Fox])),
                    rotation: 0,
                },
            ),
            (
                p1,
                StarterHex {
                    tile: TileData::dual(Wetland, River, m(&[Salmon, Fox, Hawk])),
                    rotation: rot_p1,
                },
            ),
            (
                p2,
                StarterHex {
                    tile: TileData::dual(Mountain, Forest, m(&[Bear, Elk])),
                    rotation: rot_p2,
                },
            ),
        ],
        ...
        [
            (
                p0,
                StarterHex {
                    tile: TileData::single(Forest, m(&[Elk])),
                    rotation: 0,
                },
            ),
            (
                p1,
                StarterHex {
                    tile: TileData::dual(River, Mountain, m(&[Hawk, Bear, Elk])),
                    rotation: rot_p1,
                },
            ),
            (
                p2,
                StarterHex {
                    tile: TileData::dual(Prairie, Wetland, m(&[Fox, Salmon])),
                    rotation: rot_p2,
                },
            ),
        ],
        ...
        [
            (
                p0,
                StarterHex {
                    tile: TileData::single(River, m(&[Salmon])),
                    rotation: 0,
                },
            ),
            (
                p1,
                StarterHex {
                    tile: TileData::dual(Forest, Prairie, m(&[Salmon, Bear, Elk])),
                    rotation: rot_p1,
                },
            ),
            (
                p2,
                StarterHex {
                    tile: TileData::dual(Mountain, Wetland, m(&[Fox, Hawk])),
                    rotation: rot_p2,
                },
            ),
        ],
...
impl GameState {
    ...
    pub fn execute_move(&mut self, player_move: PlayerMove) -> bool {
        ...
        // Place tile
        if board
            .place_tile(player_move.tile_coord, pair.tile, player_move.rotation)
            .is_none()
        {
            return false;
        }
    ...
    pub fn can_replace_overflow(&self) -> Option<crate::types::Wildlife> {
        if self.overflow_used_this_turn {
            return None;
        }
    ...
    pub fn final_scores(&mut self) -> Vec<ScoreBreakdown> {
        if self.num_players == 1 {
            ...
            (0..self.num_players)
                .map(|p| {
                    ScoreBreakdown::compute_with_bonuses(&mut self.boards, &self.scoring_cards, p)
                })
                .collect()
        }
    }
