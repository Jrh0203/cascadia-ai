...
fn bench_full_scoring(c: &mut Criterion) {
    ...
    c.bench_function("full_board_score", |b| {
        b.iter(|| {
            black_box(ScoreBreakdown::compute(
                black_box(&mut board),
                black_box(&cards),
            ))
        })
