...
pub fn best_move_with_potential(
    ...
    let use_potential = std::env::var("CASCADIA_GREEDY_POTENTIAL")
        .ok()
    ...
    for (tile_market_idx, tile, drafted_wildlife, wl_market_idx) in combos {
        ...
        for tp in &tile_positions {
            ...
            let skip_potential = if use_potential {
                (board_potential(board, cards) as f32 * _potential_scale) as i32
            } else {
                0
            };
            ...
            let valid_positions: arrayvec::ArrayVec<u16, 64> = board
                ...
                .filter(|&idx| {
                    board
                        .grid
                        .get(idx as usize)
                        .can_place_wildlife(drafted_wildlife)
                })
                .collect();
