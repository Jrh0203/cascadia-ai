        ...
        for s in 0..=(r - b).min(9) {
            ...
            for h in 0..=(r - b - s).min(8) {
                ...
                for e in 0..=r_eh {
                    let f = r_eh - e;
                    let other_species =
                        (b > 0) as usize + (s > 0) as usize + (h > 0) as usize + (e > 0) as usize;
        ...
        for ds in 0..=(r - db).min(9) {
            ...
            for dh in 0..=(r - db - ds).min(8) {
                ...
                for de in 0..=r_ef {
                    ...
                    let other_species = (new_b > 0) as usize
                        + (new_e > 0) as usize
                        + (new_s > 0) as usize
                        + (new_h > 0) as usize;
...
pub fn greedy_upper_bound_from_state(
    ...
    let r = moves_remaining;
    let mut best: u16 = 0;
    let cur_other = (current_bear > 0) as usize
        + (current_elk > 0) as usize
        + (current_salmon > 0) as usize
        + (current_hawk > 0) as usize;
...
mod tests {
    ...
    fn from_state_existing_bears() {
        ...
        assert!(
            with_state >= 27,
            "should include existing 27, got {}",
            with_state
        );
    }
    ...
    fn argmax_empty_board_20_moves() {
        ...
        assert!(
            b + s > 0,
            "argmax should pick some bear or salmon, got ({},{},{},{},{})",
            b,
            e,
            s,
            h,
