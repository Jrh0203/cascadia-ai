            ...
            for rot in 0..max_rot {
                ...
                for &ti in placed_snapshot.iter() {
                    ...
                    if strategic_value >= 3 || delta >= 3 {
                        ...
                        let total =
                            hab + base_wildlife + delta + board.nature_tokens as u16 + nat_bonus;
                ...
                if slot_value >= 4 {
                    ...
                    let best_wl =
                        find_best_wildlife_placement(&mut board_clone, wildlife, variant, without);
                    let nat_bonus = best_wl
                        .map(|(ti, _)| {
                            if board_clone.grid.get(ti).is_keystone() {
                                1u16
                            } else {
                                0
                            }
    ...
    match wildlife {
        Wildlife::Bear => {
            ...
            if bear_neighbors == 1 {
                ...
                let neighbor_pos = adj
                    .neighbors_of(pos)
                    .find(|&n| board.grid.get(n).placed_wildlife() == Some(Wildlife::Bear))
                    .unwrap();
                let neighbor_other_bears = adj
                    .neighbors_of(neighbor_pos)
                    .filter(|&n| {
                        n != pos && board.grid.get(n).placed_wildlife() == Some(Wildlife::Bear)
                    })
                    .count();
                if neighbor_other_bears == 0 {
                    8
                } else {
                    0
                } // valid pair = very valuable
        ...
        Wildlife::Hawk => {
            ...
            if !has_hawk_neighbor {
                ...
                let isolated = board.wildlife_positions[Wildlife::Hawk as usize]
                    .iter()
                    .filter(|&&p| {
        ...
        match w {
            Wildlife::Bear => {
                ...
                let adjacent_isolated_bear = adj.neighbors_of(idx).any(|n| {
                    if board.grid.get(n).placed_wildlife() != Some(Wildlife::Bear) {
                        return false;
                    }
                    ...
                    adj.neighbors_of(n)
                        .filter(|&nn| {
                            nn != idx
                                && board.grid.get(nn).placed_wildlife() == Some(Wildlife::Bear)
                        })
                        .count()
                        == 0
                });
                if adjacent_isolated_bear {
                    value += 5;
                }
            }
            Wildlife::Elk => {
            ...
            Wildlife::Salmon => {
                ...
                let has_salmon_neighbor = adj
                    .neighbors_of(idx)
                    .any(|n| board.grid.get(n).placed_wildlife() == Some(Wildlife::Salmon));
                if has_salmon_neighbor {
                    value += 3;
                }
            }
            Wildlife::Hawk => {
...
fn find_best_wildlife_placement(
    ...
    for &ti in placed.iter() {
        if !board.grid.get(ti as usize).can_place_wildlife(wildlife) {
            continue;
        }
    ...
    match w {
        Wildlife::Bear => {
            let adjacent_isolated_bear = adj.neighbors_of(idx).any(|n| {
                ...
                adj.neighbors_of(n)
                    .filter(|&nn| {
                        nn != idx && board.grid.get(nn).placed_wildlife() == Some(Wildlife::Bear)
                    })
                    .count()
                    == 0
            });
...
fn bear_c_setup(board: &Board, pos: usize, adj: &cascadia_core::hex::AdjacencyTable) -> u16 {
    ...
    if bn == 0 {
        ...
        let total_neighbor_bears: usize = nbrs
            .iter()
            .map(|&n| {
                adj.neighbors_of(n as usize)
                    .filter(|&nn| {
                        nn != pos && board.grid.get(nn).placed_wildlife() == Some(Wildlife::Bear)
                    })
                    .count()
            })
            .sum();
...
fn elk_b_setup(board: &Board, pos: usize, adj: &cascadia_core::hex::AdjacencyTable) -> u16 {
    let elk_n: arrayvec::ArrayVec<u16, 6> = adj
        .neighbors_of(pos)
    ...
    if n == 0 {
        ...
        let could_triangle: bool = adj.neighbors_of(pos).any(|p3| {
            p3 != nbr
                && board.grid.get(p3).is_present()
                && !board.grid.get(p3).has_wildlife()
        ...
        for i in 0..elk_n.len() {
            for j in (i + 1)..elk_n.len() {
                ...
                if adj.neighbors_of(a).any(|x| x == b) {
                    ...
                    let could_rhombus: bool = adj
                        .neighbors_of(pos)
                        .chain(adj.neighbors_of(a))
                        .chain(adj.neighbors_of(b))
                        .any(|p4| {
                            p4 != pos
                                && p4 != a
                                && p4 != b
                                && board.grid.get(p4).is_present()
                                && !board.grid.get(p4).has_wildlife()
...
fn salmon_d_setup(board: &Board, pos: usize, adj: &cascadia_core::hex::AdjacencyTable) -> u16 {
    ...
    if salmon_n > 2 {
        return 0;
    } // would create branching (invalid run)
    ...
    if salmon_n == 0 {
        ...
        let run_len = count_salmon_run(
            board,
            adj.neighbors_of(pos)
                .find(|&n| board.grid.get(n).placed_wildlife() == Some(Wildlife::Salmon))
                .unwrap(),
            adj,
        );
        ...
        // salmon_n == 2 — middle of a run, doesn't extend
        adj_types
    }
}
...
fn salmon_bc_setup(
    ...
    if salmon_n != 1 {
        return 0;
    } // only extending a chain endpoint scores
    let nbr = adj
        .neighbors_of(pos)
        .find(|&n| board.grid.get(n).placed_wildlife() == Some(Wildlife::Salmon))
        .unwrap();
...
fn fox_b_setup(board: &Board, pos: usize, adj: &cascadia_core::hex::AdjacencyTable) -> u16 {
    ...
    let immediate_score: u16 = match pair_types {
        0 => 0,
        1 => 3,
        2 => 5,
        _ => 7,
...
fn fox_d_setup(board: &Board, pos: usize, adj: &cascadia_core::hex::AdjacencyTable) -> u16 {
    ...
    let other_fox = adj
        .neighbors_of(pos)
        .find(|&n| board.grid.get(n).placed_wildlife() == Some(Wildlife::Fox))
        .unwrap();
    let mut counts = [0u8; 5];
    let mut seen = [false; 441];
    for src in [pos, other_fox] {
        for n in adj.neighbors_of(src) {
            if n == pos || n == other_fox {
                continue;
            }
            if seen[n] {
                continue;
