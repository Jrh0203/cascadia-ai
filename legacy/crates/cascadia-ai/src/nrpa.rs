...
impl MoveFeature {
    fn from_move(game: &GameState, mv: &ScoredMove) -> Self {
        ...
        let drafted = market.pairs[mv.market_index].as_ref().map(|p| p.wildlife);
        let placed_animal = mv.wildlife_q.is_some().then(|| drafted).flatten();
...
fn playout(
    ...
    let max_ai_plies: usize = std::env::var("NRPA_DEPTH")
        .ok()
    ...
    while !g.is_game_over() {
        ...
        let legal_feats: Vec<MoveFeature> = cands
            .iter()
        ...
        if !execute_scored_move(&mut g, &chosen) {
            break;
        }
        ai_plies += 1;
...
pub fn best_move_nrpa(game: &GameState, net: &NNUENetwork, rng: &mut StdRng) -> Option<ScoredMove> {
    let level: usize = std::env::var("NRPA_LEVEL")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(NRPA_LEVEL);
    let n: usize = std::env::var("NRPA_N")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(NRPA_N);
    ...
    result.seq.into_iter().next().map(|s| ScoredMove {
        score: result.score.round() as u16,
        ..s.chosen
    })
}
