...
pub fn train(net: &mut NTupleNetwork, num_games: usize, alpha: f32, seed: u64) -> TrainStats {
...
fn play_and_collect_updates(
    ...
    while !game.is_game_over() {
        ...
        // Record afterstate
        let actual_score = ScoreBreakdown::compute(&mut game.boards[0], &game.scoring_cards).total;
    ...
    if n > 0 {
        ...
        for t in (0..n - 1).rev() {
            let reward =
                (snapshots[t + 1].actual_score as f32) - (snapshots[t].actual_score as f32);
...
pub fn pick_best_move_ntuple(game: &GameState, net: &NTupleNetwork) -> Option<ScoredMove> {
    let mp: Vec<_> = game
        .market
        .available()
