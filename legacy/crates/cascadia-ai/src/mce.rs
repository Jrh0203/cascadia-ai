...
use crate::score_target::{score_total, ScoreTarget};
...
fn active_score_target() -> ScoreTarget {
    ScoreTarget::from_env(ScoreTarget::Base)
}

pub(crate) fn mce_score_total(game: &mut GameState, player: usize) -> u16 {
    score_total(game, player, active_score_target())
}
...
pub fn analyze_mulligan_fast(game: &GameState, net: &NNUENetwork) -> MulliganAnalysis {
        ...
        for animal_idx in 0..5 {
            ...
            if let Some(mv) = best {
                ...
                if eval_board.place_tile(coord, tile, mv.rotation).is_some() {
                    ...
                    let actual =
                        cascadia_core::scoring::ScoreBreakdown::compute(&mut eval_board, &cards)
                            .total as f64;
    ...
    let active_slots: Vec<usize> = (0..4).filter(|&i| game.market.pairs[i].is_some()).collect();
    ...
    let should_mulligan_pinecone = nature_tokens >= 2
        && mulligan_pinecone_ev - 2.0 > current_best
...
pub fn analyze_mulligan(
    ...
    for slot in 0..4 {
        if game.market.pairs[slot].is_none() {
            continue;
        }
        for animal_idx in 0..5 {
            let animal = Wildlife::from_u8(animal_idx as u8).unwrap();
...
fn enumerate_draws(
    ...
    for animal in 0..5u8 {
        if remaining_counts[animal as usize] == 0 {
            continue;
        }
...
pub fn best_move_expectimax_1ply(game: &GameState, net: &NNUENetwork) -> Option<ScoredMove> {
    let player = game.current_player;
    let cards = game.scoring_cards;
    ...
    for mv in &candidates {
        ...
        for animal_idx in 0..5 {
            ...
            for &ti in board.placed_tiles.iter() {
                let idx = ti as usize;
                if !board.grid.get(idx).can_place_wildlife(animal) {
                    continue;
                }
                let mut b = board.clone();
                if let Some(wa) = b.place_wildlife(idx, animal) {
                    let with_wl =
                        cascadia_core::scoring::wildlife::score_wildlife(&b, animal, variant);
                    let delta = (with_wl as f64) - (base_wl as f64);
                    // Also add nature token bonus for keystone
                    let keystone_bonus = if b.grid.get(idx).is_keystone() {
                        1.0
                    } else {
                        0.0
                    };
    ...
    best.map(|(mv, score)| ScoredMove {
        score: score.round() as u16,
        ..mv
    })
}
...
pub fn best_move_expectimax_2ply(game: &GameState, net: &NNUENetwork) -> Option<ScoredMove> {
    let player = game.current_player;
    let cards = game.scoring_cards;
...
pub fn score_all_candidates_expectimax(
    ...
    for mv in &candidates {
        ...
        let bag_info = crate::nnue::BagInfo::from_game_for_player(&g, player);
        ...
        let score = if g.is_game_over() {
            mce_score_total(&mut g, player) as f64
        } else {
    ...
    for animal_idx in 0..5 {
        ...
        for &ti in board.placed_tiles.iter() {
            ...
            if let Some(wa) = b.place_wildlife(idx, animal) {
                ...
                let delta = (with_wl as f64) - (base_wl as f64);
                let keystone = if board.grid.get(idx).is_keystone() {
                    1.0
                } else {
                    0.0
                };
    ...
    for animal_idx in 0..5 {
        ...
        if depth >= 2 {
            if let Some(ref post_board) = best_board {
                ...
                let post_actual =
                    ScoreBreakdown::compute(&mut post_board.clone(), &cards).total as f64;
...
pub fn best_move_wildlife_deep(
    ...
    for mv in &candidates {
        ...
        let wildlife_bonus =
            wildlife_deep_eval(&g.boards[player], &cards, &bag_info, wildlife_depth);
...
fn wildlife_deep_eval(
    ...
    for animal_idx in 0..5 {
        ...
        for &ti in board.placed_tiles.iter() {
            ...
            if let Some(_wa) = b.place_wildlife(idx, animal) {
                let with =
                    cascadia_core::scoring::wildlife::score_wildlife(&b, animal, variant) as f64;
                let delta = with - base;
                let keystone = if board.grid.get(idx).is_keystone() {
                    1.0
                } else {
            ...
            for t1 in 0..5u8 {
                ...
                for t2 in 0..5u8 {
                    ...
                    for t3 in 0..5u8 {
                        ...
                        let best_type = [t0, t1, t2, t3]
                            .iter()
                            .max_by(|&&a, &&b| {
                                type_delta[a as usize]
                        ...
                        let future = if depth > 1 && best_placement_idx[best_type] != usize::MAX {
                            ...
                            if let Some(_wa) =
                                next_board.place_wildlife(best_placement_idx[best_type], animal)
                            {
                                // Reduce bag counts for recursion
                                let mut next_bag = bag_info.clone();
                                next_bag.remaining[best_type] =
                                    next_bag.remaining[best_type].saturating_sub(1);
...
pub fn best_move_hybrid(
    ...
    for sc in &strategic {
        if !candidates.iter().any(|c| {
            c.tile_q == sc.tile_q
                && c.tile_r == sc.tile_r
                && c.rotation == sc.rotation
                && c.wildlife_q == sc.wildlife_q
        }) {
            candidates.push(*sc);
        }
    }
    ...
    for round in 0..num_rounds {
            ...
            .map(|chunk| {
                ...
                std::thread::spawn(move || {
                    ...
                    for &(ci, seed) in &work {
                        ...
                        let depth_limit: usize = std::env::var("MCE_DEPTH")
                            .ok()
                        ...
                        while !g.is_game_over() {
                            if g.current_player != player {
                                ...
                                match crate::search::greedy_move(&g) {
                                    Some(opp) => {
                                        if !crate::search::execute_scored_move(&mut g, &opp) {
                                            break;
                                        }
                                    }
                                    None => break,
                            ...
                            ai_turns += 1;
                            if ai_turns > depth_limit {
                                break;
                            }
                            match crate::nnue_train::pick_best_move_nnue(&g, &net) {
                                Some(ai_mv) => {
                                    if !crate::search::execute_scored_move(&mut g, &ai_mv) {
                                        break;
                                    }
                                }
                                None => break,
                        ...
                        let score = if g.is_game_over() {
                            mce_score_total(&mut g, player)
                        } else {
                            let actual = mce_score_total(&mut g.clone(), player);
        ...
        if round < num_rounds - 1 {
            let mut alive_scores: Vec<(usize, f64)> = alive
                .iter()
                .filter_map(|&ci| {
                    if counts[ci] == 0 {
                        return None;
    ...
    let mut scored: Vec<(ScoredMove, f64)> = candidates
        .iter()
        .enumerate()
        .filter_map(|(ci, mv)| {
            if counts[ci] == 0 {
    ...
    scored.into_iter().next().map(|(mv, avg)| ScoredMove {
...
pub fn collect_mce_samples(
    ...
    for (mv, avg) in scored {
        ...
        let player = game.current_player;
        let current = mce_score_total(&mut g.clone(), player) as f32;
...
pub fn expanded_candidates(game: &GameState) -> Vec<ScoredMove> {
    ...
    // CASCADIA_PATTERN_TARGET=1: augment candidate pool with multi-turn
    // pattern-target moves. Bounded ~5 extras (1 per wildlife type). These
    // carry artificially-high `eval` so the downstream prefilter doesn't
    // drop them; MCE rollouts then verify whether the pattern is real.
    let pattern_target_on = std::env::var("CASCADIA_PATTERN_TARGET")
        .ok()
        .map(|s| !s.is_empty() && s != "0")
        .unwrap_or(false);
    if pattern_target_on {
        candidates.extend(crate::pattern_target::pattern_target_candidates(game));
    }
    // CASCADIA_LAYOUT_ORACLE=1: enumerate C(N=20, 5)=15,504 hypothetical
    // 5-wildlife layouts and emit candidates targeting the highest-scoring
    // joint configurations. See `layout_oracle.rs`.
    let layout_oracle_on = std::env::var("CASCADIA_LAYOUT_ORACLE")
        .ok()
        .map(|s| !s.is_empty() && s != "0")
        .unwrap_or(false);
    if layout_oracle_on {
        candidates.extend(crate::layout_oracle::layout_oracle_candidates(game));
    }
    ...
    let max_extra: usize = std::env::var("MCE_MAX_EXTRA_CANDS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(20);
    ...
    let mut frontier_ranked: Vec<(u16, usize)> = frontier
        .iter()
        .map(|&idx| {
            let n_placed = adj
                .neighbors_of(idx as usize)
    ...
    let top_frontier: Vec<u16> = frontier_ranked
        .into_iter()
        .take(frontier_limit)
        .map(|(i, _)| i)
        .collect();
...
fn run_greedy_rollout(mut gs: GameState, player: usize, seed: u64, candidate: &ScoredMove) -> u64 {
    ...
    if !execute_scored_move(&mut gs, candidate) {
        return 0;
    }
    ...
    mce_score_total(&mut gs, player) as u64
}
...
fn run_nnue_rollout(
    ...
    let opp_temp: f32 = std::env::var("MCE_OPP_TEMPERATURE")
        .ok()
        .and_then(|s| s.parse().ok())
        .filter(|&t| t > 0.0)
        .unwrap_or(0.0);
    ...
    let rollout_opp_nnue: bool = std::env::var("MCE_ROLLOUT_OPP")
        .ok()
    ...
    let rollout_policy_expectimax: bool = std::env::var("MCE_ROLLOUT_POLICY")
        .ok()
        .map(|s| {
            s.eq_ignore_ascii_case("expectimax1")
    ...
    let cv_at_turn: usize = std::env::var("MCE_CV_AT_TURN")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(2);
    ...
    let cv_collect_enabled: bool = std::env::var("MCE_CONTROL_VARIATES")
        .ok()
        .map(|s| !s.is_empty() && s != "0")
        .unwrap_or(false);

    // CASCADIA_MCE_TRUNC=N: cap rollouts at N player turns; estimate the rest
    // via NNUE. Default 0 = no cap (play to game end). Lower values trade
    // quality for speed. Strict tie-out only when N=0 (default).
    let truncate_at: usize = std::env::var("CASCADIA_MCE_TRUNC")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(0);

    // CASCADIA_MCE_DECOUPLE_OPP=1: skip opponent simulation in rollouts.
    // Each opponent turn: pick a random market slot, remove it, refill from
    // bag, advance current_player. Opponent boards stay frozen — saves the
    // ~1ms candidate_moves_pub call per opponent ply. Quality risk: opponents'
    // habitat groups don't grow → player's habitat-bonus rank inflated. Verify
    // mean score on bench before relying on this for promotion.
    let decouple_opp: bool = std::env::var("CASCADIA_MCE_DECOUPLE_OPP")
        .ok()
        .map(|s| !s.is_empty() && s != "0")
        .unwrap_or(false);
    ...
    while !gs.is_game_over() {
        // Truncation check: if we've reached the truncation budget on the
        // player's turn, estimate remaining points via NNUE and return.
        if truncate_at > 0 && gs.current_player == player && player_turns_done >= truncate_at {
            let bag = crate::nnue::BagInfo::from_game_for_player(&gs, player);
            let cur = mce_score_total(&mut gs.clone(), player) as f64;
            let remaining = net.evaluate_with_bag(&gs.boards[player], &bag) as f64;
            let estimated = ((cur + remaining).max(0.0)) as u64;
            return (estimated, cv_eval.unwrap_or(estimated));
        }
        if gs.current_player != player {
            if gs.can_replace_overflow().is_some() {
                gs.replace_overflow();
            }
            // Decoupled-opponent path: skip board simulation, just advance the
            // market and current_player. Saves ~1ms candidate_moves_pub call
            // per opponent ply. See CASCADIA_MCE_DECOUPLE_OPP doc above.
            if decouple_opp {
                // Pick a random non-empty market slot to "consume". This
                // approximates a uniformly-random opponent picking from
                // available pairs. Avoids the cost of candidate_moves_pub.
                let mut available: arrayvec::ArrayVec<usize, 4> = arrayvec::ArrayVec::new();
                for (i, p) in gs.market.pairs.iter().enumerate() {
                    if p.is_some() {
                        available.push(i);
                    }
                }
                ...
                let pick_idx = available[rr.gen_range(0..available.len())];
                gs.market.pairs[pick_idx] = None;
                gs.market.refill(&mut gs.tile_bag, &mut gs.wildlife_bag);
            ...
            let opp_mv = if rollout_opp_nnue {
                crate::nnue_train::pick_best_move_nnue(&gs, net).or_else(|| greedy_move(&gs))
            ...
            if cv_collect_enabled
                && cv_eval.is_none()
                && cv_at_turn > 0
                && player_turns_done >= cv_at_turn
            {
                let bag = crate::nnue::BagInfo::from_game_for_player(&gs, player);
                let cur = mce_score_total(&mut gs.clone(), player) as f64;
                let remaining = net.evaluate_with_bag(&gs.boards[player], &bag) as f64;
    ...
    let final_score = mce_score_total(&mut gs, player) as u64;
    ...
    match alloc {
        ...
        GreedyMceAlloc::SeqHalvingEarlyTerm => {
            ...
            for round in 0..num_rounds {
                ...
                if alive.len() >= 2 {
                    let mut scored: Vec<(usize, f64, f64)> = alive
                        .iter()
                        .filter_map(|&ci| {
                            if counts[ci] == 0 {
                                return None;
        ...
        GreedyMceAlloc::SeqHalvingCI => {
            ...
            let z: f64 = std::env::var("MCE_HALVING_CI_Z")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(1.5);
            let hard_floor: bool = std::env::var("MCE_HALVING_CI_FLOOR")
                .ok()
                .map(|s| !s.is_empty() && s != "0")
                .unwrap_or(false);
            let num_rounds = (n_cands as f64).log2().ceil().max(1.0) as usize;
            ...
            for round in 0..num_rounds {
                ...
                if round < num_rounds - 1 && alive.len() >= 2 {
                    let stats: Vec<(usize, f64, f64)> = alive
                        .iter()
                        .filter_map(|&ci| {
                            if counts[ci] == 0 {
                                return None;
                    ...
                    let mut kept: Vec<usize> = stats
                        .iter()
                        .filter_map(|&(ci, m, se)| {
                            if m + z * se >= leader_lcb {
                                Some(ci)
                            } else {
                                None
                            }
                    ...
                    if hard_floor {
                        let mut by_mean: Vec<(usize, f64)> =
                            stats.iter().map(|&(ci, m, _)| (ci, m)).collect();
                        by_mean.sort_by(|a, b| {
                            b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal)
                        });
                        let max_keep = ((alive.len() + 1) / 2).max(2);
                        let top: std::collections::HashSet<usize> = by_mean
                            .into_iter()
                            .take(max_keep)
                            .map(|(ci, _)| ci)
                            .collect();
                    ...
                    if stats.is_empty() {
                        ...
                        let leader =
                            stats
                                .iter()
                                .cloned()
                                .fold(stats[0], |acc, x| if x.1 > acc.1 { x } else { acc });
        ...
        GreedyMceAlloc::SeqHalvingPW => {
            ...
            let widen = {
                let mut stats: Vec<(usize, f64, f64)> = alive
                    .iter()
                    .filter_map(|&ci| {
                        if counts[ci] == 0 {
                            return None;
...
pub fn nnue_prefilter_candidates(
    ...
    let diverse: bool = std::env::var("MCE_DIVERSE_PREFILTER")
        .ok()
        .map(|s| !s.is_empty() && s != "0")
        .unwrap_or(false);
    if !diverse {
    ...
    let mut add = |mv: ScoredMove,
                   sel: &mut Vec<ScoredMove>,
                   keys: &mut std::collections::HashSet<_>|
     -> bool {
        let k = key_of(&mv);
        if keys.contains(&k) {
            return false;
    ...
    {
        let picks = deterministic_market_representatives(&all_scored);
        for mv in picks {
    ...
    for i in 0..top_n {
        ...
        if gs.boards[player]
            .place_tile(tile_coord, market_pair.tile, base_mv.rotation)
            .is_none()
        {
            continue;
        }
        ...
        let without =
            cascadia_core::scoring::wildlife::score_wildlife(&gs.boards[player], wildlife, variant);
        let mut wl_options: Vec<(i8, i8, f32)> = Vec::new();
        let placed: arrayvec::ArrayVec<u16, 64> =
            gs.boards[player].placed_tiles.iter().copied().collect();
        ...
        for (wq, wr, score) in &wl_options {
            ...
            if base_mv.wildlife_q == Some(*wq) && base_mv.wildlife_r == Some(*wr) {
                continue;
            }
            ...
            if add(variant_mv, &mut selected, &mut used_keys) {
                added += 1;
            }
        }
    }
...
fn prefilter_ensemble_nets() -> &'static Vec<NNUENetwork> {
    ...
    eprintln!(
        "[prefilter-ensemble] active with {} extra net(s) for key '{}'",
        nets.len(),
        env_val
    );
...
pub fn nnue_prefilter_with_priors(
    ...
    let player = game.current_player;
    let cards = game.scoring_cards;
    let use_strategy_bias: bool = std::env::var("MCE_STRATEGY_BIAS")
        .ok()
        .map(|s| !s.is_empty() && s != "0")
        .unwrap_or(false);
    ...
    scored.sort_by(|a, b| {
        b.0.partial_cmp(&a.0)
            .unwrap_or(std::cmp::Ordering::Equal)
            .then_with(|| scored_move_identity(&a.1).cmp(&scored_move_identity(&b.1)))
    });
...
pub(crate) fn scored_move_identity(
    movement: &ScoredMove,
...
pub(crate) fn deterministic_market_representatives(candidates: &[ScoredMove]) -> Vec<ScoredMove> {
    let mut best_per = std::collections::BTreeMap::new();
    for movement in candidates {
        best_per.entry(movement.market_index).or_insert(*movement);
    }
    let mut representatives = best_per.into_values().collect::<Vec<_>>();
    representatives.sort_by(|left, right| {
        right
            .eval
            .cmp(&left.eval)
            .then_with(|| scored_move_identity(left).cmp(&scored_move_identity(right)))
    });
    representatives
}
...
pub(crate) fn strategy_commit_bonus(game: &GameState, mv: &ScoredMove) -> f32 {
...
pub fn best_move_nnue_rollout_mce(
    ...
    let cv_alpha: f32 = std::env::var("MCE_CV_ALPHA")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(1.0);
    // MCE_LMR: if set (any non-empty, non-"0"), use tiered budget by NNUE prior rank.
    let use_lmr: bool = std::env::var("MCE_LMR")
        .ok()
        .map(|s| !s.is_empty() && s != "0")
        .unwrap_or(false);
    ...
    let priors: Vec<f32> = if cv_alpha < 1.0 || use_lmr {
        candidates
            ...
            .map(|mv| {
                ...
                let bag = crate::nnue::BagInfo::from_game_for_player(&g, player);
                let board = &g.boards[player];
                let cur = mce_score_total(&mut g.clone(), player) as f32;
    ...
    let ranks: Vec<usize> = if use_lmr && !priors.is_empty() {
        let mut idx_prior: Vec<(usize, f32)> =
    ...
    let use_cv: bool = std::env::var("MCE_CONTROL_VARIATES")
        .ok()
        .map(|s| !s.is_empty() && s != "0")
        .unwrap_or(false);
    ...
    let use_gumbel_halving: bool = std::env::var("MCE_GUMBEL_HALVING")
        .ok()
        .map(|s| !s.is_empty() && s != "0")
        .unwrap_or(false);
    let gumbel_sigma: f64 = std::env::var("MCE_GUMBEL_HALVING_SIGMA")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(3.0);
    ...
    match alloc {
        GreedyMceAlloc::Uniform | GreedyMceAlloc::UniformCRN => {
            ...
            for ci in 0..n_cands {
                for k in 0..per {
                    let seed = if alloc == GreedyMceAlloc::UniformCRN {
                        base_seeds[k]
                    } else {
                        rng.gen()
                    };
        ...
        GreedyMceAlloc::SeqHalving | GreedyMceAlloc::SeqHalvingCRN => {
            ...
            for round in 0..num_rounds {
                ...
                let raw_pers: Vec<f64> = alive
                    .iter()
                    .map(|&ci| (base_per as f64) * lmr_mult(ci))
                    .collect();
                ...
                let pers: Vec<usize> = raw_pers
                    .iter()
                    .map(|p| ((p * scale).round() as usize).max(1))
                    .collect();
                ...
                for (idx, &ci) in alive.iter().enumerate() {
                    for k in 0..pers[idx] {
                        let seed = if alloc == GreedyMceAlloc::SeqHalvingCRN {
                            base_seeds[k]
                        } else {
                            rng.gen()
                        };
        ...
        GreedyMceAlloc::MctsPW => {
            ...
            let k_init: usize = std::env::var("MCE_MCTSPW_K_INIT")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(4);
            let pw_k: f64 = std::env::var("MCE_MCTSPW_K")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(2.0);
            let pw_alpha: f64 = std::env::var("MCE_MCTSPW_ALPHA")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(0.5);
            let ucb_c: f64 = std::env::var("MCE_MCTSPW_UCB_C")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(0.25);
            let batch: usize = std::env::var("MCE_MCTSPW_BATCH")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(4);
        ...
        GreedyMceAlloc::Puct => {
            ...
            let c_puct: f64 = std::env::var("MCE_PUCT_C")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(2.0);
            let tau: f64 = std::env::var("MCE_PUCT_TAU")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(8.0);
            ...
            let batch: usize = std::env::var("MCE_PUCT_BATCH")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(32);
            ...
            let exps: Vec<f64> = priors_f64
                .iter()
                .map(|&p| ((p - max_p) / tau.max(0.5)).exp())
                .collect();
    ...
    let cv_beta_cap: f64 = std::env::var("MCE_CV_BETA_CAP")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(2.0);
    ...
    Some(ScoredMove {
        score: rollout_mean_best.round() as u16,
        ..mv
    })
}
...
pub fn mce_candidates_with_features(
    ...
    mce_candidates_with_features_top_k(game, net, num_rollouts, 15, rng)
}

/// Score candidates with the legacy CZero/MCE candidate pipeline using an explicit
/// post-NNUE-rerank cap. This preserves `mce_candidates_with_features` behavior
/// while letting CZero data collection request the intended wider K.
pub fn mce_candidates_with_features_top_k(
    game: &GameState,
    net: &NNUENetwork,
    num_rollouts: usize,
    top_k: usize,
    ...
    let player = game.current_player;
    let scored = run_mce_candidates_impl(game, net, None, top_k, num_rollouts, rng);
...
/// Score the same expanded, NNUE-prefiltered candidate pool used by `mce_wide_v1`,
/// returning every evaluated candidate with post-move afterstate features. This is
/// the preferred teacher path for CascadiaZero collection.
pub fn nnue_rollout_mce_candidates_with_features(
    game: &GameState,
    net: &NNUENetwork,
    num_rollouts: usize,
    prefilter_k: usize,
    ...
    let player = game.current_player;
    let mut candidates = expanded_candidates(game);
    if prefilter_k > 0 && candidates.len() > prefilter_k {
        let expand: usize = std::env::var("MCE_MUTATE_EXPAND")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
        candidates = nnue_prefilter_candidates(game, net, candidates, prefilter_k + expand);
    }

    let scored = score_nnue_rollout_mce_seq_halving(game, net, num_rollouts, candidates, rng);
    scored
        .into_iter()
        .map(|estimate| {
            let mv = estimate.movement;
            let mut g = game.clone();
            crate::search::execute_scored_move(&mut g, &mv);
            let bag_info = crate::nnue::BagInfo::from_game_for_player(&g, player);
            let features =
                crate::nnue::extract_features_with_bag(&g.boards[player], Some(&bag_info));
            (mv, features, estimate.rollout_mean as f32)
        })
        .collect()
}

#[derive(Debug, Clone, Copy)]
pub struct MceMoveEstimate {
...
pub fn score_nnue_rollout_mce_seq_halving(
    game: &GameState,
    net: &NNUENetwork,
    num_rollouts: usize,
    candidates: Vec<ScoredMove>,
    ...
    let priors: Vec<f32> = if use_lmr {
        candidates
            ...
            .map(|mv| {
                ...
                let bag = crate::nnue::BagInfo::from_game_for_player(&g, player);
                let cur = mce_score_total(&mut g.clone(), player) as f32;
                let remaining = net.evaluate_with_bag(&g.boards[player], &bag);
                cur + remaining
            })
            .collect()
    } else {
    ...
    let cands_arc = Arc::new(candidates.clone());
    ...
    for round in 0..num_rounds {
        ...
        let mut work_items: Vec<(usize, u64)> = Vec::with_capacity(pers.iter().sum());
        for (idx, &ci) in alive.iter().enumerate() {
            for _ in 0..pers[idx] {
                work_items.push((ci, rng.gen()));
            }
        ...
        let handles: Vec<_> = work_items
            ...
            .map(|chunk| {
                ...
                let c = Arc::clone(&cands_arc);
                let n = Arc::clone(&net_arc);
                thread::spawn(move || {
                    let mut results = Vec::with_capacity(work.len());
                    for &(ci, seed) in &work {
                        let (score, _) = run_nnue_rollout((*g).clone(), player, seed, &c[ci], &n);
                        results.push((ci, score));
                    }
                    results
                })
    ...
    let mut scored: Vec<MceMoveEstimate> = candidates
        ...
        .filter_map(|(ci, mv)| {
            ...
            let samples = counts[ci];
            let count = f64::from(samples);
            let rollout_mean = totals[ci] as f64 / count;
            let variance = (sumsq[ci] as f64 / count - rollout_mean * rollout_mean).max(0.0);
...
/// CascadiaZero policy-guided MCE: with-bonus scoring is selected by the caller
/// through CASCADIA_SCORE_TARGET/MCE_SCORE_TARGET, and final choice remains the
/// rollout mean after policy pruning.
pub fn best_move_czero_mce(
    game: &GameState,
    net: &NNUENetwork,
    policy_net: &crate::nnue::PolicyNetwork,
    num_rollouts: usize,
    ...
    let top_k = std::env::var("CZERO_TOP_K")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(32);
    let player = game.current_player;
    ...
    if candidates.len() > top_k {
        let mut indexed: Vec<(usize, f32)> = candidates
            ...
            .map(|(i, mv)| {
                ...
                let features =
                    crate::nnue::extract_features_with_bag(&g.boards[player], Some(&bag_info));
                (i, policy_net.forward(&features))
            })
            .collect();
        indexed.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
        indexed.truncate(top_k);
        candidates = indexed.into_iter().map(|(i, _)| candidates[i]).collect();
    }
...
fn run_mce_candidates_with_policy(
    ...
    let filtered: Vec<ScoredMove> = scored_candidates
        .iter()
                ...
                std::thread::spawn(move || {
                    ...
                    for &(ci, seed) in &work {
                        ...
                        let score = if g.is_game_over() {
                            ...
                            let actual =
                                ScoreBreakdown::compute(&mut g.boards[player], &g.scoring_cards)
                                    .total;
                            let nval = net.evaluate(&g.boards[player]);
                            let tier = tier_bonus(&g.boards[player]);
                            (actual as f32 + nval.max(0.0) + tier) as u16
...
pub fn rank_all_candidates_mce(
                ...
                for &(ci, seed) in &work {
                    ...
                    while !g.is_game_over() {
                        if g.current_player != player {
                            ...
                            match greedy_move(&g) {
                                Some(opp) => {
                                    if !execute_scored_move(&mut g, &opp) {
                                        break;
                                    }
                                }
                                None => break,
                        ...
                        match greedy_move(&g) {
                            Some(ai_mv) => {
                                if !execute_scored_move(&mut g, &ai_mv) {
                                    break;
                                }
                            }
                            None => break,
    ...
    let mut scored: Vec<(ScoredMove, MceStat)> = candidates
        .iter()
        .enumerate()
        .filter_map(|(ci, mv)| {
            if counts[ci] == 0 {
...
fn run_mce_candidates_impl(
        ...
        if let Some(tile) = tile {
            ...
            if !crate::search::execute_scored_move(&mut after_game, mv) {
                ...
                if eval_board.place_tile(coord, tile, mv.rotation).is_some() {
                    ...
                    let fallback_bag = crate::nnue::BagInfo::from_game_for_player(game, player);
                    let remaining = net.evaluate_with_bag(&eval_board, &fallback_bag);
            ...
            if use_rank_expectimax {
                ...
                if eval_board.place_tile(coord, tile, mv.rotation).is_some() {
                    ...
                    let actual = mce_score_total(&mut after_game.clone(), player) as f32;
                    let bag_info = crate::nnue::BagInfo::from_game_for_player(&after_game, player);
    ...
    let max_candidates: usize = std::env::var("MCE_CANDIDATES")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(15);
    ...
    if let Some(pnet) = policy_net {
        if candidates.len() > top_k {
            let candidate_features: Vec<Vec<u16>> = candidates
                .iter()
                .map(|mv| {
                    let mut g = game.clone();
                    if crate::search::execute_scored_move(&mut g, mv) {
                    ...
                    for &(ci, seed) in &work {
                        ...
                        while !g.is_game_over() {
                            if g.current_player != player {
                                ...
                                match greedy_move(&g) {
                                    Some(opp_mv) => {
                                        if !execute_scored_move(&mut g, &opp_mv) {
                                            break;
                                        }
                                    }
                                    None => break,
    ...
    let mut scored: Vec<(usize, ScoredMove, f64)> = candidates
        .iter()
        .enumerate()
        .filter_map(|(ci, mv)| {
            if counts[ci] == 0 {
    ...
    if !scored.is_empty() {
        ...
        record_diagnostic(MceDiagnostics {
            ...
            from_candidate_moves: sources
                .iter()
                .filter(|&&s| s == CandidateSource::CandidateMoves)
                .count(),
            from_greedy: sources
                .iter()
                .filter(|&&s| s == CandidateSource::Greedy)
                .count(),
            from_strategic: sources
                .iter()
                .filter(|&&s| s == CandidateSource::Strategic)
                .count(),
            winner_source,
...
fn evaluate_leaf_with_next_2_markets(g: &GameState, player: usize, net: &NNUENetwork) -> f32 {
    ...
    for animal_idx in 0..5 {
        ...
        let variant = cards.variant_for(animal);
        let base_wl =
            cascadia_core::scoring::wildlife::score_wildlife(&working, animal, variant) as f32;

        let mut best_delta = 0.0f32;
        for &ti in &positions {
            let idx = ti as usize;
            if !working.grid.get(idx).can_place_wildlife(animal) {
                continue;
            }
            if let Some(wa) = working.place_wildlife(idx, animal) {
                let with =
                    cascadia_core::scoring::wildlife::score_wildlife(&working, animal, variant)
                        as f32;
                let delta = with - base_wl;
                let keystone = if working.grid.get(idx).is_keystone() {
                    1.0
                } else {
                    0.0
                };
    ...
    for s1_animal in 0..5 {
        if step1_best_idx[s1_animal] == usize::MAX {
            continue;
        }
        ...
        if let Some(wa) = working.place_wildlife(step1_best_idx[s1_animal], animal1) {
            for s2_animal in 0..5 {
                ...
                let base_wl2 =
                    cascadia_core::scoring::wildlife::score_wildlife(&working, animal2, variant2)
                        as f32;

                let mut best_delta2 = 0.0f32;
                for &ti in &positions {
                    let idx = ti as usize;
                    if !working.grid.get(idx).can_place_wildlife(animal2) {
                        continue;
                    }
                    if let Some(wa2) = working.place_wildlife(idx, animal2) {
                        let with2 = cascadia_core::scoring::wildlife::score_wildlife(
                            &working, animal2, variant2,
                        ) as f32;
                        let delta2 = with2 - base_wl2;
                        let keystone2 = if working.grid.get(idx).is_keystone() {
                            1.0
                        } else {
                            0.0
                        };
        ...
        for t1 in 0..5u8 {
            ...
            for t2 in 0..5u8 {
                ...
                for t3 in 0..5u8 {
                    ...
                    let step2_ev_inner =
                        expected_best_wildlife_4draws(&step2_value[best_t1], &bag2, total2);
    ...
    let two_step_ev = if total_prob > 0.0 {
        joint_ev / total_prob
    } else {
        0.0
    };
...
fn evaluate_leaf_market_aware_2step(g: &GameState, player: usize, net: &NNUENetwork) -> f32 {
    let cards = g.scoring_cards;
    let actual = ScoreBreakdown::compute(&mut g.boards[player].clone(), &cards).total as f32;
    let bag_info = crate::nnue::BagInfo::from_game(g);
    let nnue_remaining = net.evaluate_with_bag(&g.boards[player], &bag_info).max(0.0);
    ...
    let market_wildlife: Vec<Wildlife> = g
        .market
        .pairs
        .iter()
        .flatten()
    ...
    let step1_chosen = *market_wildlife
        .iter()
        .max_by(|a, b| {
            step1_value[**a as usize]
                .partial_cmp(&step1_value[**b as usize])
                .unwrap()
        })
        .unwrap();
    ...
    let leftover_max = market_wildlife
        .iter()
...
fn evaluate_leaf_market_aware(g: &GameState, player: usize, net: &NNUENetwork) -> f32 {
    let cards = g.scoring_cards;
    let actual = ScoreBreakdown::compute(&mut g.boards[player].clone(), &cards).total as f32;
    let bag_info = crate::nnue::BagInfo::from_game(g);
    let nnue_remaining = net.evaluate_with_bag(&g.boards[player], &bag_info).max(0.0);
...
fn evaluate_leaf_with_next_market(g: &GameState, player: usize, net: &NNUENetwork) -> f32 {
    let cards = g.scoring_cards;
    let actual = ScoreBreakdown::compute(&mut g.boards[player].clone(), &cards).total as f32;
    let bag_info = crate::nnue::BagInfo::from_game(g);
    let nnue_remaining = net.evaluate_with_bag(&g.boards[player], &bag_info).max(0.0);
...
fn tier_bonus(board: &Board) -> f32 {
    ...
    let isolated_hawks = hawk_positions
        .iter()
        .filter(|&&p| {
            !adj.neighbors_of(p as usize)
...
fn compute_wildlife_demand(board: &Board) -> [f32; 5] {
    ...
    for &pos in bear_positions.iter() {
        ...
        if bear_neighbors == 0 {
            ...
            if has_slot {
                demand[Wildlife::Bear as usize] += 3.0;
            }
        }
    }
    ...
    let hawk_positions = &board.wildlife_positions[Wildlife::Hawk as usize];
    let isolated_count = hawk_positions
        .iter()
        .filter(|&&pos| {
...
mod tests {
    ...
    #[test]
    fn diverse_market_representatives_have_total_deterministic_order() {
        let movement = |market_index, tile_q, eval| ScoredMove {
            market_index,
            tile_q,
            tile_r: 0,
            rotation: 0,
        ...
        let representatives = deterministic_market_representatives(&candidates);
        assert_eq!(
            representatives
                .iter()
                .map(|candidate| (candidate.market_index, candidate.tile_q))
                .collect::<Vec<_>>(),
            vec![(1, 2), (0, 3), (2, 4)]
        );
    }
    ...
    fn test_enumerate_draws_with_known_outcome() {
        ...
        assert!(
            (ev - 90.0).abs() < 1e-10,
            "With only bears, best paired should be 90.0, got {}",
            ev
        );
    }
    ...
    fn test_enumerate_draws_pinecone_can_beat_paired() {
        ...
        assert!(
            pine_ev > paired_ev,
            "Pinecone EV ({}) should beat paired EV ({})",
            pine_ev,
            paired_ev
        );
    }
    ...
    fn test_enumerate_impossible_draws_skipped() {
        ...
        assert!(
            (ev - 50.0).abs() < 1e-10,
            "Bear is impossible to draw, EV should be 50.0, got {}",
            ev
        );
    }
}
