...
use crate::nnue::{extract_features_with_bag, BagInfo, PositionPolicyData, NUM_FEATURES};
use crate::score_target::ScoreTarget;
...
const POLICY_ACT_CLIP: f32 = 10_000.0;
const POLICY_LOGIT_CLIP: f32 = 50.0;

fn finite_relu(v: f32) -> f32 {
    if v.is_finite() {
        v.max(0.0).min(POLICY_ACT_CLIP)
    } else {
        0.0
    }
}

fn finite_logit(v: f32) -> f32 {
    if v.is_finite() {
        v.clamp(-POLICY_LOGIT_CLIP, POLICY_LOGIT_CLIP)
    } else {
        0.0
    }
}

fn default_feature_set() -> &'static str {
...
impl PolicyNetwork {
    ...
    pub fn from_nnue(nnue: &crate::nnue::NNUENetwork) -> Self {
        ...
        for i in 0..POLICY_H1.min(crate::nnue::HIDDEN1) {
            ...
            if src_base + cols <= nnue.w2.len() && dst_base + cols <= net.w2.len() {
                net.w2[dst_base..dst_base + cols]
                    .copy_from_slice(&nnue.w2[src_base..src_base + cols]);
            }
        }
    ...
    pub fn forward(&self, features: &[u16]) -> f32 {
        ...
        for &fi in features {
            ...
            if base + POLICY_H1 > self.w1.len() {
                continue;
            }
            let col = &self.w1[base..base + POLICY_H1];
            for j in 0..POLICY_H1 {
                h1[j] += col[j];
            }
        }
    ...
    pub fn train_ranking(
        ...
        let target: Vec<f32> = {
            let exps: Vec<f32> = mce_scores
                .iter()
                .map(|s| finite_logit((s - max_mce) / temperature).exp())
                .collect();
        ...
        let pred: Vec<f32> = {
            let exps: Vec<f32> = logits
                .iter()
                .map(|l| finite_logit(l - max_logit).exp())
                .collect();
            let sum: f32 = exps.iter().sum();
            exps.iter().map(|e| e / sum.max(1e-8)).collect()
        };
    ...
    pub fn save(&self, path: &std::path::Path) -> std::io::Result<()> {
        self.save_with_magic(path, b"PLCY")
    }

    pub fn save_czp1(&self, path: &std::path::Path) -> std::io::Result<()> {
        self.save_czp1_with_metadata(path, default_feature_set(), "unknown")
    }

    pub fn save_czp1_with_metadata(
        &self,
        path: &std::path::Path,
        feature_set: &str,
        git_rev: &str,
        ...
        f.write_all(&(NUM_FEATURES as u32).to_le_bytes())?;
        f.write_all(&(POLICY_H1 as u32).to_le_bytes())?;
        f.write_all(&(POLICY_H2 as u32).to_le_bytes())?;
        f.write_all(&[ScoreTarget::WithHabitatBonus.as_u8()])?;
        f.write_all(&[0u8; 5])?; // A,A,A,A,A
        write_str(&mut f, feature_set)?;
        write_str(&mut f, git_rev)?;
        self.write_weights(&mut f)
    }

    fn save_with_magic(&self, path: &std::path::Path, magic: &[u8; 4]) -> std::io::Result<()> {
        use std::io::Write;
        let mut f = std::fs::File::create(path)?;
        f.write_all(magic)?;
        let version: u32 = 1;
    ...
    fn write_weights<W: std::io::Write>(&self, f: &mut W) -> std::io::Result<()> {
        for &v in &self.w1 {
    ...
    pub fn load(path: &std::path::Path) -> std::io::Result<Self> {
        ...
        if &magic == b"CZP1" {
            ...
            if nf != NUM_FEATURES {
                return Err(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!("feature count mismatch: file={} build={}", nf, NUM_FEATURES),
                ));
            }
            if version == 2 {
                let mut target = [0u8; 1];
                f.read_exact(&mut target)?;
                if ScoreTarget::from_u8(target[0]) != Some(ScoreTarget::WithHabitatBonus) {
                    return Err(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                ...
                let _feature_set = read_str(&mut f)?;
                let _git_rev = read_str(&mut f)?;
            }
        }
