...
pub const TOTAL_WEIGHTS: usize = NUM_DIRS * (WL_PAIR + WL_LINE3 + WL_LINE4 + TR_PAIR + TR_LINE3);
...
impl NTupleNetwork {
    ...
    pub fn scale(&mut self, factor: f32) {
        for dir in 0..NUM_DIRS {
            for w in self.wl_pair[dir].iter_mut() {
                *w *= factor;
            }
            for w in self.wl_line3[dir].iter_mut() {
                *w *= factor;
            }
            for w in self.wl_line4[dir].iter_mut() {
                *w *= factor;
            }
            for w in self.tr_pair[dir].iter_mut() {
                *w *= factor;
            }
            for w in self.tr_line3[dir].iter_mut() {
                *w *= factor;
            }
        }
    }
    ...
    pub fn merge_from(&mut self, other: &NTupleNetwork) {
        for dir in 0..NUM_DIRS {
            for i in 0..WL_PAIR {
                self.wl_pair[dir][i] += other.wl_pair[dir][i];
            }
            for i in 0..WL_LINE3 {
                self.wl_line3[dir][i] += other.wl_line3[dir][i];
            }
            for i in 0..WL_LINE4 {
                self.wl_line4[dir][i] += other.wl_line4[dir][i];
            }
            for i in 0..TR_PAIR {
                self.tr_pair[dir][i] += other.tr_pair[dir][i];
            }
            for i in 0..TR_LINE3 {
                self.tr_line3[dir][i] += other.tr_line3[dir][i];
            }
        }
    }
                ...
                if let Some(c1_idx) = c1.to_index() {
                    ...
                    if let Some(c2_idx) = c2.to_index() {
                        ...
                        if let Some(c3_idx) = c3.to_index() {
                            ...
                            value +=
                                self.wl_line4[dir][quad_index(w0, w1, w2, w3, WILDLIFE_STATES)];
                        }
                    }
                }
            }
            ...
            for (dir, &(dq, dr)) in HexCoord::LINE_DIRECTIONS.iter().enumerate() {
                ...
                if let Some(c1_idx) = c1.to_index() {
                    ...
                    if let Some(c2_idx) = c2.to_index() {
                        ...
                        tuples.push((
                            3 + dir as u8,
                            triple_index(w0, w1, w2, WILDLIFE_STATES) as u16,
                        ));
                        tuples.push((
                            12 + dir as u8,
                            triple_index(t0, t1, t2, TERRAIN_STATES) as u16,
                        ));
                        ...
                        if let Some(c3_idx) = c3.to_index() {
                            ...
                            tuples.push((
                                6 + dir as u8,
                                quad_index(w0, w1, w2, w3, WILDLIFE_STATES) as u16,
                            ));
                        }
                    }
                }
