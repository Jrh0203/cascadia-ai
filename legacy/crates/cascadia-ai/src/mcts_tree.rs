...
impl UctEdge {
    fn q(&self) -> f64 {
        if self.visits == 0 {
            ...
            self.total_value as f64 / self.visits as f64
        }
    }
...
fn nnue_rollout(mut g: GameState, ai_player: usize, net: &NNUENetwork) -> u64 {
    while !g.is_game_over() {
        ...
        let mv = if g.current_player == ai_player {
            crate::nnue_train::pick_best_move_nnue(&g, net).or_else(|| greedy_move(&g))
        } else {
            greedy_move(&g)
        };
        match mv {
...
fn simulate(
    ...
    if game.is_game_over() {
        let score =
            ScoreBreakdown::compute(&mut game.boards[ai_player].clone(), &game.scoring_cards).total
                as u64;
        node.visits += 1;
...
impl PersistentUctTree {
    ...
    pub fn new_with_net(ai_player: usize, seed: u64, net: Option<Arc<NNUENetwork>>) -> Self {
        PersistentUctTree {
    ...
    pub fn advance(&mut self, played_move: &ScoredMove) {
        ...
        let idx = self
            ...
            .edges
            .iter()
            .position(|e| action_key(&e.action) == target_key);
        if let Some(idx) = idx {
    ...
    pub fn root_edges_len(&self) -> usize {
        self.root.edges.len()
    }
}
    ...
    pub fn pick_move(&mut self, game: &GameState, total_sims: usize) -> Option<ScoredMove> {
        ...
        for t in &self.trees {
            for e in &t.root.edges {
                let entry = agg.entry(action_key(&e.action)).or_insert((e.action, 0, 0));
        ...
        best.map(|(mv, _, q)| ScoredMove {
            score: q.round() as u16,
            ..mv
        })
    }
...
mod tests {
    ...
    fn persistent_tree_grows_across_calls() {
        ...
        assert!(
            visits_after_t1 >= 16,
            "root visits should reflect sim count, got {}",
            visits_after_t1
        );
    ...
    fn forest_runs_parallel_and_aggregates() {
        ...
        assert!(
            forest.aggregate_root_visits() >= 32,
            "all trees together should have >=32 visits"
        );
        let _ = mv;
    }
}
