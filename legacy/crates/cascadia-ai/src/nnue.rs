...
pub const PHASE_FEATURES: usize =
    TURN_FEATURES + TOKEN_FEATURES + WL_COUNT_FEATURES + HAB_SIZE_FEATURES; // 110
...
pub const PATTERN_FEATURES: usize = BEAR_PAIR_FEATURES
    + ELK_LINE_FEATURES
    + SALMON_RUN_FEATURES
    + HAWK_ISO_FEATURES
    + FOX_DIV_FEATURES
    + EMPTY_SLOTS_FEATURES; // 89
...
pub const PATTERN_V2_FEATURES: usize = PAT_EXT_ELK_LINES
    + PAT_EXT_SALMON_RUNS
    + PAT_BEAR_EXT_SINGLES
    + PAT_BEAR_WASTE
    + PAT_HAWK_AT_RISK
    + PAT_FORCED_ALLOC
    + PAT_MAX_DIV_FOX
    + PAT_KEYSTONE_OPEN; // 4+4+4+4+4+20+4+4 = 48
...
pub const OPP_DET_PER_OPP: usize =
    5 * OPP_DET_WL_BINS + 5 * OPP_DET_HAB_BINS + OPP_DET_TOK_BINS + OPP_DET_PATTERN_BITS; // 55 + 55 + 9 + 4 = 123
pub const NUM_OPP_SLOTS: usize = 3;
pub const OPP_DETAILED_FEATURES: usize = NUM_OPP_SLOTS * OPP_DET_PER_OPP; // 369

// Lever-4 "OPP × MARKET" features: capture draft-competition signal that the
// per-opponent block alone misses. Block 1 (bonus-race tightness) ships first
// because it's the cheapest extraction and most leveraged for habitat
// bonus tiebreakers. Append-only — older NNUE files load with zero-padding.
pub const OPM_BONUS_RACE_BINS: usize = 5; // ≤-5, -4..-1, 0, +1..+4, ≥+5
pub const OPM_BONUS_RACE_FEATURES: usize = 5 * OPM_BONUS_RACE_BINS; // 25 (5 terrains)
pub const OPPMARKET_FEATURES: usize = OPM_BONUS_RACE_FEATURES; // 25 for v0
...
pub const NUM_FEATURES_V1: usize = CELL_FEATURES
    + PHASE_FEATURES
    + PAIR_FEATURES
    + PATTERN_FEATURES
    + BAG_FEATURES
...
pub const NUM_FEATURES_V2: usize = NUM_FEATURES_V1
    ...
    + PATTERN_V2_FEATURES
    + BAG_EXT_FEATURES
    + OPP_HAB_EXT_FEATURES
    + MARKET_FEATURES
    + TBAG_TERRAIN_FEATURES
    + TBAG_WL_FEATURES; // 10561
...
pub const NUM_FEATURES_V3: usize = NUM_FEATURES_V2
    + CELL_ADJ_FEATURES
    + TBAG_TERRAIN_EXT_FEATURES
    + TBAG_WL_EXT_FEATURES
...
pub const NUM_FEATURES_MID: usize =
    NUM_FEATURES_V2 + TBAG_TERRAIN_EXT_FEATURES + TBAG_WL_EXT_FEATURES + OVERFLOW_FEATURES; // 10862
...
pub const V5_OPP_PAT_PER_OPP: usize =
    V5_OPP_BEAR_SING_BINS + V5_OPP_ELK_LINE_BINS + V5_OPP_SALMON_RUN_BINS + V5_OPP_ISOL_HAWK_BINS; // 36
...
pub const V5_FEAT_FEATURES: usize = V5_FRONTIER_FLAG_FEATURES
    ...
    + V5_OPP_PAT_FEATURES
    + V5_BONUS_DIST_FEATURES
    + V5_HAB_STRUCT_FEATURES
    + V5_TBAG_JOINT_FEATURES; // 5884
...
// ── CascadiaZero AAAAA with-bonus feature block ───────────────────────
//
// Append-only on top of v3+v4-opp+v5-feat. This preserves every previously
// tried full-grid feature while adding the information needed by a with-bonus
// expert-iteration player.
//
//   CZERO_BONUS_RACE:     5 terrains × (my-size, rank, leader gap, second gap)
//   CZERO_OPP_MARKET:     market slot × opponent × token/terrain pressure bins
//   CZERO_NATURE:         token count, market wildlife counts, spend affordance
//   CZERO_HAB_BUCKET:     441 cells × 5 terrains × 3 cluster roles
pub const CZERO_MY_SIZE_BINS: usize = 21; // 0..20+
pub const CZERO_RANK_BINS: usize = 4; // 0,1,2,3+ opponents ahead
pub const CZERO_GAP_BINS: usize = 21; // -10..10
pub const CZERO_BONUS_RACE_PER_TERR: usize =
    CZERO_MY_SIZE_BINS + CZERO_RANK_BINS + CZERO_GAP_BINS + CZERO_GAP_BINS;
pub const CZERO_BONUS_RACE_FEATURES: usize = 5 * CZERO_BONUS_RACE_PER_TERR; // 335

pub const CZERO_MARKET_COUNT_BINS: usize = 11; // 0..10+
pub const CZERO_OPP_MARKET_TOKEN_FEATURES: usize = 4 * NUM_OPP_SLOTS * 5 * CZERO_MARKET_COUNT_BINS; // 660
pub const CZERO_OPP_MARKET_TERR_FEATURES: usize = 4 * NUM_OPP_SLOTS * 5 * CZERO_MARKET_COUNT_BINS; // 660
pub const CZERO_OPP_MARKET_FEATURES: usize =
    CZERO_OPP_MARKET_TOKEN_FEATURES + CZERO_OPP_MARKET_TERR_FEATURES; // 1320

pub const CZERO_NATURE_TOKEN_BINS: usize = 9; // 0..8+
pub const CZERO_MARKET_WL_COUNT_BINS: usize = 5; // 0..4
pub const CZERO_NATURE_FEATURES: usize =
    CZERO_NATURE_TOKEN_BINS + 5 * CZERO_MARKET_WL_COUNT_BINS + 2; // 36

pub const CZERO_HAB_BUCKET_ROLES: usize = 3; // largest, second-largest, other
pub const CZERO_HAB_BUCKET_FEATURES: usize = GRID_SIZE * 5 * CZERO_HAB_BUCKET_ROLES; // 6615

pub const CZERO_FEAT_FEATURES: usize = CZERO_BONUS_RACE_FEATURES
    + CZERO_OPP_MARKET_FEATURES
    + CZERO_NATURE_FEATURES
    + CZERO_HAB_BUCKET_FEATURES; // 8306
...
pub const ALT_PLAYER_FEATURES: usize = ALT_BEAR_FEATURES
    + ALT_ELK_FEATURES
    + ALT_SALMON_FEATURES
    + ALT_HAWK_FEATURES
    + ALT_FOX_FEATURES; // 117
...
pub const ALT_OPP_PER_OPP: usize = ALT_OPP_BEAR_BINS
    + ALT_OPP_ELK_SHAPE_BINS
    + ALT_OPP_SALMON_BINS
    + ALT_OPP_HAWK_BINS
    + ALT_OPP_FOX_BINS; // 25
...
pub const CARDS_ALT_V2_FEATURES: usize = ALT2_HAWK_LOS_FEATURES
...
/// Base index where the CascadiaZero feature block starts. In the intended
/// build (`v4-opp,v5-feat,czero-feat`) this is exactly NUM_FEATURES_V3_V4_V5.
pub const CZERO_FEAT_BASE: usize = if cfg!(feature = "v5-feat") {
    V5_FEAT_BASE + V5_FEAT_FEATURES
...
pub const NUM_FEATURES_V6_PEAK: usize = V6_CELL_FEATURES                  // 1397
    + PHASE_FEATURES                  // 110
    + PAIR_FEATURES                   // 147
    + PATTERN_FEATURES                // 89
    + BAG_FEATURES                    // 55
...
} else if cfg!(feature = "cards-alt-v2")
...
} else if cfg!(feature = "cards-alt") && cfg!(feature = "mid-features") && cfg!(feature = "v4-opp")
{
    ...
    NUM_FEATURES_LEGACY
} else if cfg!(feature = "oppmarket-feat")
    && cfg!(feature = "mid-features")
    && cfg!(feature = "v4-opp")
{
    NUM_FEATURES_MID_V4_OPM
} else if cfg!(feature = "mid-features") && cfg!(feature = "v4-opp") {
    NUM_FEATURES_MID_V4
} else if cfg!(feature = "mid-features") {
    NUM_FEATURES_MID
...
pub const HIDDEN1: usize = if cfg!(feature = "small-net") {
    256
} else if cfg!(feature = "large-net") {
    1024
} else {
    512
};
pub const HIDDEN2: usize = if cfg!(feature = "small-net") {
    32
} else if cfg!(feature = "large-net") {
    128
} else {
...
#[inline]
pub fn cell_features_before_wildlife(
    board: &Board,
    idx: usize,
    wildlife: Wildlife,
) -> arrayvec::ArrayVec<u16, 2> {
    let mut features = arrayvec::ArrayVec::new();
    let cell = board.grid.get(idx);
    if !cell.is_present() {
        return features;
        ...
        if widx != idx {
            ...
            for (dir, &(dq, dr)) in HexCoord::LINE_DIRECTIONS.iter().enumerate() {
                ...
                if let Some(nidx) = neighbor.to_index() {
                    ...
                    if n_wl > 0 || old_wl > 0 {
                        removed.push(
                            (pair_base + dir * PAIR_STATES + old_wl as usize * 7 + n_wl as usize)
                                as u16,
                        );
                    }
                    if n_wl > 0 || new_wl > 0 {
                        added.push(
                            (pair_base + dir * PAIR_STATES + new_wl as usize * 7 + n_wl as usize)
                                as u16,
                        );
                    }
                }
                ...
                if let Some(rnidx) = rev.to_index() {
                    ...
                    if rn_wl > 0 {
                        removed.push(
                            (pair_base + dir * PAIR_STATES + rn_wl as usize * 7 + old_wl as usize)
                                as u16,
                        );
                        added.push(
                            (pair_base + dir * PAIR_STATES + rn_wl as usize * 7 + new_wl as usize)
                                as u16,
                        );
                    }
                }
...
pub fn extract_phase_pattern_features(
    board: &Board,
    _cards: &cascadia_core::types::ScoringCards,
) -> Vec<u16> {
...
impl BagInfo {
    ...
    pub fn from_game_for_player(game: &cascadia_core::game::GameState, player: usize) -> Self {
        ...
        for i in 0..game.num_players {
            if i == player || slot >= NUM_OPP_SLOTS {
                continue;
            }
...
fn compute_opponent_detail(board: &Board) -> OpponentDetail {
    let adj = &*ADJACENCY;
    let wildlife_counts: [u8; 5] = std::array::from_fn(|i| board.wildlife_positions[i].len() as u8);
    let largest_group: [u8; 5] = std::array::from_fn(|i| board.largest_group[i] as u8);
    ...
    let mut bear_singleton_count: u8 = 0;
    {
        ...
        for &p in board.wildlife_positions[Wildlife::Bear as usize].iter() {
            ...
            if size == 1 {
                bear_singleton_count = bear_singleton_count.saturating_add(1);
            }
        }
    }
    ...
    for &p in board.wildlife_positions[Wildlife::Elk as usize].iter() {
        ...
        for &(dq, dr) in &cascadia_core::hex::HexCoord::LINE_DIRECTIONS {
            ...
            if len > longest_elk_line {
                longest_elk_line = len;
            }
        }
    }
...
fn compute_alt_elk_shape(board: &Board) -> u8 {
    ...
    'outer: for &p in positions.iter() {
        let nbrs: arrayvec::ArrayVec<u16, 6> = adj
        ...
        if max_shape < 4 {
            for &t in triangle_anchors.iter() {
                ...
                for n in adj.neighbors_of(t as usize) {
                    if elk[n] {
                        let _ = tri_neigh.try_push(n as u16);
                    }
                }
...
pub fn extract_tile_tokens(
    board: &Board,
    bag: Option<&BagInfo>,
) -> (Vec<TileToken>, GlobalFeatures) {
    ...
    let global = GlobalFeatures {
        ...
        market_terrain1: bag
            .map(|b| std::array::from_fn(|i| b.market[i].terrain1))
            .unwrap_or([0; 4]),
        market_terrain2: bag
            .map(|b| std::array::from_fn(|i| b.market[i].terrain2))
            .unwrap_or([0; 4]),
        market_wildlife: bag
            .map(|b| std::array::from_fn(|i| b.market[i].wildlife_token))
            .unwrap_or([0; 4]),
...
pub fn extract_features_with_bag(board: &Board, bag: Option<&BagInfo>) -> Vec<u16> {
    ...
    #[allow(unreachable_code)]
    {
        ...
        let allowed_base = CELL_FEATURES
            + PHASE_FEATURES
            + PAIR_FEATURES
            + PATTERN_FEATURES
            + BAG_FEATURES
        ...
        for &tile_idx in board.placed_tiles.iter() {
            ...
            if cell.is_present() && !cell.has_wildlife() {
                ...
                for w in Wildlife::ALL {
                    if mask.contains(w) {
                        features
                            .push((allowed_base + idx * ALLOWED_WL_PER_CELL + w as usize) as u16);
                    }
                }
            }
        }
        ...
        for &tile_idx in board.placed_tiles.iter() {
            ...
            for (dir, &(dq, dr)) in HexCoord::LINE_DIRECTIONS.iter().enumerate() {
                let neighbor = HexCoord::new(coord.q + dq, coord.r + dr);
                if let Some(nidx) = neighbor.to_index() {
                    if !board.grid.get(nidx).is_present() {
                        continue;
                    }
                    ...
                    if my_terrain > 0 && n_terrain > 0 {
                        let pair_idx = dir * TERRAIN_PAIR_STATES
        ...
        // ─────────────────────────────────────────────────────────────────
        // oppmarket-feat block (Lever 4) — appended after the v4-opp block.
        // For mid-features+v4-opp+oppmarket, the new base is
        // NUM_FEATURES_MID_V4 = 11231. Older NNUE binaries load with
        // zero-padded weights on these new columns (append-only).
        // ─────────────────────────────────────────────────────────────────
        #[cfg(feature = "oppmarket-feat")]
        if let Some(b) = bag {
            extract_oppmarket_features(b, &mut features, NUM_FEATURES_MID_V4);
        }
        ...
        #[cfg(feature = "czero-feat")]
        {
            let mut off = CZERO_FEAT_BASE;
            if let Some(b) = bag {
                extract_czero_bonus_race_features(b, &mut features, off);
                off += CZERO_BONUS_RACE_FEATURES;
                extract_czero_opp_market_features(b, &mut features, off);
                off += CZERO_OPP_MARKET_FEATURES;
                extract_czero_nature_features(board, b, &mut features, off);
                off += CZERO_NATURE_FEATURES;
            } else {
                off +=
                    CZERO_BONUS_RACE_FEATURES + CZERO_OPP_MARKET_FEATURES + CZERO_NATURE_FEATURES;
            }
            extract_czero_habitat_bucket_features(board, &mut features, off);
        }
...
#[cfg(feature = "czero-feat")]
fn czero_gap_bin(diff: i32) -> usize {
    (diff.clamp(-10, 10) + 10) as usize
}

#[cfg(feature = "czero-feat")]
fn extract_czero_bonus_race_features(bag: &BagInfo, features: &mut Vec<u16>, base: usize) {
    for t in 0..5 {
        let mine = bag.my_largest_group[t] as i32;
        let opp_sizes: [i32; NUM_OPP_SLOTS] =
            std::array::from_fn(|i| bag.opp_detail[i].largest_group[t] as i32);
        let larger = opp_sizes
            .iter()
            .filter(|&&s| s > mine)
            .count()
            .min(CZERO_RANK_BINS - 1);
        let max_opp = opp_sizes.iter().copied().max().unwrap_or(0);
        let mut sorted = opp_sizes;
        sorted.sort_unstable_by(|a, b| b.cmp(a));
        let second_ref = sorted.get(1).copied().unwrap_or(0);

        let terr_base = base + t * CZERO_BONUS_RACE_PER_TERR;
        let my_bin = (mine as usize).min(CZERO_MY_SIZE_BINS - 1);
        features.push((terr_base + my_bin) as u16);
        let rank_base = terr_base + CZERO_MY_SIZE_BINS;
        features.push((rank_base + larger) as u16);
        let leader_gap_base = rank_base + CZERO_RANK_BINS;
        features.push((leader_gap_base + czero_gap_bin(mine - max_opp)) as u16);
        let second_gap_base = leader_gap_base + CZERO_GAP_BINS;
        features.push((second_gap_base + czero_gap_bin(mine - second_ref)) as u16);
    }
}

#[cfg(feature = "czero-feat")]
fn extract_czero_opp_market_features(bag: &BagInfo, features: &mut Vec<u16>, base: usize) {
    let terr_base = base + CZERO_OPP_MARKET_TOKEN_FEATURES;
    for (slot_i, slot) in bag.market.iter().enumerate() {
        for (opp_i, opp) in bag.opp_detail.iter().enumerate() {
            if slot.wildlife_token > 0 {
                let w = (slot.wildlife_token - 1) as usize;
                let count = (opp.wildlife_counts[w] as usize).min(CZERO_MARKET_COUNT_BINS - 1);
                let block =
                    base + ((slot_i * NUM_OPP_SLOTS + opp_i) * 5 + w) * CZERO_MARKET_COUNT_BINS;
                features.push((block + count) as u16);
            }

            let mut terr_seen = [false; 5];
            for encoded in [slot.terrain1, slot.terrain2] {
                if encoded == 0 {
                    continue;
                }
                let t = (encoded - 1) as usize;
                if t >= 5 || terr_seen[t] {
                    continue;
                }
                terr_seen[t] = true;
                let count = (opp.largest_group[t] as usize).min(CZERO_MARKET_COUNT_BINS - 1);
                let block = terr_base
                    + ((slot_i * NUM_OPP_SLOTS + opp_i) * 5 + t) * CZERO_MARKET_COUNT_BINS;
                features.push((block + count) as u16);
            }
        }
    }
...
#[cfg(feature = "czero-feat")]
fn extract_czero_nature_features(
    board: &Board,
    bag: &BagInfo,
    features: &mut Vec<u16>,
    base: usize,
) {
    let tokens = (board.nature_tokens as usize).min(CZERO_NATURE_TOKEN_BINS - 1);
    features.push((base + tokens) as u16);

    let market_counts_base = base + CZERO_NATURE_TOKEN_BINS;
    let mut market_counts = [0usize; 5];
    for slot in bag.market.iter() {
        if slot.wildlife_token > 0 {
            let w = (slot.wildlife_token - 1) as usize;
            if w < 5 {
                market_counts[w] += 1;
    ...
    for w in 0..5 {
        let count = market_counts[w].min(CZERO_MARKET_WL_COUNT_BINS - 1);
        features.push((market_counts_base + w * CZERO_MARKET_WL_COUNT_BINS + count) as u16);
    }

    let afford_base = market_counts_base + 5 * CZERO_MARKET_WL_COUNT_BINS;
    if board.nature_tokens > 0 {
        features.push(afford_base as u16);
    }
    if board.nature_tokens > 1 {
        features.push((afford_base + 1) as u16);
    }
}

#[cfg(feature = "czero-feat")]
fn cell_has_terrain(board: &Board, idx: usize, terrain: Terrain) -> bool {
    let cell = board.grid.get(idx);
    if !cell.is_present() {
        return false;
    }
    (0..6).any(|d| {
        cascadia_core::board::terrain_on_edge(cell, board.rotations[idx], d) == Some(terrain)
    })
}

#[cfg(feature = "czero-feat")]
fn extract_czero_habitat_bucket_features(board: &Board, features: &mut Vec<u16>, base: usize) {
    let adj = &*ADJACENCY;
    for terrain in Terrain::ALL {
        let t = terrain as usize;
        let mut visited = [false; GRID_SIZE];
        let mut components: Vec<(u16, Vec<usize>)> = Vec::new();

        for &tile_idx in board.placed_tiles.iter() {
            let idx = tile_idx as usize;
            if visited[idx] || !cell_has_terrain(board, idx, terrain) {
                continue;
            }
            ...
            while let Some(cur) = queue.pop() {
                ...
                for d in 0..6 {
                    ...
                    let my_terr = cascadia_core::board::terrain_on_edge(
                        cur_cell,
                        board.rotations[cur_idx],
                        d,
                    );
                    let n_terr = cascadia_core::board::terrain_on_edge(
                        n_cell,
                    ...
                    if my_terr == Some(terrain) && n_terr == Some(terrain) {
                        visited[n_idx] = true;
                        if !queue.is_full() {
                            queue.push(n_val);
                        }
            ...
            components.push((cells.len() as u16, cells));
        }

        let mut sizes: Vec<u16> = components.iter().map(|(s, _)| *s).collect();
        sizes.sort_unstable_by(|a, b| b.cmp(a));
        let largest = sizes.first().copied().unwrap_or(0);
        let second = sizes.iter().copied().find(|&s| s < largest).unwrap_or(0);

        for (size, cells) in components {
            let role = if size == largest && largest > 0 {
                0
            } else if size == second && second > 0 {
                1
            } else {
                2
            };
            for idx in cells {
                let feature =
                    base + idx * 5 * CZERO_HAB_BUCKET_ROLES + t * CZERO_HAB_BUCKET_ROLES + role;
...
/// Lever-4 "OPP × MARKET" extractor — v0 ships only the bonus-race
/// tightness block (25 features). For each terrain t, fires exactly one
/// of 5 bins depending on `my_size - opp_max_size`:
///   bin 0: behind by ≥5    bin 1: behind by 1-4
///   bin 2: tied            bin 3: leading by 1-4
///   bin 4: leading by ≥5
/// Drives rank-bonus tiebreaker awareness that NNUE alone has to infer
/// implicitly from per-cell + per-opponent blocks.
#[cfg(feature = "oppmarket-feat")]
fn extract_oppmarket_features(bag: &BagInfo, features: &mut Vec<u16>, base: usize) {
    for t in 0..5 {
        let my = bag.my_largest_group[t] as i32;
        let opp = bag.max_opponent_habitat[t] as i32;
        let diff = my - opp;
        let bin = if diff <= -5 {
            0
        } else if diff <= -1 {
        ...
        features.push((base + t * OPM_BONUS_RACE_BINS + bin) as u16);
    }
}
...
fn extract_opp_detailed_features(bag: &BagInfo, features: &mut Vec<u16>, base: usize) {
    for (slot, opp) in bag.opp_detail.iter().enumerate() {
        ...
        if opp.has_bear_singleton {
            features.push((pat_base + 0) as u16);
        }
        if opp.has_elk_line_3plus {
            features.push((pat_base + 1) as u16);
        }
        if opp.has_salmon_run_4plus {
            features.push((pat_base + 2) as u16);
        }
        if opp.isolated_hawk_count >= 5 {
            features.push((pat_base + 3) as u16);
        }
    }
}
...
fn extract_v2_features(board: &Board, bag: Option<&BagInfo>, features: &mut Vec<u16>) {
    ...
    for wtype in 0..5 {
        let count = board.wildlife_positions[wtype]
            .len()
...
fn extract_pattern_features(board: &Board, features: &mut Vec<u16>, base: usize) {
    ...
    let avg_div = if fox_positions.is_empty() {
        0
    } else {
...
impl NNUENetwork {
    ...
    /// Enable the 11-head value architecture while preserving the current scalar
    /// value function at initialization. Warm-started v4/v5 weights therefore
    /// begin with identical total predictions, then specialize per subscore.
    pub fn enable_split11_from_current_value(&mut self) {
        ...
        for h in 0..NUM_HEADS {
            let dst = h * HIDDEN2;
            for j in 0..HIDDEN2 {
                self.w3_heads[dst + j] = self.w3[j] * inv;
            }
            self.b3_heads[h] = self.b3 * inv;
        }
        self.has_split11_heads = true;
        self.has_split_value_heads = false;
    }
    ...
    /// Train the 11 split heads and shared trunk from a total target plus
    /// per-subscore targets. Returns the total-value squared error.
    pub fn train_sample_split11(
        &mut self,
        features: &[u16],
        target_total: f32,
        targets: &[f32; NUM_HEADS],
        lr: f32,
    ) -> f32 {
        self.enable_split11_from_current_value();
        ...
        for h in 0..NUM_HEADS {
            let row = &self.w3_heads[h * HIDDEN2..(h + 1) * HIDDEN2];
            let mut out = self.b3_heads[h];
            for j in 0..HIDDEN2 {
                out += h2[j] * row[j];
            }
            heads[h] = out;
        }
        let sum: f32 = heads.iter().sum();
        let total_error = sum - target_total;
        let total_loss = total_error * total_error;

        let head_weight = 0.25f32;
        let total_weight = 1.0f32;
        let mut d_head = [0.0f32; NUM_HEADS];
        for h in 0..NUM_HEADS {
            let head_error = heads[h] - targets[h];
            d_head[h] = lr
                * (2.0 * total_weight * total_error
                    + 2.0 * head_weight * head_error / NUM_HEADS as f32);
        }

        let mut d_h2 = [0.0f32; HIDDEN2];
        for h in 0..NUM_HEADS {
            let row_base = h * HIDDEN2;
            for j in 0..HIDDEN2 {
                if h2_pre[j] > 0.0 {
                    d_h2[j] += d_head[h] * self.w3_heads[row_base + j];
                }
            }
        }
        ...
        for h in 0..NUM_HEADS {
            let row_base = h * HIDDEN2;
            for j in 0..HIDDEN2 {
                self.w3_heads[row_base + j] -= d_head[h] * h2[j];
            }
            self.b3_heads[h] -= d_head[h];
        }
    ...
    pub fn train_ranking_batch(
        ...
        // Top-1 check
        let mce_best = mce_scores
            .iter()
            .enumerate()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(i, _)| i)
            .unwrap_or(0);
        let pred_best = logits
            .iter()
            .enumerate()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(i, _)| i)
        ...
        for ci in 0..n {
            ...
            for i in 0..HIDDEN1 {
                if h1s[ci][i] > 0.0 {
                    let base = i * HIDDEN2;
                    for j in 0..HIDDEN2 {
                        self.w2[base + j] -= d_h2[j] * h1s[ci][i];
                    }
                }
            }
            for j in 0..HIDDEN2 {
    ...
    pub fn train_policy_delta(
        ...
        for pos in positions {
            ...
            let base_set: std::collections::HashSet<u16> =
                pos.base_features.iter().copied().collect();

            let deltas: Vec<Vec<(u16, f32)>> = pos
                .candidates
                .iter()
                .map(|(feats, _)| {
            ...
            let policy_logits: Vec<f32> = deltas
                .iter()
                .map(|delta| {
            ...
            let pred: Vec<f32> = {
                let exps: Vec<f32> = policy_logits
                    .iter()
            ...
            let mce_best = mce_scores
                ...
                .map(|(i, _)| i)
                .unwrap_or(0);
            let policy_best = policy_logits
                .iter()
                .enumerate()
                .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
                .map(|(i, _)| i)
                .unwrap_or(0);
            if mce_best == policy_best {
                agree += 1;
            }
        ...
        let avg_loss = if n_positions > 0 {
            (total_loss / n_positions as f64) as f32
        } else {
            0.0
        };
        let agree_pct = if n_positions > 0 {
            100.0 * agree as f32 / n_positions as f32
        } else {
            0.0
        };
        (avg_loss, agree_pct)
...
impl PolicyNetwork {
    ...
    pub fn forward(&self, features: &[u16]) -> f32 {
        ...
        for &fi in features {
            ...
            if base + self.hidden1 > self.w1.len() {
                continue;
            }
    ...
    pub fn rank_candidates(&self, candidate_features: &[Vec<u16>]) -> Vec<f32> {
        let logits: Vec<f32> = candidate_features.iter().map(|f| self.forward(f)).collect();
        // Softmax
    ...
    /// Load from PLCY or CZP1 binary format.
    pub fn load(path: &std::path::Path) -> std::io::Result<Self> {
        use std::io::Read;
        let mut file = std::fs::File::open(path)?;
        let mut buf4 = [0u8; 4];
        ...
        let read_str = |f: &mut std::fs::File| -> std::io::Result<String> {
        ...
        if &magic == b"CZP1" {
            ...
            if version == 2 {
                ...
                if crate::score_target::ScoreTarget::from_u8(target[0])
                    != Some(crate::score_target::ScoreTarget::WithHabitatBonus)
                {
                    return Err(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        "CZP1 is not with-bonus",
                    ));
                }
                let mut cards = [0u8; 5];
                file.read_exact(&mut cards)?;
                if cards != [0u8; 5] {
                    return Err(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        "CZP1 is not AAAAA",
                ...
                let _feature_set = read_str(&mut file)?;
                let _git_rev = read_str(&mut file)?;
            }
        }
        ...
        eprintln!(
            "Loaded PolicyNetwork: {}→{}→{}→1",
            num_features, hidden1, hidden2
        );
...
// Always compiled — the spiral lookup + habitat-role helpers are reused
// by `hybrid.rs` across feature flag combinations.
pub(crate) mod v6_peak {
    use super::*;
    ...
    fn build_global_to_local() -> [i16; 441] {
        ...
        debug_assert_eq!(
            next as usize, V6_LOCAL_CELLS,
            "spiral build produced {} cells, expected {}",
            next, V6_LOCAL_CELLS
        );
        tbl
    }
...
pub fn extract_features_v6_peak(board: &Board, bag: Option<&BagInfo>) -> Vec<u16> {
    ...
    let mut base = 0usize;
    let cell_core_base = base;
    base += V6_CELL_FEATURES; // 1397
    let phase_base = base;
    base += PHASE_FEATURES; // 110
    let pair_wl_base = base;
    base += PAIR_FEATURES; // 147
    let pattern_base = base;
    base += PATTERN_FEATURES; // 89
    let bag_base = base;
    base += BAG_FEATURES; // 55
    let opp_hab_base = base;
    base += OPP_HAB_FEATURES; // 55
    let allowed_base = base;
    base += V6_ALLOWED_WL_FEATURES; // 635
    let wl_ext_base = base;
    base += WL_COUNT_EXT_FEATURES; // 50
    let terr_pair_base = base;
    base += TERRAIN_PAIR_FEATURES; // 108
    let sec_terr_base = base;
    base += V6_SEC_TERRAIN_FEATURES; // 635
    let hab_ext_base = base;
    base += HAB_EXT_FEATURES; // 70
    let wl_ext2_base = base;
    base += WL_COUNT_EXT2_FEATURES; // 55
    let ext_cap_base = base;
    base += EXT_CAP_FEATURES; // 40
    let pat2_base = base;
    base += PATTERN_V2_FEATURES; // 48
    let bag_ext_base = base;
    base += BAG_EXT_FEATURES; // 105
    let opp_ext_base = base;
    base += OPP_HAB_EXT_FEATURES; // 70
    let market_base = base;
    base += MARKET_FEATURES; // 88
    let tbag_t_base = base;
    base += TBAG_TERRAIN_FEATURES; // 105
    let tbag_w_base = base;
    base += TBAG_WL_FEATURES; // 105
    let tbag_te_base = base;
    base += TBAG_TERRAIN_EXT_FEATURES; // 150
    let tbag_we_base = base;
    base += TBAG_WL_EXT_FEATURES; // 150
    let overflow_base = base;
    base += OVERFLOW_FEATURES; // 1
    let v4opp_base = base;
    base += OPP_DETAILED_FEATURES; // 369
    let frontier_base = base;
    base += V6_FRONTIER_FLAG_FEATURES; // 127
    let v5opp_base = base;
    base += V5_OPP_PAT_FEATURES; // 108
    let bonus_base = base;
    base += V5_BONUS_DIST_FEATURES; // 315
    let hab_str_base = base;
    base += V5_HAB_STRUCT_FEATURES; // 85
    let tbag_j_base = base;
    base += V5_TBAG_JOINT_FEATURES; // 525
    let cell_adj_wl_base = base;
    base += V6_CELL_ADJ_WL_FEATURES; // 5334
    let cell_adj_terr_base = base;
    base += V6_CELL_ADJ_TERR_FEATURES; // 4572
    let hab_bucket_base = base;
    base += V6_HAB_BUCKET_FEATURES; // 1905
    ...
    for &tile_idx in board.placed_tiles.iter() {
        ...
        for (dir, &(dq, dr)) in HexCoord::LINE_DIRECTIONS.iter().enumerate() {
            ...
            if let Some(nidx) = neighbor.to_index() {
                ...
                if my_terrain > 0 && n_terrain > 0 {
                    let pair_idx =
                        dir * TERRAIN_PAIR_STATES + my_terrain as usize * 6 + n_terrain as usize;
    ...
    if let Some(b) = bag {
        ...
        for (i, slot) in b.market.iter().enumerate() {
            ...
            if slot.wildlife_token > 0 {
                features
                    .push((slot_base + 5 + 6 + 5 + 1 + (slot.wildlife_token - 1) as usize) as u16);
            }
        }
        for t in 0..5 {
            let count = (b.tbag_terrain[t] as usize).min(BAG_EXT_BINS - 1);
...
fn extract_alt_elk_features(board: &Board, features: &mut Vec<u16>, base: usize) {
    ...
    if positions.is_empty() {
        ...
        for k in 0..5 {
            features.push((base + k * ALT_ELK_SHAPE_BINS) as u16);
        }
        return;
    }
    let adj = &*ADJACENCY;
    ...
    for &p in positions.iter() {
        ...
        let kind = match n {
            ...
            _ => {
                ...
                // triangle
                else if n == 4 && has_triangle {
                    3
                }
                // rhombus = 4-cluster with triangle subshape
                else {
                    4
                } // blob (line, Y-shape, larger cluster)
            }
        };
        counts[kind] += 1;
    }
                        ...
                        Some(idx) => {
                            if hawk_set[idx] {
                                ...
                                if (i as u8) < j && steps >= 2 {
                                    let unique = (types_mask & !(1 << Wildlife::Hawk as u8))
                                        .count_ones()
                                        as usize;
...
fn extract_alt2_hawk_los_features(board: &Board, features: &mut Vec<u16>, base: usize) {
    ...
    for &p in positions.iter() {
        let local = match alt2_local_idx(p as usize) {
            Some(l) => l,
            None => continue,
        };
        let coord = HexCoord::from_index(p as usize);
...
fn extract_alt2_salmon_features(board: &Board, features: &mut Vec<u16>, base: usize) {
    ...
    for &p in positions.iter() {
        ...
        let len_class: usize = if !valid || len <= 1 {
            0
        } else if len == 2 {
            1
        } else if len <= 4 {
...
fn extract_alt2_fox_features(board: &Board, features: &mut Vec<u16>, base: usize) {
    ...
    for &p in positions.iter() {
        ...
        let density_class: usize = if occupied <= 1 {
            0
        } else if occupied <= 3 {
            1
        } else if occupied <= 5 {
...
fn extract_alt2_bear_features(board: &Board, features: &mut Vec<u16>, base: usize) {
    ...
    for &p in positions.iter() {
        ...
        let size_class: usize = match size {
            1 => 0,
            2 => 1,
            3 => 2,
            _ => 3,
...
mod oppmarket_tests {
    ...
    #[test]
    fn oppmarket_block_appended_at_correct_base() {
        // Sanity: NUM_FEATURES with this combo equals NUM_FEATURES_MID_V4_OPM.
        assert_eq!(NUM_FEATURES, NUM_FEATURES_MID_V4_OPM);
        assert_eq!(OPPMARKET_FEATURES, 25);
        assert_eq!(NUM_FEATURES_MID_V4_OPM - NUM_FEATURES_MID_V4, 25);
    }

    #[test]
    fn oppmarket_fires_exactly_one_bin_per_terrain() {
        // Build a fresh game and extract features. Filter to those in the
        // oppmarket range; assert exactly 5 (one per terrain).
        let mut rng = StdRng::seed_from_u64(0xF00D);
        let g = GameState::new(4, ScoringCards::all_a(), &mut rng);
        let bag = BagInfo::from_game_for_player(&g, 0);
        let features = extract_features_with_bag(&g.boards[0], Some(&bag));
        let lo = NUM_FEATURES_MID_V4;
        let hi = NUM_FEATURES_MID_V4_OPM;
        let opm_count = features
            .iter()
            .filter(|&&f| (f as usize) >= lo && (f as usize) < hi)
            .count();
        assert_eq!(
            opm_count, 5,
            "expected 5 oppmarket features, got {}",
            opm_count
        );
    }

    #[test]
    fn oppmarket_tied_terrain_fires_bin_2() {
        // Construct a synthetic BagInfo with my_size == opp_max for one
        // terrain and verify the "tied" bin (2) fires.
        let mut bag = BagInfo::default();
        bag.my_largest_group = [3, 0, 0, 0, 0];
        bag.max_opponent_habitat = [3, 0, 0, 0, 0];
        let mut features: Vec<u16> = Vec::new();
        extract_oppmarket_features(&bag, &mut features, NUM_FEATURES_MID_V4);
        // Forest (terrain 0) has my_size = opp_max = 3 → tied → bin 2.
        let expected = (NUM_FEATURES_MID_V4 + 0 * OPM_BONUS_RACE_BINS + 2) as u16;
        assert!(
            features.contains(&expected),
            "tied bin {} not fired; features = {:?}",
            expected,
            features
        );
    }
