...
use cascadia_ai::eval::{best_move_lookahead, best_move_with_potential};
...
static CZERO_POLICY_NET: OnceLock<Arc<cascadia_ai::nnue::PolicyNetwork>> = OnceLock::new();

fn opponent_net() -> Option<&'static Arc<cascadia_ai::nnue::NNUENetwork>> {
    OPPONENT_NET.get()
}

fn czero_policy_net() -> Option<&'static Arc<cascadia_ai::nnue::PolicyNetwork>> {
    if CZERO_POLICY_NET.get().is_none() {
        let path = std::env::var("CASCADIA_POLICY_WEIGHTS")
            .or_else(|_| std::env::var("CZERO_POLICY_WEIGHTS"))
            .unwrap_or_else(|_| "czero.policy".to_string());
        if let Ok(policy) = cascadia_ai::nnue::PolicyNetwork::load(std::path::Path::new(&path)) {
            let _ = CZERO_POLICY_NET.set(Arc::new(policy));
        } else {
            eprintln!("⚠ failed to load CascadiaZero policy weights from {}", path);
        }
    }
    CZERO_POLICY_NET.get()
}
...
enum Strategy {
    ...
    PolicyMCE {
        net: Arc<cascadia_ai::nnue::NNUENetwork>,
        policy: Arc<cascadia_ai::nnue::PolicyNetwork>,
        rollouts: usize,
        top_k: usize,
    },
    CZeroMCE {
    ...
    AlphaZero {
        net: Arc<cascadia_ai::alphazero::AlphaZeroNet>,
        simulations: usize,
    },
    #[cfg(feature = "az-v2")]
    AlphaZeroV2 {
        net: Arc<cascadia_ai::alphazero_v2::AlphaZeroNetV2>,
        simulations: usize,
    },
    NRPA {
        net: Arc<cascadia_ai::nnue::NNUENetwork>,
    ...
    GreedyMCE {
        rollouts: usize,
        alloc: cascadia_ai::mce::GreedyMceAlloc,
        expanded: bool,
    },
    NnueRolloutMCE {
        net: Arc<cascadia_ai::nnue::NNUENetwork>,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Strategy::Expectimax {
                ...
                branching,
                ..
            } => write!(f, "expectimax(k={},d={},b={})", samples, depth, branching),
            Strategy::ExactExpectimax { .. } => write!(f, "exact-expectimax"),
            Strategy::Hybrid {
                rollouts, top_k, ..
            } => write!(f, "hybrid(k={},n={})", top_k, rollouts),
            Strategy::MCTS { simulations, .. } => write!(f, "mcts(n={})", simulations),
            Strategy::PolicyMCE {
                rollouts, top_k, ..
            } => write!(f, "policy-mce(k={},n={})", top_k, rollouts),
            Strategy::CZeroMCE { rollouts, .. } => write!(f, "czero-mce(n={})", rollouts),
            Strategy::AlphaZero { simulations, .. } => write!(f, "alphazero(n={})", simulations),
            #[cfg(feature = "az-v2")]
            Strategy::AlphaZeroV2 { simulations, .. } => {
                write!(f, "alphazero_v2(n={})", simulations)
            }
            Strategy::NRPA { level, n, .. } => write!(f, "nrpa(L={},N={})", level, n),
            Strategy::OpenLoopMCTS { rollouts, .. } => write!(f, "ol-mcts(n={})", rollouts),
            Strategy::GumbelMCTS { rollouts, m, .. } => {
                write!(f, "gumbel-mcts(n={},m={})", rollouts, m)
            }
            Strategy::GreedyMCE {
                rollouts,
                alloc,
            ...
            Strategy::NnueRolloutMCE {
                ...
                let e = if *expanded { ",expanded" } else { "" };
                let p = if *prefilter_k > 0 {
                    format!(",pfk={}", prefilter_k)
                } else {
                    String::new()
                };
                let g = if *exact_endgame > 0 {
                    format!(",eg={}", exact_endgame)
                } else {
                    String::new()
                };
...
fn pick_move(
    ...
    match strategy {
        ...
        Strategy::MCE { net, rollouts } => {
            cascadia_ai::mce::best_move_mce(game, net, *rollouts, search_rng)
        }
        Strategy::Expectimax {
            net,
            samples,
            ...
            if *depth <= 1 {
                ...
                cascadia_ai::expectimax::best_move_expectimax_deep(
                    game, net, *samples, *depth, *branching, search_rng,
                )
            }
        }
        ...
        Strategy::PolicyMCE {
            ...
            rollouts,
            top_k,
        } => cascadia_ai::mce::best_move_mce_with_policy(
            game, net, policy, *rollouts, *top_k, search_rng,
        ),
        Strategy::CZeroMCE {
            net,
            policy,
            rollouts,
        } => cascadia_ai::mce::best_move_czero_mce(game, net, policy, *rollouts, search_rng),
        Strategy::AlphaZero { net, simulations } => {
            cascadia_ai::alphazero::best_move_alpha_zero(game, net, *simulations, search_rng)
        }
        #[cfg(feature = "az-v2")]
        Strategy::AlphaZeroV2 { net, simulations } => {
            cascadia_ai::alphazero_v2::best_move_v2(game, net, *simulations, search_rng)
        }
        Strategy::NRPA { net, .. } => cascadia_ai::nrpa::best_move_nrpa(game, net, search_rng),
        ...
        Strategy::GreedyMCE {
            ...
            cascadia_ai::mce::best_move_greedy_mce_v2(
                game, *rollouts, *alloc, candidates, search_rng,
            )
        }
        Strategy::NnueRolloutMCE {
            net,
            ...
            if *prefilter_k > 0 && candidates.len() > *prefilter_k {
                ...
                let expand: usize = std::env::var("MCE_MUTATE_EXPAND")
                    .ok()
                    .and_then(|s| s.parse().ok())
                    .unwrap_or(0);
            ...
            cascadia_ai::mce::best_move_nnue_rollout_mce(
                game, net, *rollouts, *alloc, candidates, search_rng,
            )
        }
        Strategy::UctMcts {
            simulations,
...
fn pre_move_optimize(
    ...
    let net = match strategy {
        Strategy::NNUE { ref net }
        | Strategy::MCE { ref net, .. }
        | Strategy::Hybrid { ref net, .. }
        | Strategy::ExactExpectimax { ref net }
...
fn pick_move_by_tag(
    ...
    match tag {
        ...
        "mce_strategy" | "mce_strat" => pick_with_env(&[("MCE_STRATEGY_BIAS", "1")], || {
            pick_move_by_tag(game, "mce", net, search_rng)
        }),
        "mce_cv" | "mce_cv_0.85" => pick_with_env(&[("MCE_CV_ALPHA", "0.85")], || {
            pick_move_by_tag(game, "mce", net, search_rng)
        }),
        "mce_lmr" => pick_with_env(&[("MCE_LMR", "1")], || {
            pick_move_by_tag(game, "mce", net, search_rng)
        }),
        ...
        "czero_mce" | "czero-mce" => pick_with_env(
            &[
                ("CASCADIA_SCORE_TARGET", "with-bonus"),
                ("MCE_SCORE_TARGET", "with-bonus"),
                ("MCE_LMR", "1"),
                ("MCE_DIVERSE_PREFILTER", "1"),
                ("MCE_CANDIDATES", "32"),
            ],
            || match (net, czero_policy_net()) {
                (Some(n), Some(policy)) => {
                    cascadia_ai::mce::best_move_czero_mce(game, n, policy, 600, search_rng)
                }
                (Some(n), None) => {
                    let mut cands = cascadia_ai::mce::expanded_candidates(game);
                    if cands.len() > 32 {
                ...
                _ => cascadia_ai::search::greedy_move(game),
            },
        ),
...
fn simulate_game(
    ...
    cascadia_core::scoring::ScoreBreakdown,
    cascadia_core::scoring::ScoreBreakdown,
) {
...
fn with_bonus_breakdown(game: &GameState, player: usize) -> cascadia_core::scoring::ScoreBreakdown {
    let mut g = game.clone();
    cascadia_core::scoring::ScoreBreakdown::compute_with_bonuses(
        &mut g.boards,
        &g.scoring_cards,
        player,
    )
...
fn arg_usize(args: &[String], names: &[&str], default: usize) -> usize {
    arg_value(args, names)
        .and_then(|s| s.parse().ok())
        .unwrap_or(default)
}

fn arg_u64(args: &[String], names: &[&str], default: u64) -> u64 {
    arg_value(args, names)
        .and_then(|s| s.parse().ok())
        .unwrap_or(default)
}

fn arg_f32(args: &[String], names: &[&str], default: f32) -> f32 {
    arg_value(args, names)
        .and_then(|s| s.parse().ok())
        .unwrap_or(default)
}

fn alpha_zero_checkpoint_path(out: &std::path::Path, suffix: &str) -> std::path::PathBuf {
    let parent = out.parent().unwrap_or_else(|| std::path::Path::new(""));
    let stem = out
        .file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("alphazero");
    let ext = out.extension().and_then(|s| s.to_str()).unwrap_or("azr");
    parent.join(format!("{}_{}.{}", stem, suffix, ext))
}

fn save_alpha_zero_checkpoint(
    net: &cascadia_ai::alphazero::AlphaZeroNet,
    out_path: &std::path::Path,
    suffix: &str,
) {
    let ckpt = alpha_zero_checkpoint_path(out_path, suffix);
    if let Err(e) = net.save(&ckpt) {
        eprintln!(
            "Failed to save AlphaZero checkpoint {}: {}",
            ckpt.display(),
            e
        );
        std::process::exit(1);
    }
    if let Err(e) = net.save(out_path) {
        eprintln!(
            "Failed to save AlphaZero final weights {}: {}",
            out_path.display(),
            e
        );
        std::process::exit(1);
    }
    eprintln!("Saved AlphaZero checkpoint {}", ckpt.display());
}

fn run_alpha_zero_training(args: &[String], default_games: usize) {
    let out_path = std::path::PathBuf::from(
        arg_value(args, &["--az-out", "--weights"]).unwrap_or("alphazero.azr"),
    );
    let seed = if args.iter().any(|a| a == "--random-seed") {
        rand::thread_rng().gen::<u64>()
    } else {
        arg_u64(args, &["--seed"], 42)
    };
    let mut rng = StdRng::seed_from_u64(seed);

    let bootstrap_games = arg_usize(args, &["--bootstrap-games"], default_games.max(32));
    let selfplay_iters = arg_usize(args, &["--az-iters", "--iterations"], 2);
    let selfplay_games = arg_usize(
        args,
        &["--az-games", "--selfplay-games"],
        default_games.max(16),
    );
    let simulations = arg_usize(args, &["--az-sims", "--simulations", "--rollouts"], 32);
    let bench_sims = arg_usize(args, &["--bench-sims"], simulations);
    let bench_games = arg_usize(args, &["--bench-games"], 8);
    let epochs = arg_usize(args, &["--epochs"], 2);
    let replay_cap = arg_usize(args, &["--replay-cap"], 100_000);
    let lr = arg_f32(args, &["--lr"], 0.0003);
    let temperature = arg_f32(args, &["--temperature"], 1.0);
    let c_puct = arg_f32(args, &["--c-puct"], 2.0);

    let cfg = cascadia_ai::alphazero::AlphaZeroConfig {
        channels: arg_usize(args, &["--az-channels", "--channels"], 16),
        blocks: arg_usize(args, &["--az-blocks", "--blocks"], 2),
        value_hidden: arg_usize(args, &["--az-hidden", "--hidden"], 64),
        max_candidates: arg_usize(args, &["--max-candidates"], 24),
        c_puct,
    };
    let mut net = if let Some(init_path) = arg_value(args, &["--init-az", "--init-weights"]) {
        eprintln!("Loading AlphaZero init weights from {}", init_path);
        cascadia_ai::alphazero::AlphaZeroNet::load(std::path::Path::new(init_path)).unwrap_or_else(
            |e| {
                eprintln!("Failed to load AlphaZero init weights {}: {}", init_path, e);
                std::process::exit(1);
            },
        )
    } else {
        cascadia_ai::alphazero::AlphaZeroNet::new(cfg, seed ^ 0xA17E_0A00_0000_0001)
    };

    eprintln!("AlphaZero scratch training");
    eprintln!(
        "  out={} seed={} cfg=channels:{} blocks:{} hidden:{} max_candidates:{} c_puct:{}",
        out_path.display(),
        seed,
        net.config().channels,
        net.config().blocks,
        net.config().value_hidden,
        net.config().max_candidates,
        net.config().c_puct
    );
    eprintln!(
        "  bootstrap_games={} selfplay_iters={} selfplay_games/iter={} sims={} temp={} epochs={} lr={} replay_cap={} bench_games={} bench_sims={}",
        bootstrap_games,
        selfplay_iters,
        selfplay_games,
        simulations,
    ...
    let mut replay: Vec<cascadia_ai::alphazero::AzSample> = Vec::new();
    if bootstrap_games > 0 {
        let start = Instant::now();
        let samples =
            cascadia_ai::alphazero::collect_greedy_bootstrap_games(bootstrap_games, &mut rng);
        eprintln!(
            "Bootstrap collected {} samples from {} greedy games in {:.1}s",
            samples.len(),
            bootstrap_games,
            start.elapsed().as_secs_f32()
        );
        replay.extend(samples);
        let stats = net.train_epochs(&replay, epochs, lr);
        eprintln!(
            "Bootstrap train: samples={} policy_loss={:.4} value_loss={:.4}",
            stats.samples, stats.policy_loss, stats.value_loss
        );
        save_alpha_zero_checkpoint(&net, &out_path, "bootstrap");
        if bench_games > 0 {
            let (base, bonus) = cascadia_ai::alphazero::benchmark_alpha_zero(
                &net,
                bench_games,
                bench_sims,
                &mut rng,
            ...
            eprintln!(
                "Bootstrap benchmark vs greedy opponents: base_mean={:.2} with_bonus_mean={:.2}",
                base, bonus
            );
        }
    }

    for iter in 1..=selfplay_iters {
        let start = Instant::now();
        let samples = cascadia_ai::alphazero::collect_selfplay_games(
            &net,
            selfplay_games,
            simulations,
            temperature,
        ...
        eprintln!(
            "Iter {} collected {} AlphaZero self-play samples from {} games in {:.1}s",
            iter,
            samples.len(),
            selfplay_games,
            start.elapsed().as_secs_f32()
        ...
        if replay.len() > replay_cap {
            let drain = replay.len() - replay_cap;
            replay.drain(0..drain);
        }
        let stats = net.train_epochs(&replay, epochs, lr);
        eprintln!(
            "Iter {} train: replay={} sample_updates={} policy_loss={:.4} value_loss={:.4}",
            iter,
            replay.len(),
            stats.samples,
            stats.policy_loss,
        ...
        save_alpha_zero_checkpoint(&net, &out_path, &format!("iter{:03}", iter));
        if bench_games > 0 {
            let (base, bonus) = cascadia_ai::alphazero::benchmark_alpha_zero(
                &net,
                bench_games,
            ...
            eprintln!(
                "Iter {} benchmark vs greedy opponents: base_mean={:.2} with_bonus_mean={:.2}",
                iter, base, bonus
            );
        }
    }

    println!("AlphaZero weights saved to {}", out_path.display());
}

fn run_alpha_zero_collect(args: &[String], default_games: usize) {
    let out_path = std::path::PathBuf::from(
        arg_value(args, &["--out", "--az-data-out"]).unwrap_or("alphazero_samples.azd"),
    );
    let seed = if args.iter().any(|a| a == "--random-seed") {
        rand::thread_rng().gen::<u64>()
    } else {
    ...
    let mut rng = StdRng::seed_from_u64(seed);
    let games = arg_usize(args, &["--az-games", "--games"], default_games.max(1));
    let simulations = arg_usize(args, &["--az-sims", "--simulations", "--rollouts"], 32);
    let temperature = arg_f32(args, &["--temperature"], 1.0);

    let az_arch = arg_value(args, &["--az-arch"]).unwrap_or("v1");

    // Boot-time scoring assertion shared by all AZ pipelines (AAAAA only for v2,
    // matches `CASCADIA_SCORING_CARDS=A,A,A,A,A`).
    let scoring_str =
        std::env::var("CASCADIA_SCORING_CARDS").unwrap_or_else(|_| "A,A,A,A,A".to_string());
    let score_target_str = arg_value(args, &["--score-target"]).unwrap_or("with-bonus");
    eprintln!(
        "az-collect arch={} scoring={} score_target={}",
        az_arch, scoring_str, score_target_str
    );

    #[cfg(feature = "az-v2")]
    if az_arch == "v2" {
        let start = Instant::now();
        let samples = if let Some(weights_path) = arg_value(args, &["--az-weights", "--weights"]) {
            let net =
                cascadia_ai::alphazero_v2::AlphaZeroNetV2::load(std::path::Path::new(weights_path))
                    .unwrap_or_else(|e| {
                        eprintln!("Failed to load AZR2 weights {}: {}", weights_path, e);
                        std::process::exit(1);
                    });
            eprintln!(
                "Collecting AZv2 self-play: games={} sims={} temp={} weights={}",
                games, simulations, temperature, weights_path
            );
            cascadia_ai::alphazero_v2::collect_selfplay_v2(
                &net,
                games,
                simulations,
                temperature,
            ...
            eprintln!("Collecting greedy-bootstrap AZv2 data: games={}", games);
            cascadia_ai::alphazero_v2::collect_bootstrap_v2(games, &mut rng)
        };
        cascadia_ai::alphazero_v2::save_samples_v2(&out_path, &samples).unwrap_or_else(|e| {
            eprintln!("Failed to save AZD2 samples {}: {}", out_path.display(), e);
            std::process::exit(1);
        });
        let summary =
            cascadia_ai::alphazero_v2::inspect_samples_v2(&out_path).unwrap_or_else(|e| {
                eprintln!(
                    "Failed to inspect AZD2 samples {}: {}",
                    out_path.display(),
                    e
                );
                std::process::exit(1);
        ...
        println!(
            "AZD2 saved: samples={} max_candidates={} channels={} local_cells={} \
             max_opponents={} elapsed={:.1}s -> {}",
            summary.samples,
            summary.max_candidates,
            summary.input_channels,
            summary.local_cells,
            summary.max_opponents,
            start.elapsed().as_secs_f32(),
            out_path.display()
        );
        return;
    ...
    let start = Instant::now();
    let samples = if let Some(weights_path) = arg_value(args, &["--az-weights", "--weights"]) {
        let net = cascadia_ai::alphazero::AlphaZeroNet::load(std::path::Path::new(weights_path))
            .unwrap_or_else(|e| {
                eprintln!("Failed to load AlphaZero weights {}: {}", weights_path, e);
                std::process::exit(1);
            });
        eprintln!(
            "Collecting AlphaZero self-play: games={} sims={} temp={} weights={}",
            games, simulations, temperature, weights_path
        );
        cascadia_ai::alphazero::collect_selfplay_games(
            &net,
            games,
            simulations,
            temperature,
        ...
        eprintln!(
            "Collecting greedy-bootstrap AlphaZero data: games={}",
            games
        );
        cascadia_ai::alphazero::collect_greedy_bootstrap_games(games, &mut rng)
    };
    cascadia_ai::alphazero::save_samples(&out_path, &samples).unwrap_or_else(|e| {
        eprintln!(
            "Failed to save AlphaZero samples {}: {}",
            out_path.display(),
            e
        );
        std::process::exit(1);
    });
    let summary = cascadia_ai::alphazero::inspect_samples(&out_path).unwrap_or_else(|e| {
        eprintln!(
            "Failed to inspect AlphaZero samples {}: {}",
            out_path.display(),
            e
        );
        std::process::exit(1);
    ...
    println!(
        "AZD1 saved: samples={} max_candidates={} channels={} grid={} elapsed={:.1}s -> {}",
        summary.samples,
        summary.max_candidates,
        summary.input_channels,
        summary.grid_size,
...
fn czero_subscores(game: &GameState, player: usize) -> [f32; cascadia_ai::nnue::NUM_HEADS] {
    let bd = with_bonus_breakdown(game, player);
    let mut out = [0.0f32; cascadia_ai::nnue::NUM_HEADS];
    for i in 0..5 {
        out[i] = bd.wildlife[i] as f32;
        out[5 + i] = (bd.habitat[i] + bd.habitat_bonus[i] as u16) as f32;
    }
    out[10] = bd.nature_tokens as f32;
    out
}
...
fn simulate_game_inner(
    ...
    let opponents_same = std::env::var("CASCADIA_OPPONENTS_SAME")
        .ok()
    ...
    let seat_tags: Option<Vec<String>> = std::env::var("CASCADIA_SEAT_STRATEGIES")
        .ok()
        .filter(|s| !s.is_empty())
    ...
    let seat_net: Option<Arc<cascadia_ai::nnue::NNUENetwork>> = match strategy {
        ...
        | Strategy::ExactExpectimax { net }
        | Strategy::MCTS { net, .. }
        | Strategy::PolicyMCE { net, .. }
        | Strategy::CZeroMCE { net, .. }
        | Strategy::NRPA { net, .. }
        | Strategy::OpenLoopMCTS { net, .. }
        | Strategy::GumbelMCTS { net, .. }
        | Strategy::NnueRolloutMCE { net, .. }
    ...
    let seat_nets: Option<Vec<Arc<cascadia_ai::nnue::NNUENetwork>>> =
        std::env::var("CASCADIA_SEAT_WEIGHTS")
            .ok()
            .filter(|s| !s.is_empty())
            .map(|s| {
                s.split(':')
                    .map(|path| {
                        Arc::new(
                            cascadia_ai::nnue::NNUENetwork::load(std::path::Path::new(path))
                                .unwrap_or_else(|_| {
                                    panic!("failed to load seat weights: {}", path)
                                }),
                        )
                    })
                    .collect()
    ...
    let mcts_tree_tag_sims: usize = std::env::var("MCTS_TREE_TAG_SIMS")
        ...
        .and_then(|s| s.parse().ok())
        .unwrap_or(600);
    let mcts_tree_tag_parallel: bool = std::env::var("MCTS_TREE_TAG_PARALLEL")
        .ok()
        .map(|s| !s.is_empty() && s != "0" && s.to_ascii_lowercase() != "false")
        .unwrap_or(false);
    let init_forest_for =
        |p: usize, parallel: bool, seed: u64, net: Option<Arc<cascadia_ai::nnue::NNUENetwork>>| {
            let n = if parallel {
                thread::available_parallelism()
                    .map(|nn| nn.get())
                    .unwrap_or(4)
    ...
    let mut mcts_forests: Vec<Option<cascadia_ai::mcts_tree::PersistentUctForest>> = (0..num_seats)
        .map(|p| {
            ...
            if let Some(ref tags) = seat_tags {
                if tags.get(p).map(|t| t == "mcts_tree").unwrap_or(false) {
                    let n = seat_nets
                        .as_ref()
                        .and_then(|nets| nets.get(p))
                        .or(seat_net.as_ref())
                        .map(Arc::clone);
                    return Some(init_forest_for(
                        p,
                        mcts_tree_tag_parallel,
                        search_rng.gen(),
                        n,
                    ));
                }
    ...
    while !game.is_game_over() {
        ...
        if let Some(ref tags) = seat_tags {
            ...
            let net_for_seat: Option<&Arc<cascadia_ai::nnue::NNUENetwork>> = seat_nets
                .as_ref()
                .and_then(|nets| nets.get(p))
                .or(seat_net.as_ref());
            let mv = if let Some(forest) = mcts_forests[p].as_mut() {
                ...
                match tag {
                    "random" => {
                        cascadia_ai::draft_opponents::random_draft_move(&game, &mut search_rng)
                    }
                    "scarcity" => {
                        cascadia_ai::draft_opponents::scarcity_draft_move(&game, &mut search_rng)
                    }
                    "preference" => {
                        if seat_preferences[p].is_none() {
                            seat_preferences[p] = Some(
                                cascadia_ai::draft_opponents::sample_preferences(&mut search_rng),
                            );
                        }
                        let prefs = seat_preferences[p].as_ref().unwrap();
                        cascadia_ai::draft_opponents::preference_draft_move(
                            &game,
                            prefs,
                            &mut search_rng,
                        )
            ...
            match mv {
                Some(m) => {
                    if !execute_scored_move(&mut game, &m) {
                        break;
                    }
        ...
        let mv = if let Some(forest) = mcts_forests[0].as_mut() {
            ...
            if let Strategy::MCE { ref net, rollouts } = strategy {
                let tops =
                    cascadia_ai::mce::top_moves_mce(&game, net, *rollouts, &mut search_rng, 15);
                ...
                for (mv, avg) in &tops {
                    ...
                    if cascadia_ai::search::execute_scored_move(&mut g, mv) {
                        let current = cascadia_core::scoring::ScoreBreakdown::compute(
                            &mut g.boards[game.current_player],
                            &g.scoring_cards,
                        )
                        .total as f32;
                        let target = (*avg as f32 - current).max(0.0);
                        let features =
                            cascadia_ai::nnue::extract_features(&g.boards[game.current_player]);
                        sample_sink
                            .as_mut()
                            .unwrap()
                            .push(cascadia_ai::nnue_train::Sample {
                                features,
                                target,
                                aux_bear: 0.0,
                                aux_salmon: 0.0,
                ...
                tops.into_iter()
                    .next()
                    .map(|(mv, avg)| cascadia_ai::eval::ScoredMove {
    ...
    let base =
        cascadia_core::scoring::ScoreBreakdown::compute(&mut game.boards[0], &game.scoring_cards);
...
fn run_benchmark(strategy: &Strategy, num_games: usize) -> BenchResult {
    ...
    for v in habitat_per_game.iter_mut() {
        v.reserve(num_games);
    }
    for v in wildlife_per_game.iter_mut() {
        v.reserve(num_games);
    }
    ...
    for i in 0..num_games {
        ...
        total_habitat_bonus += with_bonus
            .habitat_bonus
            .iter()
            .map(|&b| b as u64)
            .sum::<u64>();
...
fn print_result(r: &BenchResult) {
    ...
    println!(
        "Results ({} games in {:.1?}, strategy={}):",
        n, r.elapsed, r.strategy
    );
    ...
    println!(
        "    Tokens:   {:5.1} | {:3} | {:3} | {:3} | {:3}",
        tok_m, tok_p10, tok_med, tok_p90, tok_max
    );
    println!();
    ...
    while bucket < max_bucket {
        let count = r
            .scores
            .iter()
            .filter(|&&s| (s as usize) >= bucket && (s as usize) < bucket + bucket_size)
            .count();
...
fn print_comparison(results: &[BenchResult]) {
    ...
    println!(
        "║ Strategy                 ║  Mean  ║ Median ║  P10   ║  P90   ║ StdDev ║ Time      ║"
    );
    println!(
        "╠══════════════════════════╬════════╬════════╬════════╬════════╬════════╬═══════════╣"
    );
    ...
    for line in stdin.lock().lines() {
        ...
        match parts[0] {
            "reset" => {
                ...
                writeln!(
                    out,
                    "{{\"n_candidates\":{},\"current_score\":{},\"done\":{}}}",
                    n_cands, current_score, done
                )
                .unwrap();
                out.flush().unwrap();
            }
            "obs" => {
                ...
                if let Some(ref g) = game {
                    let bag_info = cascadia_ai::nnue::BagInfo::from_game(g);
                    let board_features =
                        cascadia_ai::nnue::extract_features_with_bag(&g.boards[0], Some(&bag_info));
                    ...
                    let scores: Vec<f32> = candidates
                        ...
                        .map(|(i, mv)| {
                            ...
                            let remaining = if features.is_empty() {
                                0.0
                            } else {
                    ...
                    let json = format!(
                        "{{\"board_features\":{:?},\"candidate_scores\":{:?}}}",
            ...
            "step" => {
                ...
                if let Some(ref mut g) = game {
                    ...
                    writeln!(
                        out,
                        "{{\"reward\":{},\"done\":{},\"n_candidates\":{},\"current_score\":{}}}",
                        reward, done, n_cands, current_score
                    )
                    .unwrap();
                    out.flush().unwrap();
                }
...
fn main() {
    ...
    if let Some(opp_path) = args
        .iter()
        .position(|a| a == "--opp-weights")
        .and_then(|i| args.get(i + 1))
    {
    ...
    if let Some(score_target) = args
        .iter()
        .position(|a| a == "--score-target")
        .and_then(|i| args.get(i + 1))
    {
        if cascadia_ai::score_target::ScoreTarget::parse(score_target).is_none() {
            eprintln!(
                "Unknown --score-target '{}'; expected base or with-bonus",
                score_target
            );
            std::process::exit(2);
        }
        std::env::set_var("CASCADIA_SCORE_TARGET", score_target);
        std::env::set_var("MCE_SCORE_TARGET", score_target);
    }
    if let Some(policy_path) = args
        .iter()
        .position(|a| a == "--policy-weights")
        .and_then(|i| args.get(i + 1))
    {
        std::env::set_var("CASCADIA_POLICY_WEIGHTS", policy_path);
    }
    if args.iter().any(|a| a == "--az-parallel") {
        std::env::set_var("CASCADIA_AZ_PARALLEL", "1");
    }
    if let Some(threads) = arg_value(&args, &["--az-threads"]) {
        std::env::set_var("CASCADIA_AZ_THREADS", threads);
    }
    if let Some(min_sims) = arg_value(&args, &["--az-min-sims-per-thread"]) {
        std::env::set_var("CASCADIA_AZ_MIN_SIMS_PER_THREAD", min_sims);
    }
    ...
    let run_collect_czero = args.iter().any(|a| a == "--collect-czero");
    let run_train_czero = args.iter().any(|a| a == "--train-czero");
    let run_alpha_zero_collect_mode = args
        .iter()
        .any(|a| a == "--az-collect" || a == "--collect-az");
    let run_alpha_zero_train = args
        .iter()
        .any(|a| a == "--az-train" || a == "--alphazero-train");
    ...
    if run_alpha_zero_train {
        run_alpha_zero_training(&args, num_games);
        return;
    }
    if run_alpha_zero_collect_mode {
        run_alpha_zero_collect(&args, num_games);
        return;
    }
    ...
    if args.iter().any(|a| a == "--train-policy") {
        ...
        let rollouts_per_cand: usize = args
            .iter()
            .position(|a| a == "--rollouts-per-cand")
            .and_then(|i| args.get(i + 1))
            .and_then(|s| s.parse().ok())
            .unwrap_or(100);
        let max_cands: usize = args
            .iter()
            .position(|a| a == "--max-candidates")
            .and_then(|i| args.get(i + 1))
            .and_then(|s| s.parse().ok())
            .unwrap_or(50);
        let temperature: f32 = args
            ...
            .and_then(|s| s.parse().ok())
            .unwrap_or(2.0);
        let policy_lr: f32 = args
            .iter()
            .position(|a| a == "--policy-lr")
            .and_then(|i| args.get(i + 1))
            .and_then(|s| s.parse().ok())
            .unwrap_or(0.01);
        let epochs: usize = args
            ...
            .and_then(|s| s.parse().ok())
            .unwrap_or(20);
        let seed_base: u64 = std::env::var("CASCADIA_SEED_OFFSET")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(0);
        ...
        let load_data_path = args
            .iter()
            .position(|a| a == "--load-policy-data")
            .and_then(|i| args.get(i + 1).map(|s| s.as_str()));
        ...
        if let Some(path) = load_data_path {
            ...
            let mut all_positions: Vec<cascadia_ai::nnue::PositionPolicyData> =
            ...
            for _ in 0..n_pos {
                ...
                for _ in 0..nb {
                    f.read_exact(&mut buf2).unwrap();
                    base.push(u16::from_le_bytes(buf2));
                }
                // Candidates
                f.read_exact(&mut buf4).unwrap();
                ...
                for _ in 0..nc {
                    ...
                    for _ in 0..nf {
                        f.read_exact(&mut buf2).unwrap();
                        feats.push(u16::from_le_bytes(buf2));
                    }
                    f.read_exact(&mut buf4).unwrap();
            ...
            for epoch in 0..epochs {
                ...
                if (epoch + 1) % 10 == 0 || epoch == 0 || epoch == epochs - 1 {
                    eprint!(
                        "\r  Epoch {}/{}: loss={:.4} agree={:.1}%",
                        epoch + 1,
                        epochs,
                        eloss / en.max(1) as f64,
                        100.0 * eagree as f64 / en.max(1) as f64
                    );
                }
            }
            eprintln!(
                "\n  Training: {} epochs in {:.1?}",
                epochs,
                train_start.elapsed()
            );
            let mut fa = 0usize;
            ...
            eprintln!(
                "  Final: loss={:.4} agree={:.1}%",
                fl / fn_.max(1) as f64,
                100.0 * fa as f64 / fn_.max(1) as f64
            );
            policy_net
                .save(std::path::Path::new(weights_path))
                .expect("save");
        ...
        for g in 0..n_games {
            ...
            while !game.is_game_over() {
                ...
                if let Some((best, _)) = mce_results.first() {
                    if !execute_scored_move(&mut game, best) {
                        break;
                    }
                } else {
                    break;
        ...
        if let Some(path) = data_path {
            ...
            for pos in &all_positions {
                ...
                for &fi in &pos.base_features {
                    f.write_all(&fi.to_le_bytes()).unwrap();
                }
                // Candidates
    ...
    if run_collect_czero {
        std::env::set_var("CASCADIA_SCORE_TARGET", "with-bonus");
        std::env::set_var("MCE_SCORE_TARGET", "with-bonus");
        std::env::set_var("MCE_LMR", "1");
        std::env::set_var("MCE_DIVERSE_PREFILTER", "1");

        let weights_path = args
            .iter()
            .position(|a| a == "--weights")
            .and_then(|i| args.get(i + 1).map(|s| s.as_str()))
            .unwrap_or("nnue_weights_v4opp_modal_iter3.bin");
        let out_path = args
            .iter()
            .position(|a| a == "--out")
            .and_then(|i| args.get(i + 1).map(|s| s.as_str()))
            .unwrap_or("czero_samples.czr");
        let rollouts: usize = args
            .iter()
            .position(|a| a == "--rollouts")
            .and_then(|i| args.get(i + 1))
            .and_then(|s| s.parse().ok())
            .unwrap_or(160);
        let prefilter_k: usize = args
            .iter()
            .position(|a| a == "--prefilter-k" || a == "--czero-top-k")
            .and_then(|i| args.get(i + 1))
            .and_then(|s| s.parse().ok())
            .unwrap_or(32);
        ...
        let net = Arc::new(
            cascadia_ai::nnue::NNUENetwork::load(std::path::Path::new(weights_path))
                .expect("failed to load teacher weights"),
        );

        println!(
            "CascadiaZero collect: games={}, rollouts={}, prefilter_k={}, weights={}, out={}, random_seed={}, seed_offset={}",
            num_games, rollouts, prefilter_k, weights_path, out_path, use_random_seed, seed_offset
        );
        let start = Instant::now();
        let mut all_groups: Vec<cascadia_ai::nnue_train::CZeroGroup> = Vec::new();
        let mut pending: Vec<(usize, usize, f32, [f32; cascadia_ai::nnue::NUM_HEADS])> = Vec::new();
        let mut total_bonus_score = 0u64;
        let mut entropy = if use_random_seed {
            StdRng::from_entropy()
        } else {
            StdRng::seed_from_u64(0xC2E0_C2E0u64.wrapping_add(seed_offset))
        };

        for game_i in 0..num_games {
            let mut rng = StdRng::seed_from_u64(entropy.gen());
            let cards = ScoringCards::all_a();
            let mut game = GameState::new(4, cards, &mut rng);
            let mut search_rng = StdRng::seed_from_u64(rng.gen());
            ...
            while !game.is_game_over() {
                ...
                let player = game.current_player;
                let results = cascadia_ai::mce::nnue_rollout_mce_candidates_with_features(
                    &game,
                    &net,
                    rollouts,
                    prefilter_k,
                ...
                for (mv, features, teacher_score) in &results {
                    let mut after = game.clone();
                    if execute_scored_move(&mut after, mv) {
                        let current_score = with_bonus_breakdown(&after, player).total as f32;
                        candidates.push(cascadia_ai::nnue_train::CZeroCandidate {
                ...
                if !execute_scored_move(&mut game, &best_mv) {
                    break;
                }
                let after_score = with_bonus_breakdown(&game, player).total as f32;
                let after_subscores = czero_subscores(&game, player);
                let group_idx = all_groups.len();
                all_groups.push(cascadia_ai::nnue_train::CZeroGroup {
                    candidates,
                    value_target: 0.0,
                    subscore_targets: [0.0; cascadia_ai::nnue::NUM_HEADS],
                });
                pending.push((group_idx, player, after_score, after_subscores));
            }

            let mut final_scores = [0.0f32; 4];
            let mut final_subscores = [[0.0f32; cascadia_ai::nnue::NUM_HEADS]; 4];
            for p in 0..game.num_players {
                final_scores[p] = with_bonus_breakdown(&game, p).total as f32;
                final_subscores[p] = czero_subscores(&game, p);
            }
            for (group_idx, player, after_score, after_subscores) in pending.drain(..) {
                all_groups[group_idx].value_target = (final_scores[player] - after_score).max(0.0);
                let mut sub = [0.0f32; cascadia_ai::nnue::NUM_HEADS];
                for h in 0..cascadia_ai::nnue::NUM_HEADS {
                    sub[h] = (final_subscores[player][h] - after_subscores[h]).max(0.0);
                }
                all_groups[group_idx].subscore_targets = sub;
            }
            total_bonus_score += final_scores[0] as u64;
            eprint!(
                "\r  Game {}/{} groups={} p0_bonus_avg={:.1}    ",
                game_i + 1,
                num_games,
                all_groups.len(),
                total_bonus_score as f64 / (game_i + 1) as f64
            );
        }
        eprintln!();

        cascadia_ai::nnue_train::save_czero_data(
            std::path::Path::new(out_path),
            &all_groups,
            if cfg!(feature = "czero-feat") {
                "v3+v4-opp+v5-feat+czero-feat"
            } else {
                "build-without-czero-feat"
            },
            option_env!("GIT_REV").unwrap_or("unknown"),
            &ScoringCards::all_a(),
            cascadia_ai::score_target::ScoreTarget::WithHabitatBonus,
        )
        .expect("failed to save CZR1 data");

        println!(
            "CZR1 saved: {} groups in {:.1?} -> {}",
            all_groups.len(),
            start.elapsed(),
            out_path
        );
    ...
    if run_train_czero {
        ...
        while ai < args.len() {
            if args[ai] == "--samples" {
                ai += 1;
                while ai < args.len() && !args[ai].starts_with("--") {
                    sample_paths.extend(
                        args[ai]
                            .split(',')
        ...
        if sample_paths.is_empty() {
            sample_paths.push("czero_samples.czr".to_string());
        }
        let samples_label = sample_paths.join(",");
        let weights_path = args
            .iter()
            .position(|a| a == "--weights")
            .and_then(|i| args.get(i + 1).map(|s| s.as_str()))
            .unwrap_or("czero_weights.bin");
        let init_path = args
            .iter()
            .position(|a| a == "--init-weights")
            .and_then(|i| args.get(i + 1).map(|s| s.as_str()))
        ...
        let temperature: f32 = args
            ...
            .unwrap_or(2.0);
        let policy_path_buf;
        let policy_path = if let Some(p) = args
            .iter()
            .position(|a| a == "--policy-weights")
            .and_then(|i| args.get(i + 1).map(|s| s.as_str()))
        {
            ...
            policy_path_buf = std::path::Path::new(weights_path)
        ...
        println!(
            "CascadiaZero train: samples={}, init={}, value_out={}, policy_out={}",
            samples_label, init_path, weights_path, policy_path
        );
        let mut data =
            cascadia_ai::nnue_train::load_czero_data(std::path::Path::new(&sample_paths[0]))
                .expect("failed to load CZR1 data");
        for shard_path in sample_paths.iter().skip(1) {
            let mut shard =
                cascadia_ai::nnue_train::load_czero_data(std::path::Path::new(shard_path))
                    .expect("failed to load CZR1 shard");
            assert_eq!(
                data.feature_set, shard.feature_set,
                "CZR1 feature set mismatch between shards"
            );
            data.groups.append(&mut shard.groups);
        }
        println!(
            "  Loaded {} groups from {} shard(s) ({}, git={})",
            data.groups.len(),
            sample_paths.len(),
            data.feature_set,
            data.git_rev
        ...
        let mut net = cascadia_ai::nnue::NNUENetwork::load(std::path::Path::new(init_path))
            .unwrap_or_else(|_| cascadia_ai::nnue::NNUENetwork::new());
        let value_stats = cascadia_ai::nnue_train::train_czero_value(&mut net, &data, epochs, lr);
        net.save(std::path::Path::new(weights_path))
            .expect("failed to save CZero value weights");

        let mut positions: Vec<cascadia_ai::nnue::PositionPolicyData> = data
            .groups
            .iter()
            .map(|g| cascadia_ai::nnue::PositionPolicyData {
                base_features: Vec::new(),
                candidates: g
                    .candidates
                    .iter()
                    .map(|c| (c.features.clone(), c.teacher_score))
                    .collect(),
            })
            .filter(|p| p.candidates.len() >= 2)
            .collect();
        let mut policy_net = cascadia_ai::policy_net::PolicyNetwork::from_nnue(&net);
        let mut rng = StdRng::seed_from_u64(0xC2E0_9011);
        for epoch in 0..epochs {
            ...
            for pos in &positions {
                let (l, correct) = policy_net.train_ranking(pos, policy_lr, temperature);
                loss += l as f64;
                if correct {
                    agree += 1;
                }
            ...
            eprint!(
                "\r  CZero policy epoch {}/{}: loss={:.4} top1={:.1}%    ",
                epoch + 1,
                epochs,
                loss / n.max(1) as f64,
        ...
        eprintln!();
        policy_net
            .save_czp1_with_metadata(
                std::path::Path::new(policy_path),
                &data.feature_set,
                &data.git_rev,
            )
            .expect("failed to save CZP1 policy");
        println!(
            "Saved CZero value weights to {} (RMSE {:.2})",
            weights_path, value_stats.final_rmse
        );
        println!("Saved CZero policy weights to {}", policy_path);
        return;
    }
    ...
    if args.iter().any(|a| a == "--rank-correlation") {
        ...
        eprintln!(
            "rank-correlation: seed_offset={}, rollouts_per_cand={}, weights={}",
        ...
        while !game.is_game_over() {
            ...
            // Build MCE rank map
            let mut mce_map: std::collections::HashMap<
                CandKey,
                (usize, cascadia_ai::mce::MceStat),
            > = std::collections::HashMap::new();
...
\"hab\":{},\"wl_bear\":{},\"wl_elk\":{},\"wl_salmon\":{},\"wl_hawk\":{},\"wl_fox\":{},\"tokens\":{},\"turns_left\":{}}}",
            ...
            eprintln!(
                "  turn {} — {} expanded, {} NNUE top-{}, {} MCE-scored, {} common",
                ai_turn,
                n_cands,
                nnue_sorted.len(),
                max_cands,
    ...
    if run_exact_selfplay {
        ...
        println!(
            "Exact expectimax self-play: {} games, weights={}",
            num_games, weights_path
        );
        ...
        for game_i in 0..num_games {
            ...
            while !game.is_game_over() {
                ...
                let policy_candidates: Vec<(Vec<u16>, f32)> = results
                    .iter()
                ...
                let features =
                    cascadia_ai::nnue::extract_features_with_bag(&game.boards[0], Some(&bag_info));
                turn_value_records.push((
                    features,
                    after_bd.total as f32,
                    after_bd.wildlife_total() as f32,
                ));
            }
            ...
            let final_bear_pairs =
                cascadia_ai::nnue_train::count_bear_pairs(&game.boards[0]) as f32;
            let final_salmon_chain =
                cascadia_ai::nnue_train::longest_salmon_chain(&game.boards[0]) as f32;
            ...
            eprint!(
                "\r  Game {}/{} — final={:.0}, avg={:.1}, v={}, p={}    ",
                game_i + 1,
                num_games,
                final_score,
                avg_so_far,
                all_value_samples.len(),
                all_policy_groups.len()
            );
        }
        eprintln!();
        ...
        println!(
            "Done in {:.1?}. {} games (avg {:.1})",
            elapsed, num_games, avg_score
        );
        println!(
            "  Value: {} samples → {}",
            all_value_samples.len(),
        ...
        println!(
            "MCE value self-play: {} games, rollouts={}, weights={}",
            num_games, rollouts, weights_path
        );
        println!("  Value samples (MCV3) → {}", out_path);
        println!("  Policy samples → {}", policy_out);
        let start = Instant::now();
        ...
        for game_i in 0..num_games {
            ...
            let avg_so_far = total_final_score as f64 / (game_i + 1) as f64;
            eprint!(
                "\r  Game {}/{} — final={:.0}, avg={:.1}, v_samples={}, p_groups={}    ",
                game_i + 1,
                num_games,
                final_score,
                avg_so_far,
        ...
        let temperature: Option<f32> = args
            ...
            .and_then(|i| args.get(i + 1))
            .and_then(|s| s.parse().ok());
        let seed: u64 = std::env::var("CASCADIA_TRAIN_SEED")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(42);
        ...
        let net = init_path.and_then(|p| {
            match cascadia_ai::nnue::NNUENetwork::load(std::path::Path::new(p)) {
                Ok(n) => {
                    eprintln!("[selfplay-pool] loaded weights from {}", p);
                    Some(n)
                }
                Err(e) => {
                    eprintln!("[selfplay-pool] failed to load {}: {}", p, e);
                    None
                }
            }
        });
        ...
        #[cfg(feature = "v5-feat")]
        cascadia_ai::nnue_train::append_mce_samples_v4(std::path::Path::new(out_path), &samples)
            .expect("Failed to write shard");
        #[cfg(not(feature = "v5-feat"))]
        cascadia_ai::nnue_train::append_mce_samples_v3(std::path::Path::new(out_path), &samples)
            .expect("Failed to write shard");
        let fmt_label = if cfg!(feature = "v5-feat") {
            "MCV4"
        } else {
            "MCV3"
        };
        eprintln!(
            "[selfplay-pool] {} samples from {} games in {:.1?} → {} ({})",
            samples.len(),
            num_games,
            elapsed,
            out_path,
        ...
        let top_pct: f32 = args
            .iter()
            .position(|a| a == "--top-pct")
            .and_then(|i| args.get(i + 1))
            .and_then(|s| s.parse().ok())
            .unwrap_or(100.0);

        let net = weights_path
            .and_then(|p| cascadia_ai::nnue::NNUENetwork::load(std::path::Path::new(p)).ok());
        let strategy = if net.is_some() { "NNUE" } else { "greedy" };
        let filter_str = if top_pct < 100.0 {
            format!(", top {}%", top_pct)
        } else {
            String::new()
        };
        ...
        println!(
            "Generating {} self-play games ({}, {}{}, out={})",
        ...
        // This gives a near-noise-free measure of the AI's play quality at this iteration.
        {
            ...
            if n > 0 {
                ...
                while b < max_b {
                    let count = scores
                        .iter()
                        .filter(|&&s| (s as usize) >= b && (s as usize) < b + bucket)
                        .count();
                    let bar_len = (count * 60) / n.max(1);
                    let bar: String = "█".repeat(bar_len);
        ...
        let samples = if top_pct < 100.0 {
            ...
            println!(
                "  Top {}%: keeping {} games (score >= {})",
                top_pct, keep, cutoff
            );
            let avg_score: f64 = games[..keep]
                .iter()
        ...
        if args.iter().any(|a| a == "--tile-token-out") {
            ...
            println!(
                "  The standard --self-play pipeline only saves sparse features, not board states."
            );
        }
        return;
        ...
        let strategy = if net.is_some() { "NNUE" } else { "greedy" };
        println!(
            "Generating {} tile-token self-play games ({}, epsilon={}, out={})",
        ...
        println!(
            "  Using {} threads ({} games/thread)",
            num_threads, games_per_thread
        );

        type TTSample = (
            Vec<cascadia_ai::nnue::TileToken>,
            cascadia_ai::nnue::GlobalFeatures,
            f32,
        );
                ...
                std::thread::spawn(move || {
                    ...
                    for game_idx in 0..n_this {
                        let mut rng = rand::rngs::StdRng::seed_from_u64(
                            thread_seed.wrapping_add(game_idx as u64),
                        );
                        let cards = cascadia_core::types::ScoringCards::all_a();
                        let mut game =
                            cascadia_core::game::GameState::new(num_players, cards, &mut rng);

                        let mut snapshots: Vec<(
                            cascadia_core::board::Board,
                            cascadia_ai::nnue::BagInfo,
                        )> = Vec::new();
                        while !game.is_game_over() {
                            if game.current_player == 0 {
                                if game.can_replace_overflow().is_some() {
                                ...
                                let mv = if let Some(ref n) = *net {
                                    let cands =
                                        cascadia_ai::search::candidate_moves_decomposed(&game, n);
                                    if cands.is_empty() {
                        ...
                        for (board, bag) in &snapshots {
                            ...
                            let (tokens, global) =
                                cascadia_ai::nnue::extract_tile_tokens(board, Some(bag));
        ...
        println!(
            "Generated {} tile-token samples from {} games in {:.1?} (avg score {:.1})",
            all_samples.len(),
            num_games,
            start.elapsed(),
            avg
        ...
        let strategy = if net.is_some() { "NNUE" } else { "greedy" };
        println!(
            "Generating {} RICH tile-token self-play games ({}, epsilon={}, out={})",
            num_games, strategy, epsilon, out_path
        );
        ...
        type RichSample = (
            Vec<cascadia_ai::nnue::RichTileToken>,
            cascadia_ai::nnue::GlobalFeatures,
            f32,
        );
        let net_arc = std::sync::Arc::new(net);
                ...
                std::thread::spawn(move || {
                    ...
                    for game_idx in 0..n_this {
                        ...
                        for (board, bag) in &snapshots {
                            ...
                            let delta = (final_score as f32) - (current_score as f32);
                            let (tokens, global) =
                                cascadia_ai::nnue::extract_rich_tile_tokens(board, Some(bag));
                            results.push((tokens, global, delta));
                        }
                    }
                    (results, total_score, n_this)
        ...
        let avg = total_score as f64 / num_games as f64;
        println!(
            "Generated {} rich tile-token samples from {} games in {:.1?} (avg score {:.1})",
            all_samples.len(),
            num_games,
            start.elapsed(),
            avg
        ...
        eprintln!(
            "[external-eval] Playing {} games with external evaluator",
            num_games
        );
        ...
        for game_idx in 0..num_games {
            ...
            while !game.is_game_over() {
                ...
                for mv in &candidates {
                    ...
                    let (tokens, global) =
                        cascadia_ai::nnue::extract_tile_tokens(&g.boards[0], Some(&bag));
                    let current_score = cascadia_core::scoring::ScoreBreakdown::compute(
                        &mut g.boards[0].clone(),
                        &g.scoring_cards,
                    )
                ...
                if header[0] != 0x10 {
                    eprintln!(
                        "[external-eval] FATAL: expected PICK (0x10), got {:#x}",
                        header[0]
                    );
                    std::process::exit(1);
                }
                ...
                if chosen_idx >= candidates.len() {
                    eprintln!(
                        "[external-eval] FATAL: chosen index {} >= num candidates {}",
                        chosen_idx,
                        candidates.len()
                    );
                    std::process::exit(1);
                ...
                if !cascadia_ai::search::execute_scored_move(&mut game, &chosen) {
                    break;
                }
            }
            ...
            stdout
                ...
                .expect("stdout write");
            stdout
                .write_all(&final_score.to_le_bytes())
                .expect("stdout write");
            stdout.flush().expect("stdout flush");

            eprintln!(
                "[external-eval] Game {}/{}: {} pts ({:.1}s total)",
                game_idx + 1,
                num_games,
                final_score,
        ...
        eprintln!(
            "[external-eval] DONE: mean {:.1} across {} games",
            mean,
            all_scores.len()
        );
        return;
        ...
        eprintln!(
            "[gnn-mce-bench] {} games, rollouts={}, depth={}",
        ...
        for game_idx in 0..num_games {
            ...
            while !game.is_game_over() {
                    ...
                    .map(|chunk| {
                        ...
                        std::thread::spawn(move || {
                            ...
                            for &(ci, seed) in &work {
                                ...
                                while !gs.is_game_over() {
                                    if gs.current_player != 0 {
                                        ...
                                        match cascadia_ai::search::greedy_move(&gs) {
                                            Some(mv) => {
                                                if !cascadia_ai::search::execute_scored_move(
                                                    &mut gs, &mv,
                                                ) {
                                                    break;
                                                }
                                ...
                                let (tokens, global) = cascadia_ai::nnue::extract_tile_tokens(
                                    &gs.boards[0],
                                    Some(&bag),
                                );
                                let leaf_score = cascadia_core::scoring::ScoreBreakdown::compute(
                                    &mut gs.boards[0].clone(),
                                    &gs.scoring_cards,
                                )
                                .total as f32;
                ...
                let mut leaves_by_ci: Vec<Vec<Leaf>> = (0..candidates.len())
                    .map(|_| Vec::with_capacity(rollouts))
                    .collect();
                for h in handles {
                ...
                if chosen_idx >= candidates.len() {
                    eprintln!(
                        "[gnn-mce-bench] FATAL: chosen idx {} >= num candidates {}",
                        chosen_idx,
                        candidates.len()
                    );
                    std::process::exit(1);
            ...
            eprintln!(
                "[gnn-mce-bench] Game {}/{}: {} pts ({:.1}s total)",
                game_idx + 1,
                num_games,
                final_score,
        ...
        let mean = all_scores.iter().map(|&s| s as f64).sum::<f64>() / all_scores.len() as f64;
        eprintln!(
            "[gnn-mce-bench] DONE: mean {:.1} across {} games",
            mean,
            all_scores.len()
        );
        return;
        ...
        let raw = cascadia_ai::nnue_train::load_mce_samples(std::path::Path::new(samples_path))
        ...
        eprintln!(" done");
        println!(
            "Exported {} samples ({} features, {:.1} MB) in {:.1?}",
            samples.len(),
            num_features,
        ...
        println!(
            "Collecting MCE samples: {} games, rollouts={}, weights={}, out={}, seed={}",
            num_games,
            rollouts,
            weights_path,
            out_path,
        ...
        println!(
            "Collecting policy data: {} games, weights={}, out={}",
            num_games, weights_path, out_path
        );
        let start = Instant::now();
        ...
        for game_i in 0..num_games {
            ...
            while !game.is_game_over() {
                ...
                let candidates: Vec<(Vec<u16>, f32)> = scored
                    .iter()
                    .map(|(_, features, score)| (features.clone(), *score as f32))
                    .collect();
        ...
        eprintln!();
        println!(
            "Done in {:.1?}. {} position groups written to {}",
            start.elapsed(),
            total_groups,
            out_path
        );
        ...
        let num_threads = std::thread::available_parallelism()
            ...
            .unwrap_or(4);
        println!(
            "MCTS self-play: {} games, sims={}, temp={}, weights={}, out={}, threads={}",
            ...
            .map(|chunk| {
                ...
                std::thread::spawn(move || {
                    ...
                    for &seed in &chunk_seeds {
                        ...
                        for (group_idx, current_score) in &move_scores {
                            game_groups[*group_idx].value_target =
                                (final_score - current_score) as f32;
                        }
        ...
        // Write all groups
        cascadia_ai::nnue_train::save_policy_data(std::path::Path::new(out_path), &all_groups)
            .expect("Failed to save self-play data");

        let elapsed = start.elapsed();
        let avg_score = total_final_score as f64 / num_games as f64;
        println!(
            "Done in {:.1?}. {} groups from {} games (avg {:.1}), written to {}",
            elapsed, total_groups, num_games, avg_score, out_path
        );
        println!(
            "  {:.1}s/game wall, {:.0} groups/game, {} threads",
            elapsed.as_secs_f64() / num_games as f64,
            total_groups as f64 / num_games as f64,
            num_threads
        );
        return;
        ...
        let num_threads = std::thread::available_parallelism()
            ...
            .unwrap_or(4);
        println!(
            "MCE policy collection: {} games, rollouts={}, weights={}, out={}, threads={}",
                ...
                std::thread::spawn(move || {
                    ...
                    for &seed in &chunk_seeds {
                        ...
                        while !game.is_game_over() {
                            ...
                            let candidates: Vec<(Vec<u16>, f32)> = results
                                .iter()
                                .map(|(_, features, score)| (features.clone(), *score))
                                .collect();
        ...
        for handle in handles {
            ...
            for &s in &scores {
                ...
                eprint!(
                    ...
                    total_games,
                    num_games,
                    total_final_score as f64 / total_games as f64
                );
            }
        }
        eprintln!();
        ...
        let weights_in = args
            ...
            .position(|a| a == "--init-weights")
            .and_then(|i| args.get(i + 1).map(|s| s.as_str()));
        let weights_out = args
            .iter()
            .position(|a| a == "--weights")
            .and_then(|i| args.get(i + 1).map(|s| s.as_str()))
            .unwrap_or("nnue_weights_pairwise.bin");
        let pairwise_weight: f32 = args
            .iter()
            .position(|a| a == "--pairwise-weight")
            .and_then(|i| args.get(i + 1))
            .and_then(|s| s.parse().ok())
            .unwrap_or(0.5);
        let margin: f32 = args
        ...
        net.save(std::path::Path::new(weights_out))
            .expect("Failed to save");
        ...
        let freeze_below = if freeze_legacy {
            cascadia_ai::nnue::NUM_FEATURES_LEGACY
        } else {
            0
        };

        println!(
            "Training from MCE samples: samples={}, epochs={}, lr={}",
            samples_path, epochs, lr
        );
        if freeze_legacy {
            println!(
                "  FROZEN: only training features >= {} (new features only)",
                freeze_below
            );
        }
        ...
        let stats = cascadia_ai::nnue_train::train_from_mce_samples_with_checkpoint(
            &mut net,
            std::path::Path::new(samples_path),
            epochs,
            lr,
    ...
    if run_cache_train {
        ...
        println!(
            "Cache training: cache={}, epochs={}, lr={}",
            cache_path, epochs, lr
        );
        let start = Instant::now();
        ...
        let init_weights_path: Option<std::path::PathBuf> = args
            .iter()
            .position(|a| a == "--init-weights")
            .and_then(|i| args.get(i + 1))
        ...
        let seed: u64 = std::env::var("CASCADIA_TRAIN_SEED")
            ...
            .unwrap_or(42);
        println!(
            "Training NNUE: {} games, {} epochs, lr={}, seed={}, weights={:?}",
        ...
        if !stats.final_rmse.is_finite() {
            eprintln!(
                "  [ABORTED save: final RMSE non-finite ({}), weights file left untouched]",
                stats.final_rmse
            );
            std::process::exit(2);
        }
        net.save(&weights_path)
            .expect("Failed to save NNUE weights");
        ...
        println!(
            "Training N-tuple network: {} games, alpha={}, weights={:?}",
            train_games, alpha, weights_path
        );
        let start = Instant::now();
        ...
        println!(
            "Benchmarking {} strategies with {} games each (Card A)...\n",
            strategies.len(),
            num_games
        );
        ...
        let handles: Vec<_> = strategies
            .into_iter()
            .map(|s| thread::spawn(move || run_benchmark(&s, num_games)))
            .collect();
        ...
        let strategy = if args.iter().any(|a| a == "--beam") {
        ...
        } else if args.iter().any(|a| a == "--czero-mce") {
            std::env::set_var("CASCADIA_SCORE_TARGET", "with-bonus");
            std::env::set_var("MCE_SCORE_TARGET", "with-bonus");
            std::env::set_var("MCE_CANDIDATES", "32");
            let weights_path = args
                .iter()
                .position(|a| a == "--weights")
                .and_then(|i| args.get(i + 1).map(|s| s.as_str()))
            ...
            let rollouts = args
                ...
                .unwrap_or(600);
            let net = cascadia_ai::nnue::NNUENetwork::load(std::path::Path::new(weights_path))
                .expect("Failed to load CZero NNUE weights");
            let policy = cascadia_ai::nnue::PolicyNetwork::load(std::path::Path::new(policy_path))
                .expect("Failed to load CZero policy weights");
            Strategy::CZeroMCE {
                net: Arc::new(net),
            ...
            let alloc = args
                ...
                .map(|s| match s.as_str() {
                    ...
                    "halving-et" | "halving-early-term" => {
                        cascadia_ai::mce::GreedyMceAlloc::SeqHalvingEarlyTerm
                    }
                    "halving-ci" | "halving-conf" => cascadia_ai::mce::GreedyMceAlloc::SeqHalvingCI,
                    "sr" | "successive-rejects" => {
                        cascadia_ai::mce::GreedyMceAlloc::SuccessiveRejects
                    }
                    "halving-pw" | "halving-progressive" => {
                        cascadia_ai::mce::GreedyMceAlloc::SeqHalvingPW
                    }
                    "thompson" | "ts" => cascadia_ai::mce::GreedyMceAlloc::ThompsonSampling,
                    "mcts-pw" | "mcts" => cascadia_ai::mce::GreedyMceAlloc::MctsPW,
                    "halving-hetero" | "hetero" | "ocba" => {
                        cascadia_ai::mce::GreedyMceAlloc::SeqHalvingHetero
                    }
            ...
            let prefilter_k: usize = args
                .iter()
                .position(|a| a == "--prefilter-k")
                .and_then(|i| args.get(i + 1))
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            let exact_endgame: usize = args
                .iter()
                .position(|a| a == "--exact-endgame")
                .and_then(|i| args.get(i + 1))
                .and_then(|s| s.parse().ok())
                .unwrap_or(0);
            let net = cascadia_ai::nnue::NNUENetwork::load(std::path::Path::new(weights_path))
        ...
        } else if args.iter().any(|a| a == "--az" || a == "--alphazero") {
            let weights_path = args
                .iter()
                .position(|a| a == "--weights" || a == "--az-weights")
                .and_then(|i| args.get(i + 1).map(|s| s.as_str()))
                .unwrap_or("alphazero.azr");
            let simulations = args
                .iter()
                .position(|a| a == "--simulations" || a == "--rollouts" || a == "--az-sims")
                .and_then(|i| args.get(i + 1))
                .and_then(|s| s.parse().ok())
                .unwrap_or(64);
            // Auto-detect AZR1 vs AZR2 by reading the first 4 bytes.
            let mut magic = [0u8; 4];
            let is_v2 = std::fs::File::open(weights_path)
                .and_then(|mut f| {
                    use std::io::Read;
                    f.read_exact(&mut magic).map(|_| magic == *b"AZR2")
                })
                .unwrap_or(false);
            #[cfg(feature = "az-v2")]
            {
                if is_v2 {
                    let net = cascadia_ai::alphazero_v2::AlphaZeroNetV2::load(
                        std::path::Path::new(weights_path),
                    )
                    .expect("Failed to load AZR2 weights.");
                    Strategy::AlphaZeroV2 {
                        net: Arc::new(net),
                        simulations,
                    }
                } else {
                    let net = cascadia_ai::alphazero::AlphaZeroNet::load(std::path::Path::new(
                        weights_path,
                    ))
                    .expect("Failed to load AlphaZero weights. Train first with --az-train");
                    Strategy::AlphaZero {
                        net: Arc::new(net),
                        simulations,
                    }
            ...
            #[cfg(not(feature = "az-v2"))]
            {
                if is_v2 {
                    eprintln!(
                        "Refusing to load AZR2 weights: binary built without --features az-v2"
                    );
                    std::process::exit(1);
                }
                let net =
                    cascadia_ai::alphazero::AlphaZeroNet::load(std::path::Path::new(weights_path))
                        .expect("Failed to load AlphaZero weights. Train first with --az-train");
                Strategy::AlphaZero {
                    net: Arc::new(net),
                    simulations,
        ...
        println!(
            "Simulating {} games with {} AI (Card A)...",
            num_games, strategy
        );
        ...
        if !diags.is_empty() {
            ...
            println!(
                "      candidate_moves: {:>5} ({:.1}%)",
                wins_candidate,
                wins_candidate as f64 / n * 100.0
            );
            println!(
                "      greedy:          {:>5} ({:.1}%)",
                wins_greedy,
                wins_greedy as f64 / n * 100.0
            );
            println!(
                "      strategic:       {:>5} ({:.1}%)",
                wins_strategic,
                wins_strategic as f64 / n * 100.0
            );
            println!(
                "    Avg pre-MCE rank of winner: {:.2} (0=eval agreed, higher=MCE reranked)",
                rank_sum as f64 / n
            );
            println!(
                "    Avg Spearman correlation (eval vs MCE rank): {:.3}",
                corr_sum / n
            );
...
fn run_daemon(weights_path: &str) {
    ...
    let net = match NNUENetwork::load(std::path::Path::new(weights_path)) {
        ...
        Err(e) => {
            eprintln!(
                "DAEMON_ERROR failed to load weights {}: {}",
                weights_path, e
            );
            std::process::exit(1);
        }
...
fn daemon_pick(
    ...
    let mv: Option<ScoredMove> =
        pick_with_env(&[("MCE_LMR", "1"), ("MCE_DIVERSE_PREFILTER", "1")], || {
            let mut cands = cascadia_ai::mce::expanded_candidates(game);
            if cands.len() > 32 {
