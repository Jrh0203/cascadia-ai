...
fn evals_to_log_priors(candidates: &[ScoredMove]) -> Vec<f64> {
    ...
    candidates.iter().map(|c| (c.eval as f64 - max_eval) / PRIOR_TEMP).collect()
}
...
fn arm_simulation(
    ...
    g.shuffle_bags(rng);
    if !execute_scored_move(&mut g, action) { return 0.0; }
    ...
    while !g.is_game_over() {
        ...
        ai_plies += 1;
        if ai_plies > PLAYOUT_DEPTH { break; }
        match greedy_move(&g) {
            Some(mv) => { if !execute_scored_move(&mut g, &mv) { break; } }
            None => break,
        }
...
pub fn best_move_gumbel_mcts(
    ...
    let mut arms: Vec<GumbelArm> = candidates.iter().enumerate().map(|(i, c)| {
        GumbelArm {
            action: *c,
    ...
    for phase in 0..num_phases {
        ...
        let handles: Vec<_> = work_items.chunks(chunk_size).map(|chunk| {
            ...
            std::thread::spawn(move || {
                ...
                for &(ai, seed) in &work {
                    ...
                    let val = arm_simulation(&game, &actions[ai], &net, ai_player, &mut thread_rng);
                    results.push((ai, val));
                }
                results
            })
        ...
        if phase < num_phases - 1 {
            ...
            let mut scored: Vec<(usize, f64)> = alive.iter().map(|&i| {
    ...
    Some(ScoredMove { score: arm.q().round() as u16, ..arm.action })
}
