    ...
    match wildlife {
        ...
        Wildlife::Salmon => {
            ...
            if salmon_neighbors == 1 {
                ...
                let extension_slots: usize = adj.neighbors_of(pos)
                    .filter(|&n| {
                        let cell = board.grid.get(n);
                        cell.is_present()
                    ...
                    .count();
                if extension_slots >= 2 { 3 } else if extension_slots >= 1 { 2 } else { 1 }
    ...
    for combo in &combos {
        ...
        for &fi in frontier.iter() {
            ...
            for rot in 0..max_rot {
                ...
                placements.push(TilePlacement { q: coord.q, r: coord.r, rot, hab });
...
pub fn execute_scored_move(game: &mut GameState, mv: &ScoredMove) -> bool {
    ...
    if let Some(wmi) = mv.wildlife_market_index {
        game.execute_independent_move(
            mv.market_index, wmi, tile_coord, mv.rotation, wildlife_placement,
        )
    } else {
...
pub fn best_move_beam(
    ...
    for mv in candidates.iter().take(beam_width) {
        ...
        if execute_scored_move(&mut g, mv) {
            ...
            beam.push(BeamEntry { root_move: *mv, game: g });
        }
    }
    ...
    for _d in 1..depth {
        ...
        beam = next_beam
            .into_iter()
            .map(|(root, g, _)| BeamEntry { root_move: root, game: g })
            .collect();
    }
    ...
    best.map(|(mv, eval)| ScoredMove { score: (eval / EVAL_SCALE) as u16, ..mv })
}
