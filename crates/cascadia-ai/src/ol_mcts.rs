...
impl OLNode {
    fn new() -> Self { OLNode { visits: 0, children: Vec::new() } }
}
...
fn evals_to_priors(candidates: &[ScoredMove]) -> Vec<f64> {
    ...
    let exps: Vec<f64> = candidates.iter()
        .map(|c| ((c.eval as f64 - max_eval) / temperature).exp())
        .collect();
    let z: f64 = exps.iter().sum();
    if z > 0.0 { exps.iter().map(|&e| e / z).collect() }
    else { vec![1.0 / candidates.len() as f64; candidates.len()] }
}
...
fn simulate(
    ...
    let value = simulate(
        edge.child.as_mut().unwrap(), g, net, ai_player, depth + 1, rng,
    );
...
pub fn best_move_ol_mcts(
    ...
    while rollouts_done < num_rollouts {
        ...
        for _ in 0..batch_size {
            ...
            loop {
                ...
                if g.is_game_over() || depth >= MAX_TREE_DEPTH { break; }
                if node.children.is_empty() {
                    // Expand
                    let cands = candidate_moves_decomposed(&g, net);
                ...
                path.push(best_idx);
                let action = node.children[best_idx].action;
                if !execute_scored_move(&mut g, &action) { break; }
                g.shuffle_bags(rng);
        ...
        let handles: Vec<_> = jobs.iter().map(|(_path, g)| {
    ...
    root.children.into_iter().next().map(|edge| {
        let avg = if edge.visits > 0 { edge.total_value / edge.visits as f64 } else { 0.0 };
        ScoredMove { score: avg.round() as u16, ..edge.action }
    })
}
