...
impl MCTSEdge {
    ...
    fn puct_score(&self, parent_visits: u32) -> f64 {
        let q = self.q_value();
        let u = C_PUCT as f64 * self.prior as f64
...
fn expand_node(
    ...
    if candidates.is_empty() {
        ...
        let score = ScoreBreakdown::compute(
            &mut game.boards[player].clone(), &game.scoring_cards,
        ).total as f64;
    ...
    for mv in &candidates {
        ...
        let current = ScoreBreakdown::compute(
            &mut g.boards[player].clone(), &g.scoring_cards,
        ).total as f32;
    ...
    let priors: Vec<f32> = policy_logits.iter().map(|&l| (l - max_logit).exp() / exp_sum).collect();
    ...
    let valid: Vec<f64> = values.iter().map(|&v| v as f64).filter(|&v| v > 0.0).collect();
...
pub fn mcts_search_with_features(
    ...
    let selected = if temperature < 0.01 {
        ...
        let counts: Vec<f64> = root.children.iter()
    ...
    Some((
        ScoredMove { score: avg_value.round() as u16, ..best_edge.action },
        candidates,
    ))
}
