...
## Status at a glance (updated 2026-07-24 19:25 EDT)
...
- **Pure-wildlife catalogs (local CPU exploration, in progress):** AAAAA has
  ...
  The seven one-branch rulesets then closed exactly across john1–john4:
  all seven threshold queries were `INFEASIBLE`, with no timeout or unknown.
  ...
  **22/1,024 exact**, with 119,116 branches across 1,002 unresolved rows.
  ...
  as a balanced 90/90/90/84-query fleet batch. It certified 36/43 rows,
  ...
  Interactive field-guide viewer and its deterministic compact-data pipeline:
