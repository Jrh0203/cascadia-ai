#!/usr/bin/env python3
"""Benchmark the exact ADR 0156 C0 checkpoint on one treatment host."""
...
REPLAY_KIND = "same-host-exact-c0-serving-control"
...
def build_replay_report(
    *,
    ...
    """Bind one same-host C0 serving measurement to the production evidence."""
    ...
    if treatment_arm not in ARMS[1:] or expected_host != normalized_host:
        raise ValueError("ADR 0156 C0 replay treatment/host assignment is invalid")
    if (
        ...
        or control_report.get("optimization", {}).get("global_step") != TRAINING_STEPS
    ):
        raise ValueError("C0 origin report is not the completed ADR 0156 control")
    ...
    if (
        ...
        raise ValueError("ADR 0156 authorization is stale or malformed")
    open_data = authorization_identity.get("open_data_verification")
    if not isinstance(open_data, dict):
        raise ValueError("ADR 0156 authorization lacks its open-data proof")
    proof_id = open_data_verification_id(open_data)
    if proof_id != authorization_identity.get("open_data_verification_id"):
        raise ValueError("ADR 0156 authorization open-data identity differs")
    ...
    if observed_manifest != origin_checkpoint.get(
        "manifest_blake3"
    ) or observed_model != origin_checkpoint.get("model_blake3"):
        raise ValueError("host-paired C0 checkpoint bytes differ from john1")

    measurement = performance.get("measurement")
    complete = performance.get("complete_decisions")
    fixed = performance.get("fixed_chunk")
    parent = performance.get("parent_encode")
    memory = performance.get("memory")
    if not all(isinstance(value, dict) for value in (measurement, complete, fixed, parent, memory)):
        raise ValueError("host-paired C0 performance payload is incomplete")
    worker_runtime = measurement.get("worker_runtime")
    if (
        not isinstance(worker_runtime, dict)
        or _normalize_host(str(worker_runtime.get("host", ""))) != normalized_host
        ...
        or complete.get("groups") != VALIDATION_GROUPS
        or complete.get("actions") != VALIDATION_ACTIONS
        or complete.get("parent_encodes") != VALIDATION_GROUPS
        or complete.get("parent_encode_count_exact") is not True
    ):
        raise ValueError("host-paired C0 benchmark identity or coverage differs")
    ...
    assertions = {
        ...
        "replay_host_differs_from_control_host": normalized_host != ARM_HOSTS[CONTROL_ARM],
    ...
    scientific_identity = {
        ...
        "parent_cache_id": control_report["parent_cache_id"],
...
def _complete_validation_rows(control_report: dict[str, Any]) -> np.ndarray:
    ...
    if (
        ...
        raise ValueError("C0 origin report does not certify complete validation coverage")
...
def main() -> int:
    ...
    parser.add_argument("--treatment-arm", choices=ARMS[1:], required=True)
    ...
    control_report = _read_json(args.control_report, "C0 origin report")
    authorization = _read_json(args.authorization, "ADR 0156 authorization")
    ...
    benchmark_artifact_dir = args.output.parent / "paired-control-benchmarks" / args.treatment_arm
    performance = run_isolated_serving_benchmark(
        ...
        arm=CONTROL_ARM,
        global_step=TRAINING_STEPS,
        open_data_verification=authorization_identity["open_data_verification"],
