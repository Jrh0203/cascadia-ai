#!/usr/bin/env python3
"""Validate, classify, and render the ADR 0120 schedule treatment."""
...
ARTIFACT_ROOT = Path("artifacts/experiments/conditional-tile-optimizer-schedule-v1")
SOURCE_ROOT = Path("artifacts/experiments/conditional-tile-extended-exposure-v1")
PIPELINE_ROOT = Path(
    "artifacts/experiments/full-legal-hierarchical-factor-retrieval-pilot-v1"
)
DEFAULT_MARKDOWN = Path(
    "docs/v2/reports/conditional-tile-optimizer-schedule-v1-result.md"
)


def classify_optimizer_schedule(gates: dict[str, bool]) -> str:
    """Classify integrity before treatment strength."""
    if not gates["pipeline_passed"]:
        return "optimizer_schedule_pipeline_invalid"
    if not gates["treatment_passed"]:
        return "optimizer_schedule_tile_insufficient"
    return "optimizer_schedule_tile_sufficient"


def schedule_matches(events: list[dict[str, Any]]) -> bool:
    """Return whether every epoch records the exact frozen schedule."""
    if len(events) != EPOCHS:
        return False
    for event in events:
        epoch = int(event["epoch"])
        observed = float(event.get("learning_rate", -1.0))
        expected = late_cosine_learning_rate(epoch, EPOCHS)
        if not math.isclose(observed, expected, rel_tol=1e-5, abs_tol=1e-10):
            return False
    return True


def optimizer_schedule_gates(
    *,
    ...
    """Apply the complete frozen ADR 0120 gate set."""
    ...
    pipeline = {
        ...
        "origin_schedule_identity": (
            origin.get("schedule_id") == SCHEDULE_ID and schedule_valid
        ),
...
def render_markdown(combined: dict[str, Any]) -> str:
    """Render the immutable scientific result."""
    ...
    decision = (
        "The schedule treatment restores the learned proposal. Freeze this "
        "tile checkpoint and proceed to the complete-action selector."
        if scientific["classification"] == "optimizer_schedule_tile_sufficient"
        else (
            "The valid schedule treatment is insufficient. Close further "
            "exposure, sampling, and optimizer-schedule variants for this "
            "conditional pointwise tile ranker."
            if scientific["classification"] == "optimizer_schedule_tile_insufficient"
            else "The pipeline is invalid. Repair only the failed integrity condition."
        )
    )
    return f"""# Conditional Tile Optimizer Schedule V1 Result
...
`{SCHEDULE_ID}`: `{scientific["schedule_valid"]}`.
Maximum train recall was `{trajectory["maximum_train_recall"]:.2%}`.
...
def combine(
    *,
    ...
    if len(origin_paths) != 1 or len(replay_paths) != 1:
        raise ValueError("ADR 0120 origin or replay coverage is incomplete")
    ...
    gates = optimizer_schedule_gates(
    ...
    source_comparison = {
        ...
        "train_recall_delta": (
            origin["train"]["target_factor_recall"]
            - source_origin["train"]["target_factor_recall"]
        ),
        ...
        "train_exact_delta": (
            origin["train"]["exact_query_fraction"]
            - source_origin["train"]["exact_query_fraction"]
        ),
    ...
    scientific = {
        "classification": classify_optimizer_schedule(gates),
        "gates": gates,
        "origin": origin,
        "replay": replay,
        "replay_comparison": compare_stage_replay(origin, replay),
        ...
        "source_adr0118_scientific_blake3": source_combined["scientific_blake3"],
