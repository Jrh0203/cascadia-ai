...
from t1_horizon_search_queue import (
    PRIMARY_HOSTS,
    REPLAY_HOSTS,
    TASK_PREFIX,
    task_specs,
...
def _tasks() -> list[dict]:
    return task_specs(
        bundle_relative=Path("artifacts/experiments/t1hs/bundles") / ("a" * 64),
        bundle_id="a" * 64,
    )


def test_campaign_has_four_concurrent_primaries_and_rotated_replays() -> None:
    assert TASK_PREFIX == "t1hs-v1"
    tasks = _tasks()
...
def test_commands_use_bundle_and_keep_all_closed_domains_closed() -> None:
    tasks = _tasks()
    bundle_id = "a" * 64
    for task in tasks:
    ...
    for task in run_tasks:
        ...
        assert any(value.endswith("/t1-horizon-search") for value in command)
        assert "RAYON_NUM_THREADS=10" in command
        assert "--role" in command
        assert "--host" in command
        assert any("/runs-v1/" in value for value in command)


def test_all_inputs_are_fanned_out_and_verified() -> None:
    tasks = _tasks()
    by_id = {task["id"]: task for task in tasks}
    for name in ("bundle", "dataset", "cohort", "model", "authorization"):
    ...
    for host in PRIMARY_HOSTS.values():
        ...
        assert "complete-action-graded-oracle-v1-train" in command
        assert "legacy-nnue-v4opp-mlx-v1" in command
        assert "authorization-package-v1" in command
        assert "preflights-v1" in command
...
def test_collection_binds_every_primary_and_replay_report() -> None:
    ...
    assert "runs-v1" in joined
    assert "collected-v1" in joined
