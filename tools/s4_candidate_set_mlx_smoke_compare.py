#!/usr/bin/env python3
"""Validate the frozen john1/john4 ADR 0153 numerical-parity smoke."""
...
PASS = "s4_candidate_context_mlx_cross_host_smoke_pass"
INVALID = "s4_candidate_context_mlx_cross_host_smoke_invalid"


class SmokeParityError(ValueError):
    """The bounded S4 cross-host smoke is malformed or outside tolerance."""
...
def compare_smoke(
    ...
    if (left["host"], right["host"]) != EXPECTED_HOSTS:
        raise SmokeParityError("S4 smoke reports must be ordered john1 then john4")
    for field in (
        "arm",
        "cache_id",
        "s1_cache_id",
        ...
        if left.get(field) != right.get(field):
            raise SmokeParityError(f"S4 smoke shared field differs: {field}")
    ...
    if left_batches != right_batches or left_candidates != right_candidates:
        raise SmokeParityError("S4 smoke scientific batch identity differs")
    ...
    for field in (
        ...
        if left_model.get(field) != right_model.get(field):
            raise SmokeParityError(f"S4 smoke initialization differs: {field}")

    left_parity = left["initial_prediction_parity"]
    right_parity = right["initial_prediction_parity"]
    if (
        left_parity != right_parity
        or left_parity.get("scores_byte_identical") is not True
        or left_parity.get("standard_errors_byte_identical") is not True
    ):
        raise SmokeParityError("S4 zero-context warm-start parity differs")
    ...
    if left_panel["action_hashes"] != right_panel["action_hashes"]:
        raise SmokeParityError("S4 smoke prediction-panel identities differ")
    ...
    for name, values in (
        ...
        if not np.all(np.isfinite(values)):
            raise SmokeParityError(f"S4 smoke contains nonfinite {name}")
    ...
    if parameter["parameters"] != left_model["parameter_count"]:
        raise SmokeParityError(
            "S4 smoke checkpoint parameter count differs from the report"
        )
    ...
    checks = {
        ...
        "zero_context_warm_start_exact": left_parity == right_parity,
    ...
    if not all(checks.values()):
        raise SmokeParityError(f"S4 smoke numerical parity failed: {checks}")
    ...
    identity = {
        ...
        "cache_id": left["cache_id"],
        "s1_cache_id": left["s1_cache_id"],
        "context_cache_id": left["context_cache_id"],
        "warm_start": left["warm_start"],
        "report_ids": {
...
def _read_smoke_report(path: Path) -> dict[str, Any]:
    report = _read_json(path, f"S4 smoke report {path}")
    ...
    if (
        ...
        raise SmokeParityError(f"S4 smoke report is malformed: {path}")
    ...
    if (
        ...
        raise SmokeParityError(f"S4 smoke loss trace is malformed: {path}")
    return report
...
def _load_checkpoint(
    ...
    if (
        ...
        raise SmokeParityError(f"S4 smoke checkpoint checksum differs: {path}")
    tensors = mx.load(path)
    if not isinstance(tensors, dict) or not tensors:
        raise SmokeParityError(f"S4 smoke checkpoint is empty: {path}")
    return tensors
...
def _parameter_drift(
    ...
    if left.keys() != right.keys():
        raise SmokeParityError("S4 smoke checkpoint tensor names differ")
    total = 0
    changed = 0
    absolute_sum = 0.0
    maximum = 0.0
    for name in left:
        ...
        if (
            ...
            raise SmokeParityError(
                f"S4 smoke checkpoint tensor layout differs: {name}"
            )
        ...
        if (
            ...
            raise SmokeParityError(
                f"S4 smoke checkpoint tensor is nonfinite: {name}"
            )
