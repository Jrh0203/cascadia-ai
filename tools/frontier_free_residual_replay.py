#!/usr/bin/env python3
"""Compare ADR 0103 origin/replay science while excluding host wall time."""
...
def canonical_scientific_payload(report: dict[str, Any]) -> dict[str, Any]:
    if report.get("experiment_id") != EXPERIMENT_ID:
        raise ValueError("unexpected ADR 0103 experiment identity")
    scientific = report.get("scientific")
    if not isinstance(scientific, dict):
        raise ValueError("ADR 0103 report is missing scientific payload")
...
def compare_reports(
    ...
    if origin_payload.get("arm") != replay_payload.get("arm"):
        raise ValueError("origin and replay arm identities differ")
    if origin_payload.get("group_index") != replay_payload.get("group_index"):
        raise ValueError("origin and replay neural shard identities differ")
    origin_digest = canonical_digest(origin_payload)
    replay_digest = canonical_digest(replay_payload)
    return {
        "schema_version": 1,
        "experiment_id": EXPERIMENT_ID,
        "arm": origin_payload["arm"],
        "group_index": origin_payload.get("group_index"),
