...
mod tests {
    ...
    fn rejects_duplicate_keys_at_every_depth() {
        let object = from_str::<Value>(r#"{"outer":{"value":1,"value":2}}"#).unwrap_err();
        assert!(object.to_string().contains("duplicate object key"));

        let array = from_str::<Value>(r#"{"outer":[{"value":1,"value":2}]}"#).unwrap_err();
        assert!(array.to_string().contains("duplicate object key"));
    }

    #[test]
    fn rejects_trailing_json_and_accepts_unique_objects() {
