...
import pytest
import relational_substrate_mlx_smoke_compare as smoke_tool


def _checkpoint(path: Path, values: list[float]) -> None:
    mx.save_safetensors(
        path,
        {"weight": mx.array(values, dtype=mx.float32)},
    )
...
def _report(
    ...
    *,
    ...
    report = {
        ...
        "protocol": {"seed": 2026061716},
        "model": {
            "parameter_count": 3,
            "parameter_layout_blake3": "5" * 64,
            "initial_parameter_tensor_blake3": "6" * 64,
            "final_parameter_tensor_blake3": "7" * 64,
        },
        "optimization": {
            "global_step": 10,
            "candidates": sum(
...
def _reseal(report: dict) -> dict:
    report["scientific_identity"] = (
        smoke_tool._report_scientific_identity(report)
    )
    report["report_id"] = smoke_tool._canonical_blake3(
        report["scientific_identity"]
    )
    return report
...
def _fixture(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    _write(
        right_report,
        _report("john4", right_checkpoint, offset=1e-6),
    )
    return (
        left_report,
        right_report,
...
def test_numerically_equivalent_smoke_passes(
    tmp_path: Path,
) -> None:
    ...
    assert (
        result["scientific_identity"]["checks"][
    ...
    assert (
        result["scientific_identity"]["checks"][
            "panel_stable_ranking_exact"
        ]
        is True
    )


def test_batch_drift_fails_closed(tmp_path: Path) -> None:
    paths = _fixture(tmp_path)
    right = json.loads(paths[1].read_text())
    right["optimization"]["loss_trace"][5][
        "batch_blake3"
    ] = "9" * 64
    _reseal(right)
    _write(paths[1], right)
    with pytest.raises(
        smoke_tool.SmokeParityError,
        match="batch identity",
    ):
        smoke_tool.compare_smoke(*paths)


def test_parameter_drift_outside_tolerance_fails(
    tmp_path: Path,
) -> None:
    paths = _fixture(tmp_path)
    _checkpoint(paths[3], [1.01, 2.0, 3.0])
    right = json.loads(paths[1].read_text())
    right["checkpoint"]["model_blake3"] = smoke_tool._checksum(
        paths[3]
    )
    _reseal(right)
    _write(paths[1], right)
    with pytest.raises(
        smoke_tool.SmokeParityError,
        match="numerical parity",
    ):
        smoke_tool.compare_smoke(*paths)


def test_unsealed_report_mutation_fails_closed(
    tmp_path: Path,
) -> None:
    paths = _fixture(tmp_path)
    right = json.loads(paths[1].read_text())
