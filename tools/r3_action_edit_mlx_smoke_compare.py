#!/usr/bin/env python3
"""Validate the frozen john1/john4 ADR 0150 numerical-parity smoke."""
...
ADR_ID = "0150"
SMOKE_ARM = "t3-r3-radius1-global"
SMOKE_STEPS = 10
...
PASS = "r3_action_edit_mlx_cross_host_smoke_pass"
INVALID = "r3_action_edit_mlx_cross_host_smoke_invalid"
...
def compare_smoke(
    ...
    if (left["host"], right["host"]) != EXPECTED_HOSTS:
        raise SmokeParityError("R3 smoke reports must be ordered john1 then john4")
    for field in ("arm", "cache_id", "s1_cache_id", "protocol"):
        if left.get(field) != right.get(field):
            raise SmokeParityError(f"R3 smoke shared field differs: {field}")
    ...
    if left_batches != right_batches or left_candidates != right_candidates:
        raise SmokeParityError("R3 smoke scientific batch identity differs")
    ...
    for field in (
        ...
        if left_model.get(field) != right_model.get(field):
            raise SmokeParityError(f"R3 smoke initialization differs: {field}")
    ...
    if left_panel["action_hashes"] != right_panel["action_hashes"]:
        raise SmokeParityError("R3 smoke prediction-panel action identities differ")
    ...
    for name, values in (
        ...
        if not np.all(np.isfinite(values)):
            raise SmokeParityError(f"R3 smoke contains nonfinite {name}")
    ...
    if parameter["parameters"] != left_model["parameter_count"]:
        raise SmokeParityError("R3 smoke checkpoint parameter count differs from the report")
    ...
    if not all(checks.values()):
        raise SmokeParityError(f"R3 smoke numerical parity failed: {checks}")
...
def _read_smoke_report(path: Path) -> dict[str, Any]:
    report = _read_json(path, f"R3 smoke report {path}")
    ...
    if (
        ...
        raise SmokeParityError(f"R3 smoke report is malformed: {path}")
    ...
    if (
        ...
        raise SmokeParityError(f"R3 smoke loss trace is malformed: {path}")
    return report
...
def _load_checkpoint(path: Path, report: dict[str, Any]) -> dict[str, mx.array]:
    if not path.is_file() or _checksum(path) != report["checkpoint"].get("model_blake3"):
        raise SmokeParityError(f"R3 smoke checkpoint checksum differs: {path}")
    tensors = mx.load(path)
    if not isinstance(tensors, dict) or not tensors:
        raise SmokeParityError(f"R3 smoke checkpoint is empty: {path}")
    return tensors
...
def _parameter_drift(
    ...
    if left.keys() != right.keys():
        raise SmokeParityError("R3 smoke checkpoint tensor names differ")
    total = 0
    changed = 0
    absolute_sum = 0.0
    maximum = 0.0
    for name in left:
        ...
        if left_value.shape != right_value.shape or left_value.dtype != right_value.dtype:
            raise SmokeParityError(f"R3 smoke checkpoint tensor layout differs: {name}")
        ...
        if not np.all(np.isfinite(left_float)) or not np.all(np.isfinite(right_float)):
            raise SmokeParityError(f"R3 smoke checkpoint tensor is nonfinite: {name}")
